...
const hash=crypto.createHash('sha256').update(fs.readFileSync('apps/core/extension/collections/分支武将/extension.js')).digest('hex');
...
fs.writeFileSync('apps/core/extension/collections/分支武将/info.json',JSON.stringify({name:'分支武将',author:'分支原作者；增量整合 PXLNGU',version:'1.0.0',intro:'只增量添加分支武将与依赖；同名定义保留主工程版本。'},null,2)+'\n');
