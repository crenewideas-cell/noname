...
const testedSkin=process.env.NONAME_TEST_SKIN==='stock'?'':process.env.NONAME_TEST_SKIN||'builtin-decade-ingame';
const output=process.env.NONAME_LOGIC_REPORT||'docs/references/decade-ingame-ui/evidence/logic-regression.json';
...
try{for(const mode of process.argv.slice(2).length?process.argv.slice(2):['identity','guozhan','doudizhu','versus']){
 ...
 for(const skin of ['', testedSkin]){
  ...
  try{
   ...
   await page.evaluate(async({skin,mode})=>{const{lib,game}=await import('/noname.js');for(const n of lib.config.extensions)await game.promises.saveConfig('extension_'+n+'_enable',false);for(const[k,v]of Object.entries({extension_auto_import:false,new_tutorial:true,version:lib.version,show_splash:'off',mode,game_speed:'vfast',characters:['standard'],cards:['standard']}))await game.promises.saveConfig(k,v);await game.promises.saveConfig('player_number','5','identity');await game.promises.saveConfig('player_number','4','guozhan');await lib.uiWorkshop.use(skin);localStorage.setItem(lib.configprefix+'directstart',true);},{skin,mode});
   ...
   // The engine defers pile construction through onfree. Wait for the same
   // ready state before either skin receives input, otherwise selection can
   // race the shuffle and change RNG order even in two stock-skin runs.
   await page.waitForFunction(async()=>{const{lib,ui,_status}=await import('/noname.js');return !lib.onfree&&!_status.waitingForCards&&ui.cardPile.childElementCount>0;},null,{timeout:60000});
   await page.waitForFunction(async()=>{const{backgroundTasks}=await import('/noname/util/backgroundTasks.js');return backgroundTasks.jobs.length===0;},null,{timeout:60000});
   // Deferred menu/card jobs may be queued after the first empty-queue sample.
   // Require a quiet interval while the real choice is still awaiting input.
   await page.waitForFunction(()=>{const now=performance.now(),count=window.__randomLog.length;if(window.__quietCount!==count){window.__quietCount=count;window.__quietSince=now;return false;}return now-window.__quietSince>=2000;},null,{timeout:60000});
   run.candidates=await page.locator('#arena .button.character.selectable').evaluateAll(nodes=>nodes.map(n=>n.link));
   run.beforeInput=await page.evaluate(async()=>{const{lib,ui,_status}=await import('/noname.js');return {rng:window.__randomState,calls:window.__randomLog.length,mode:lib.config.mode,pile:ui.cardPile.childElementCount,event:_status.event.name};});
 ...
 await fs.writeFile(output,JSON.stringify({at:new Date().toISOString(),report},null,2));console.log(mode,pair.passed?'passed':pair.failure?.split('\n')[0]);
}}
