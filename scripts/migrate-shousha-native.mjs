...
manifest.description='手杀标准UI：完整启动动画、登录与大厅场景、全屏选将、换将换牌、对局控件、皮肤、标记及结算。九个界面模块封装为独立套装，不展示内部扩展；联机房间沿用项目实现。';
...
if(registration){
 registration.files={};
 for(const file of ['extension.js','native-runtime.js','native.css','ui-workshop.json'])registration.files[file]=crypto.createHash('sha256').update(fs.readFileSync(path.join(target,file))).digest('hex');
