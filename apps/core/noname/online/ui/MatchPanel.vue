<template>
  <section class="online-panel online-match">
    ...
    <p v-if="modeId === 'identity'">仅支持 5 / 8 人场 · 点将开启 · 主公先选，其余玩家同时共享抢选 · 每人可换候选 2 次、使用手气卡 1 次 · 开局准备 60 秒 · 回合操作 30 秒。</p>
...
const capacity = ref<number>(numbers.value[0] || 5), busy = ref(false), error = ref(''), notice = ref(''), now = ref(Date.now());
watch(() => props.modeId, () => { capacity.value = numbers.value[0] || 5; });
