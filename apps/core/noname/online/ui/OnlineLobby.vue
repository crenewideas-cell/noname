  ...
  <main class="online-lobby">
    ...
    <template v-else>
      <section v-if="s.room" class="online-room online-panel">
        ...
        <header><div><p class="online-eyebrow">{{ roomState[s.room.state] }}</p><h2>{{ s.room.name }}</h2><p>房间码 {{ s.room.code }} · {{ ruleName }} · {{ s.room.capacity }} 人</p></div><button :disabled="busy || s.room.state === 'starting'" @click="leaveRoom">离开房间</button></header>
        ...
        <div class="online-room-bottom"><section><h3>房间消息</h3><div class="online-chat" aria-live="polite"><p v-if="!s.chat.length">分享房间码，邀请朋友加入。房间消息仅本次会话保留。</p><p v-for="message in s.chat" :key="message.id"><strong>{{ message.nickname }}</strong> {{ message.text }}</p></div><form class="online-chat-form" @submit.prevent="chat"><input v-model="chatText" maxlength="300" aria-label="房间消息" placeholder="说点什么…" /><button :disabled="busy || !chatText.trim()">发送</button></form></section><aside><h3>对局规则</h3><p>{{ ruleName }} · 标准武将/卡牌</p><p>单将 · 30 秒行动时限</p><p>全员到齐并准备后，由房主开始。</p><button @click="copyCode">{{ codeCopied ? '已复制' : '复制房间码' }}</button><button v-if="s.room.state === 'waiting'" class="online-primary" :disabled="s.maintenance || busy" @click="ready">{{ myReady ? '取消准备' : '准备就绪' }}</button><button v-if="isOwner && s.room.state === 'waiting'" :disabled="s.maintenance || busy || !canStart" @click="start">开始对局</button><button v-if="isOwner && s.room.state === 'finished'" :disabled="s.maintenance || busy" @click="run(() => roomCommand('room.rematch'))">再来一局</button><p v-if="s.room.state === 'starting'" role="status">正在准备托管对局，请保持在线…</p><p v-if="s.room.state === 'in_game'">对局正在进行，可恢复原席位继续对战。</p></aside></div>
      </section>
