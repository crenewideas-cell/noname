...
export async function precontent(config, pack) {
	var mjs = {
		...
		createLogv(text) {
		  // 来源《扩展OL》（诗笺）的代码，感谢诗笺大佬（暂离无名杀，未获得授权）
          const node = ui.create.div(".gameLogv", ui.window);
          node.innerHTML = text;
          ...
          const {
            ...
            width,
            height
          } = ui.window.getBoundingClientRect();
          const {
            width: nodeWidth,
            height: nodeHeight
          } = node.getBoundingClientRect();
          node.style.left = (x + (width - nodeWidth) / 2) / game.documentZoom + "px";
          node.style.top = (y + (height - nodeHeight) / 2) / game.documentZoom + "px";
		...
		replaceFunction(targetFunction, str, content) {
            ...
            let func = toString.replace(str, content);
            if (name) {
            	func = "function" + func.slice(name.length);
            }
            return eval(`(${func})`);
        },
	};
	...
	//lib.init.css(lib.assetURL + "extension/名将杀/main", "spinefolio");
	...
	game.addGroup("xizhou", "西周", "西周", { color: "#A81C1B"});
	game.addGroup("feichu", "无", "废除", { color: [[28, 28, 28], [54, 51, 49], [59, 57, 58], [30, 29, 29]] }, "custom");
