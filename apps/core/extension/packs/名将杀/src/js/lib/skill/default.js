...
export default {
    ...
    _mjs_nu_discard: {
        ...
        ruleSkill: true,
        filter(event, player) {
            if (!["nu", "jiu"].includes(event.card.name)) {
                return false;
            }
            if (!player.name?.startsWith("mjs")) {
                return false;
            }
            if (event.getParent().type == "dying") {
                return false;
            }
            return player.getHistory("useCard", evt => evt.card.name == event.card.name && evt.getParent().type != "dying").indexOf(event) != 0;
        },
        async content(event, trigger, player) {
