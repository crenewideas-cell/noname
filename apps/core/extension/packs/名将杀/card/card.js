...
const card = {
	        ...
	        result: {
	            target(player, target) {
	            	...
	            	if (targets.length == 0) {
	            		...
	            		return get.damageEffect(from, target, player);
	            	}
	            },
	        },
	    },
	    ...
	    async content(event, trigger, player) {
	        ...
	        if (target1.countCards("he")) {
	        	const result = await target1
	        		...
	        		.set("prompt", "选择交给" + get.translation(target2) + "一张牌" + (target1.isDamaged() ? "并回复1点体力" : ""), true)
                    .set("ai", card => {
	...
	mjsyouyunmengze: {
	    ...
	    async content(event, trigger, player) {
	    	...
	    	if (cards.length) {
	    		await target2.gain(cards.randomGet(2), target1, "giveAuto");
	    	}
	    },
	    ai: {
	        order: 10,
...
const trick = {
	...
	mjsdiaobingqianjiang: {
	    ...
	    async content(event, trigger, player) {
	    	...
	    	const result = await target
                ...
                .forResult();
            if (event.isFake || get.is.mjsWeakenedCard(event.card, player)) {
            	return;
            }
            if (!result.bool) {
            	if (get.is.mjsStrengthenedCard(card)) {
	...
	mjsjiuhan: {
	    ...
	    effect() {
	        if (result.bool == false) {
	            player.addTempSkill("mjsjiuhan_debuff");
	        }
	    },
	    get ai() {
	...
	mjsshuangdong: {
	    ...
	    judge(card) {
	        if (["spade", "diamond"].includes(get.suit(card))) {
	            return 1;
	        }
	        return -2;
	    },
	...
	mjsliehuofencheng: {
        ...
        async content(event, trigger, player) {
            ...
            const pos = "h" + (get.is.mjsWeakenedCard(event.card, player) ? "e" : "");
            ...
            if (!result2?.bool || !result2?.cards.length || !suits.includes(get.suit(result2.cards[0]))) {
                const cards = target.getCards("he", card => {
                	if (!lib.filter.cardCombustible(card, player, "mjsliehuofencheng")) {
		                return false;
		            }
                	return true;
                }).randomGets(1);
	...
	mjsduoduoyishan: {
	    ...
	    modTarget: true,
	    async content(event, trigger, player) {
	    	const num = get.is.mjsStrengthenedCard(event.card) ? 3 : get.is.mjsWeakenedCard(event.card, player) ? 1 : 2;
