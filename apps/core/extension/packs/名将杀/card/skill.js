		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					...
					return get.effect(target, "mjs_debuff_liuxue", player, target);
				})
				.forResult();
		},
		async content(event, trigger, player) {
...
const skill2 = {
	...
	mjskaishanfu_skill: {
		...
		async cost(event, trigger, player) {
			...
			const num = get.is.mjsWeakenedCard(card) ? 2 : 1;
			event.result = player
				.chooseToDiscard(get.prompt(event.skill), `你可以弃置${num}张牌，令此杀的伤害+1。`, "he", num, "chooseonly", (card, player) => {
