...
const skills = {
    ...
    mjsnewyueyanxili: {
        ...
        async content(event, trigger, player) {
            player.addTempSkill("mjsnewyueyanxili_use", { player: "phaseBegin" });
        },
        subSkill: {
            use: {
                trigger: {
                ...
                async cost(event, trigger, player) {
                    game.broadcastAll(function (card) {
                        lib.skill.mjsnewyueyanxili_use_backup.viewAs = {
                            name: card.name,
                            nature: card.nature,
                            suit: card.suit,
                            number: card.number,
    ...
    mjsnewyitongliuhe: {
        ...
        async content(event, trigger, player) {
            if (event.cost_data == "相同势力") {
                ...
                await trigger.player.changeGroup("feichu", false);
        ...
        subSkill: {
            change: {
                ...
                filter(event, player) {
                    ...
                    return groups.remove("feichu").length == 1;
                },
                async content(event, trigger, player) {
    ...
    mjsnewliuchuqiji: {
        ...
        async cost(event, trigger, player) {
            const list = lib.skill[event.skill].getList
                .filter(name => {
                    return !player.hasStorage("mjsnewliuchuqiji", name);
                })
                .randomGets(3);
            const cards = [];
...
const skills2 = {
    //华佗
    /**刮骨疗毒
     * 每个回合限1次，当一名有手牌的角色受伤后，你可以将其所有手牌移出游戏，直到其下个回合开始，然后令其中毒-1并回复1点体力。
     * */
    mjsnewguaguliaodu: {
        nobracket: true,
        audio: "mjsguaguliaodu",
        trigger: {
            global: "damageEnd",
        },
        usable: 1,
        ...
        async content(event, trigger, player) {
            ...
            if (cards.length) {
                target.addSkill("mjsnewguaguliaodu_effect");
                const next = target.addToExpansion(cards, "giveAuto", target);
                next.gaintag.add("mjsnewguaguliaodu_effect");
                await next;
            }
            if (target.hasMark("mjs_debuff_zhongdu")) {
                target.removeMark("mjs_debuff_zhongdu");
            }
    ...
    /**青囊妙手
     * 当你打出♣的牌后，可以令一名角色流血-1并回复1点体力，若其体力值已满，则改为体力上限+1并回复1点体力。
     * */
    mjsnewqingnangmiaoshou: {
        nobracket: true,
        audio: "mjsqingnangmiaoshou",
        mod: {
...
const skills3 = {
    //黄月英
    mjsoffqiaoshoujiangxin: {
        nobracket: true,
        audio: "mjsqiaoshoujiangxin",
        enable: "phaseUse",
        usable: 1,
        async precontent(event, trigger, player) {
            ...
            const result = await player
                ...
                .set("ai", () => {
                    const player = get.player();
                    if (player.canMoveCard(true, true)) {
                        return "移动场上一张牌";
                    }
                    if (player.hasSkill("mjsoffqiaoshoujiangxin_effect")) {
                        return "cancel2";
                    }
                    return "获得重放1";
                })
            ...
            player.addTempSkill("mjsoffqiaoshoujiangxin_aiCheck", {
                player: ["useCard1", "useSkillBegin", "phaseUseEnd"],
            });
            event.getParent().goto(0);
        },
        async content(event, trigger, player) {
            if (event.getParent(2).cost_data == "移动场上一张牌") {
                if (player.canMoveCard()) {
                    await player.moveCard(true);
                }
            } else {
                player.addSkill("mjsoffqiaoshoujiangxin_effect");
            }
        },
        ai: {
            order: 10,
        ...
        subSkill: {
            ...
            effect: {
                ...
                mark: true,
                intro: {
                    content: "你打出的下一张牌生效2次",
                },
            },
        },
    },
    //庞统
    mjsoffyuhuochongsheng: {
        nobracket: true,
        audio: "mjsyuhuochongsheng",
        derivation: "mjsyuhuochongsheng_faq",
        trigger: {
            player: ["phaseEnd", "dieBefore", "dieAfter"],
            global: "phaseBegin",
        },
        forced: true,
        forceDie: true,
        ...
        async content(event, trigger, player) {
            if (event.triggername == "dieAfter") {
                ...
                const result = await player
                    .chooseTarget(get.prompt(event.name), "你可以将所有牌交给一名其他角色，然后受到1点火焰伤害。", lib.filter.notMe)
                    .set("ai", target => {
                        const player = get.player();
                        const att = get.attitude(player, target);
                        return att;
                ...
                if (result?.targets?.length) {
                    const target = result.targets[0];
                    player.logSkill(event.name, target);
                    await player.give(player.getCards("he"), target);
    ...
    //吕布
    mjsoffwushuangfeijiang: {
        audio: "mjswushuangfeijiang",
        nobracket: true,
        skill_tag: ["增益"],
        mod: {
    ...
    mjsoffwanjunqushou: {
        ...
        async content(event, trigger, player) {
            const target = trigger.player;
            const result = await player
                .choosePlayerCard(target, "h", "visible", [1, Infinity])
                .set("filterButton", button => {
                    return get.name(button.link, get.owner(button.link)) == "sha";
                })
                .forResult();
            if (result?.links?.length) {
                await target.showCards(result.links);
    ...
    mjsoffshuiyanqijun: {
        nobracket: true,
        audio: "mjsshuiyanqijun",
        enable: "phaseUse",
        onremove: true,
        getNum(player) {
            return 3 - player.countMark("mjsoffshuiyanqijun");
        },
        filter(event, player) {
            if (player.hasSkill("mjsoffshuiyanqijun_used")) {
                return false;
            }
            return player.countCards("he", { suit: "spade" }) >= lib.skill.mjsoffshuiyanqijun.getNum(player);
        },
        filterCard(card) {
            const player = get.player();
            const num = lib.skill.mjsoffshuiyanqijun.getNum(player);
            if (num == 0) {
                return false;
            }
        ...
        prompt() {
            const player = get.player();
            const num = lib.skill.mjsoffshuiyanqijun.getNum(player);
            return `你可以弃置${num}张♠牌，令一名其他角色弃置所有牌并受到1点伤害。`;
        },
        async content(event, trigger, player) {
            player.addSkill("mjsoffshuiyanqijun_used");
        ...
        subSkill: {
            use: {
                ...
                locked: false,
                async cost(event, trigger, player) {
                    const num = player.countMark("mjsoffshuiyanqijun");
                    if (player.hasSkill("mjsoffshuiyanqijun_used") || num >= 3) {
                        event.result = await player
                            .chooseBool({
                                ...
                                ai() {
                                    const player = get.player();
                                    return get.effect(player, { name: "draw" }, player) > 0;
                                }
                            })
                            .forResult();
                        return;
                    ...
                    event.result = await player
                        .chooseControl("摸1张牌", "弃牌数-1", "cancel2")
                        .set("ai", () => {
                        ...
                        .set(
                            "choice",
                            (() => {
                                const hs = player.getDiscardableCards(player, "h", card => get.suit(card) == "spade");
                                if (hs.length == num - 1) {
                                    return 1;
                                }
                                return 0;
                ...
                async content(event, trigger, player) {
                    player.logSkill("mjsoffshuiyanqijun");
                    if (event.cost_data != 1) {
                        await player.draw();
                    } else {
                        player.addMark("mjsoffshuiyanqijun");
                    }
                },
            },
            used: {
    ...
    mjsoffluzhanfenghui: {
        nobracket: true,
        audio: "mjsluzhanfenghui",
        skill_tag: ["增益", "减益"],
        mod: {
            targetInRange(card, player, target) {
                if (card.name != "sha" || !card._mjsoffluzhanfenghui) {
                    return;
                }
                if (player == target) {
                    return true;
        ...
        filter(event, player) {
            ...
            if (!event.target.hasCards("he", card => lib.filter.cardDiscardable(card, event.target))) {
                return false;
            }
            return true;
        },
        async cost(event, trigger, player) {
            trigger.card._mjsoffluzhanfenghui = true;
            event.result = await trigger.target
                .chooseCardTarget({
                    prompt: get.prompt(event.skill),
                    prompt2: "你可以弃置一张牌，将此杀转移给座次为此牌点数的角色",
                    position: "he",
                    filterCard: (card, player) => {
                        if (!game.hasPlayer(target => target.getSeatNum() == get.number(card, player))) {
                            return false;
                        }
                        return lib.filter.cardDiscardable(card, player, "mjsoffluzhanfenghui");
                    },
                    filterTarget: (card, player, target) => {
                        if (target.getSeatNum() != get.number(card, player)) {
                            return false;
                        }
                        const trigger = _status.event;
                        if (lib.filter.targetEnabled2(trigger.card, trigger.source, target)) {
                            return true;
                        }
                        return false;
                    },
                ...
                .forResult();
            trigger.card._mjsoffluzhanfenghui = false;
        },
        async content(event, trigger, player) {
            const target = event.targets[0];
            await trigger.target.discard(event.cards);
    ...
    mjsoffbaiwanxiongbing: {
        ...
        async content(event, trigger, player) {
            while (true) {
                const result = await player.chooseToUse()
                    .set("openskilldialog", `###${get.prompt(event.name)}###你可以将一张战法牌当箭雨齐射打出且可以重复此流程。`)
                    .set("norestore", true)
        ...
        subSkill: {
            backup: {
                ...
                filterCard(card) {
                    return get.itemtype(card) == "card" && get.type2(card) == "trick";
                },
                position: "hs",
                ai1(card) {
                    return 7 - get.value(card);
    ...
    mjsoffqianjinshigu: {
        ...
        async content(event, trigger, player) {
            ...
            const result = await player
                .chooseCardButton(cards, "选择任意张类型各不同的牌获得（不能包含你上次发动此技能时选择过的牌名）", true, [1, Infinity])
                .set("ai", button => {
                    if (get.event().nameList.includes(button.link)) {
                        return false;
                    }
                    return !ui.selected.buttons.some(buttonx => get.type2(buttonx.link) == get.type2(button.link));
                })
                .set("ai", button => {
        ...
        ai: {
            ...
            result: {
                player(player) {
                    ...
                    if (cardList.reduce((list, card) => list.add(get.type2(card)), []).length < 3) {
                        return 0;
                    }
    ...
    mjsoffhuchi: {
        ...
        locked: false,
        filter(event, player) {
            return event.name != "chooseToUse" || !player.hasSkill("mjsoffhuchi_used");
        },
        enable: "phaseUse",
        viewAs: {
            name: "mjszhenqianduijue",
        },
        filterCard: true,
        position: "hes",
        check(card) {
        ...
        async precontent(event, trigger, player) {
            player.addTempSkill("mjsoffhuchi_used", ["phaseBefore", "phaseChange", "phaseAfter"]);
        },
        async content(event, trigger, player) {
            ...
            const result2 = await player
                .chooseToUse()
                .set("openskilldialog", `###${get.prompt(event.name)}###你可以将一张牌当作阵前对决对${get.translation(trigger.source)}打出。`)
                .set("norestore", true)
                .set("_backupevent", `${event.name}_backup`)
                .set("custom", {
                    add: {},
    ...
    mjsoffxiejiahanzhan: {
        ...
        filterOk() {
            const player = get.player();
            const es = player.getCards("e", card => get.type(card) == "equip");
            return ui.selected.cards.containsAll(...es);
        },
        async content(event, trigger, player) {
            player.awakenSkill(event.name);
...
const skills5 = {
    ...
    mjs1017001: {
        ...
        async content(event, trigger, player) {
            ...
            let num = 0;
            const result = await target
                .chooseToGive(player, "he", `你可以交给${get.translation(player)}一张牌，其令令从游戏中获得一张战法牌。`)
                .set("ai", card => {
                    if (get.event().goon) {
                        return 8 - get.value(card);
                    }
            ...
            const result2 = await player
                .chooseTarget(`徙木立信：令${get.translation(target)}对另一名角色打出一张杀`, (card, player, target) => {
                    return target != _status.event.sourcex;
                })
                .set("sourcex", target)
                .set("ai", target => {
                    const player = get.player();
                    return get.effect(target, { name: "sha" }, _status.event.sourcex, player);
                })
                .forResult();
            if (result2.bool && result2.targets?.length) {
                ...
                const result = await target
                    .chooseToUse(
                        `徙木立信：你可以对${get.translation(target2)}打出一张杀`,
                        function (card) {
            ...
            while (num > 0) {
                ...
                const result = await player
                    .chooseButton([get.translation(event.name), [list, "vcard"]], true)
                    .set("ai", button => {
                        return _status.event.sourcex.getUseValue({ name: button.link[2] });
                    })
                    .set("sourcex", target)
                    .forResult();
                if (result.bool && result.links?.length) {
    ...
    mjs1024002: {
        ...
        async content(event, trigger, player) {
            ...
            await player.give(event.cards, target);
            const skill = "mjs1024002_effect";
            target.addSkill(skill);
            target.addGaintag(event.cards, skill);
            let cards = [];
            if (target.storage[skill].has(player)) {
                cards = target.storage[skill].get(player);
            }
            cards.addArray(event.cards)
            target.storage[skill].set(player, event.cards);
        },
        subSkill: {
            effect: {
                init(player, skill) {
                ...
                mod: {
                    cardDiscardable(card, player, name) {
                        if (card.hasGaintag("mjs1024002_effect")) {
                            return false;
                        }
                    },
                    canBeDiscarded(card) {
                ...
                charlotte: true,
                filter(event, player) {
                    return player.hasHistory("lose", (evt) => evt.getParent() == event && Object.values(evt.gaintag_map).some((value) => value.includes("mjs1024002_effect")));
                },
                async content(event, trigger, player) {
                    const map = player.getStorage("mjs1024002_effect", new Map());
                    const targets = game.filterPlayer(target => {
                        return map.has(target) && map.get(target).containsSome(...trigger.cards);
                    });
                    if (targets.length) {
                        targets.add(player);
                        await game.asyncDraw(targets)
    ...
    mjs1051402: {
        ...
        async content(event, trigger, player) {
            const { target } = event;
            const skill = "mjs1051402_debuff";
            const suits = event.cards.reduce((list, card) => list.add(get.suit(card)), []);
            target.addSkill(skill);
            let list = [];
            if (target.storage[skill].has(player)) {
                list = target.storage[skill].get(player);
            }
            list.addArray(suits);
            target.storage[skill].set(player, list);
            player.addTempSkill("mjs1051402_clear", { player: "phaseBeginStart" });
        },
        ai: {
            order: 10,
            result: {
                target(player, target) {
                    if (target.hasSkill("mjs1051402_debuff")) {
                        return 0;
                    }
                    return -2;
                },
            ...
            debuff: {
                ...
                mod: {
                    cardEnabled2(card, player, target) {
                        const map = player.getStorage("mjs1051402_debuff", new Map());
                        const suits = Array.from(map.values()).flat();
                ...
                intro: {
                    content(storage, player) {
                        const map = player.getStorage("mjs1051402_debuff", new Map());
                        const suits = Array.from(map.values()).flat();
                        const str = suits.reduce((str, suit) => str + get.translation(suit), "");
                        return `你不能使用${str}花色的牌`;
                    },
                },
            },
            clear: {
                ...
                onremove(player) {
                    game.countPlayer2(current => {
                        if (current.getStorage("mjs1051402_debuff", new Map()).has(player)) {
                            current.storage.mjs1051402_debuff.set(player, []);
                            const suits = Array.from(current.getStorage("mjs1051402_debuff", new Map()).values()).flat();
                            if (!suits.length) {
                                current.removeSkill("mjs1051402_debuff");
                            }
                        }
                    }, true);
                },
    ...
    mjs1038101: {
        ...
        forced: false,
        filter(event, player) {
            return event.target != player && (event.target.hp < event.player.hp || event.target.hasSkill("mjs1038101"));
        },
        async cost(event, trigger, player) {
            event.result = await player.chooseCard(get.prompt2(event.skill, trigger.target), "he").set("ai", (card) => {
                const player = get.player();
    ...
    mjs1038102: {
        trigger: {
            global: ["gainAfter", "loseAsyncAfter", "recoverEnd"],
        },
        getIndex(event, player) {
            ...
            return game.filterPlayer(target => {
                return event.getl && event.getl(target)?.cards2?.length;
            });
        },
        filter(event, player, triggername, target) {
            if (!target?.isIn()) {
            ...
            if (event.giver !== target) {
                return false;
            }
            if (event.name === "gain") {
                return event.player != target && event.getg(event.player).length > 0;
            }
            return game.hasPlayer(current => current != target && event.getg(current).length > 0);
        },
        check(event, player, triggername, target) {
            return get.attitude(player, target) > 0;
        },
        ...
        async content(event, trigger, player) {
            const target = event.targets[0];
            if (!target.hasSkill("mjs1038101", null, null, false)) {
                await target.addSkills("mjs1038101");
            } else {
                await target.draw();
            }
        },
    ...
    mjs1072601: {
        ...
        locked: false,
        filter(event, player) {
            return !event._mjs1072601;
        },
        async content(event, trigger, player) {
            const targets = game.filterPlayer(target => target != trigger.player)
            for (const target of targets) {
                const result = await target
                    .chooseBool({
                        prompt: "是否对" + get.translation(player) + "发动【兼爱非攻】？",
                        prompt2: "将此" + (trigger.source ? "来源为" + get.translation(trigger.source) : "无来源") + "的" + trigger.num + "点伤害转移给你",
                        ai() {
                            return _status.event.bool;
                        },
                    })
                    .set("bool", get.damageEffect(trigger.player, trigger.source, target) > get.damageEffect(target, trigger.source, target))
                    .forResult();
                if (result?.bool) {
                    trigger._mjs1072601 = true;
                    trigger.cancel();
                    await target.damage(trigger.source, trigger.nature, trigger.num).set("card", trigger.card).set("cards", trigger.cards).set("_mjs1072601", true);
                    break;
                }
            }
        },
        group: ["mjs1072601_add", "mjs1072601_remove"],
        subSkill: {
            add: {
                trigger: {
                    global: "damageEnd",
                ...
                filter(event, player) {
                    return event._mjs1072601 && !event.player.hasMark("mjs1072601_mark");
                },
                prompt2: "你可令其获得标记“墨”",
                check(event, player) {
                    return get.attitude(player, event.player) > 0;
                ...
                logTarget: "player",
                async content(event, trigger, player) {
                    trigger.player.addMark("mjs1072601_mark");
                },
            },
            remove: {
                trigger: {
                ...
                locked: false,
                filter(event, player) {
                    return event.source?.isIn() && event.source.hasMark("mjs1072601_mark");
                },
                logTarget: "source",
                async content(event, trigger, player) {
                    trigger.source.clearMark("mjs1072601_mark");
                },
            },
            mark: {
                marktext: "墨",
    ...
    mjs1072602: {
        ...
        locked: false,
        filter(event, player) {
            return event.player.hasMark("mjs1072601_mark");
        },
        logTarget: "player",
        async content(event, trigger, player) {
            const target = trigger.player;
            const chooseCard = async (player, type) => {
                const list = mjs.getVCardList(type).map(info => [info, get.translation(info[2])]);
                const result = await player
                    .chooseButton(
                        [
                            [["墨家子弟"], "addNewRow"],
                            `<div><div style="width:100%;text-align:center">———— ${type == "trick" ? "战法牌" : "装备牌"} ————</div></div>`,
                            [list, "tdnodes"],
                            [
                                dialog => {
                                    dialog.css({
            ...
            if (!target.hasHistory("sourceDamage", evt => evt.player != target)) {
                await chooseCard(player, "equip");
            }
            if (game.hasGlobalHistory("changeHp", evt => {
                return evt.getParent().name == "recover" && evt.player != target && evt.source == target;
            })) {
               await chooseCard(player, "trick");
            }
        },
    },
};
