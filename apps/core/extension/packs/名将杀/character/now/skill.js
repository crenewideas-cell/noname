...
const skills = {
	//杨艳
	/**立嫡以长
	 * 登场，你选择一名其他角色，当有角色交给另外一名其他角色牌时，若获得牌的角色手牌数≥你选择的角色：你可以令交出牌的角色交给你选择的角色等量张牌；否则令其随机弃置等量张牌，若弃置的牌不足，则由获得牌的角色继续弃置。
	 * */
	mjslidiyichang: {
		nobracket: true,
	...
	/**枕膝荐从
	 * 重伤/出牌阶段限1次，你可以交给一名其他角色至少1张牌，然后令其交给你选择的另一名其他角色等量+1张牌，并且你失去1点体力。
	 * */
	mjszhenxijiancong: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	//临海公主
	/**明珠蒙尘 
	 * 登场，弃置所有牌。你的摸牌阶段改为选择一名其他角色对你打出牌堆顶的2张牌，然后你获得其无法打出的牌 。受伤，将对你造成伤害的牌放入牌堆底。其他角色出牌阶段限1次，令你可以获得其1张牌，然后你令其获得牌堆底的1张牌，此牌无法选择你为目标且无法被其他角色响应。
	 * */
	mjsmingzhumengchen: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**复为公主 
	 * 重伤，将你的武将牌上的技能修改为“临海清仇”，初始的手牌上限+4，并立即重新登场。 
	 * */
	mjsfuweigongzhu: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		derivation: "mjslinhaiqingchou",
	},
	/**临海清仇
	 * 出牌阶段限2次，你可以令一名角色获得牌堆底的2张牌，这些牌无法选择你为目标且无法被其他角色响应。 
	 * */
	mjslinhaiqingchou: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	//羊献容
	/**凤落金牢
	 * 你打出的牌无法选择其他角色为目标。每个角色的回合开始时，你受到1点伤害。当你即将受到伤害时，若你的手牌上限>0，将此伤害改为手牌上限-1。
	 * 你打出的牌无法选择其他角色为目标。每个角色的回合开始时，你受到1点伤害。当你即将受到致命伤害时，若你的手牌上限>0，将此伤害改为手牌上限-1。
	 * */
	mjsfengluojinlao: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**五废六立
	 * 其他角色出牌阶段限2次，令你的手牌上限+1，然后你可以令其摸1张牌；或令你的手牌上限-1。当你的手牌上限变为0或累计改变6次后，将你的武将技能修改为“两朝凤仪”，然后你重新登场，并立即进行1个出牌阶段。
	 * 其他角色出牌阶段限2次，令你的手牌上限+1，然后你摸1张牌；或令你的手牌上限-1，然后随机削弱其1张牌。当你的手牌上限变为0或累计改变6次后，将你的武将技能修改为“两朝凤仪”，然后你重新登场。
	 * */
	mjswufeiliuli: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		derivation: "mjsliangchaofengyi",
		global: "mjswufeiliuli_global",
		trigger: {
			player: "maxHandcardChangeAfter",
		},
		silent: true,
		popup: true,
		filter(event, player) {
			...
			return game.getAllGlobalHistory("everything", evt => evt.name == "maxHandcardChange" && evt.player == player).length >= 6;
		},
		async content(event, trigger, player) {
			if (player.skin?.name == "mjs_yangxianrong_yelandiewu") {
				player.changeSkin({ characterName: "mjs_yangxianrong" }, "mjs_yangxianrong_yelandiewu2");
			} else {
				player.changeSkin({ characterName: "mjs_yangxianrong" }, "mjs_yangxianrong_shadow");
			}
	        ...
	        for (const skill of skills) {
	        	const createSkills = mjs.addCreateSkills("mjsliangchaofengyi");
	        	//await player.addSkills(createSkills);
	        	player.addSkill(createSkills);
	        }*/
	        player.removeSkill(skills);
	        const addSkills = mjs.addCreateSkills("mjsliangchaofengyi");
        	player.addSkill(addSkills);
	        const num = get.character(player.name).maxHp;
	        ...
	        const max = get.character(player.name).maxHandcardBase;
	        if (max && typeof max === "number") {
	        	lib.skill.mjsallmax.change(player, max - player.getHandcardLimit());
	        } else {
	        	const max = get.character(player.name).maxHp;
	        	lib.skill.mjsallmax.change(player, max - player.getHandcardLimit());
	        }
	        const group = get.character(player.name).group;
		...
		intro: {
	        markcount(storage, player) {
	            return player.countMark("mjswufeiliuli_counter");
	        },
	        content(storage, player) {
	            return `手牌上限累计${player.countMark("mjswufeiliuli_counter")}次改变`;
	        },
	    },
		group: "mjswufeiliuli_counter",
		subSkill: {
			counter: {
				trigger: {
					player: "maxHandcardChangeEnd",
				},
				popup: false,
	            forced: true,
	            locked: false,
	            ...
	            async content(event, trigger, player) {
	                ...
	                player.markSkill("mjswufeiliuli");
	            },
			},
			global: {
				enable: "phaseUse",
				prompt() {
	                ...
	                const targets = game.filterPlayer(target => lib.skill.mjswufeiliuli_global.filterTarget(null, player, target));
	                let str = "令" + get.translation(targets);
	                if (targets.length > 1) {
	                    str += "中的一人";
	                }
	                str += "手牌上限+1，然后其可以令你摸1张牌，或者令其手牌上限-1。";
	                return str;
	            },
	            filter(event, player) {
	                return game.hasPlayer(target => lib.skill.mjswufeiliuli_global.filterTarget(null, player, target));
	            },
	            filterTarget(card, player, target) {
	                return target != player && target.hasSkill("mjswufeiliuli") && target.countMark("mjswufeiliuli_used") < 2;
	            },
	            selectTarget() {
	                const player = get.player();
	                const count = game.countPlayer(target => lib.skill.mjswufeiliuli_global.filterTarget(null, player, target));
	                return count > 1 ? 1 : -1;
	            },
	            line: true,
	            log: false,
	            async precontent(event, trigger, player) {
	                //event.result.targets[0].logSkill("mjswufeiliuli", player);
	            },
	            async content(event, trigger, player) {
	                const { target } = event;
	                target.addTempSkill("mjswufeiliuli_used", "phaseUseEnd");
	                target.addMark("mjswufeiliuli_used", 1, false);
	...
	/**两朝凤仪
	 * 登场，你可以选择一名其他角色，令其选择获得1个“出牌阶段限1次”的技能。出牌阶段限2次，你可以选择发动一名其他角色的“出牌阶段限1次”的技能。
	 * */
	mjsliangchaofengyi: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		initList(player) {
			game.initCharacterList();
	      	...
	      	const characters = list.filter(name => {
	      		return get.character(name).skills?.some(skill => {
	      			const info = get.info(skill);
		            if (!info || info.charlotte || !get.skillInfoTranslation(skill).length) {
		                return false;
		            }
		            var translation = get.skillInfoTranslation(skill);
		            if (!translation) {
		                return false;
		            }
	      	...
	      	player.setStorage("mjsliangchaofengyi", characters);
	    },
		enable: "phaseUse",
		usable: 2,
		filterTarget(card, player, target) {
			...
			return skills.some(skill => {
				return !player.hasStorage("mjsliangchaofengyi_used", skill);
			});
		},
		async content(event, trigger, player) {
			const target = event.target;
			const skills = get.info("mjszhujianyishi").getSkills(target).filter(skill => {
				return !player.hasStorage("mjsliangchaofengyi_used", skill);
			});
	        if (!skills.length) {
	        	return;
	        ...
	        const result = await player
	            .chooseButton(
	                [
	                    [[`###${get.translation(event.name)}###选择一个技能发动`], "addNewRow"],
	                    [
	                        dialog => {
	                            dialog.css({
	            ...
	            .set("ai", button => {
	            	const player = get.player();
	            	if (player.hasStorage("mjsliangchaofengyi_aiCheck", button.link)) {
	            		return 0;
	            	}
	            	switch (button.link) {
	            		case "mjsjianshuruye": 
	        ...
	        if (result?.links?.length) {
	        	...
	        	const skill = skills
	        		.find(skill => {
	        			...
	        			return info.content || info.chooseButton;
	        		});
	        	if (!skill) {
	        		return;
	        	}
               ...
               if (result2?.bool) {
               		player.addTempSkill("mjsliangchaofengyi_used");
               		player.markAuto("mjsliangchaofengyi_used", skills);
               } else {
               		player.addTempSkill("mjsliangchaofengyi_aiCheck");
               		player.markAuto("mjsliangchaofengyi_aiCheck", skills);
               }
	        }
		},
		ai: {
			...
			result: {
				target(player, target) {
					const skills = get.info("mjszhujianyishi").getSkills(target)
						.filter(skill => {
							return !player.hasStorage("mjsliangchaofengyi_aiCheck", skill);
						});
					return skills.length * get.sgnAttitude(player, target) * get.attitude(player, target);
				},
			},
		},
		group: "mjsliangchaofengyi_init",
		subSkill: {
			init: {
				trigger: {
			        global: "phaseBefore",
			    ...
			    filter(event, player) {
			        ...
			        if (event.name == "changeSkills") {
			            return event.addSkill.includes("mjsliangchaofengyi");
			        }
			        return (event.name != "phase" || game.phaseNumber == 0);
			    },
			    async cost(event, trigger, player) {
			        event.result = await player
			            .chooseTarget(get.prompt(event.skill), "选择一名其他角色，令其选择获得1个“出牌阶段限1次”的技能。", lib.filter.notMe)
			            .set("ai", target => {
			                const player = get.player();
			    ...
			    async content(event, trigger, player) {
			        ...
			        if (!player.storage.mjsliangchaofengyi) {
				        lib.skill.mjsliangchaofengyi.initList(player);
				    }
				    const list = player.storage.mjsliangchaofengyi.randomGets(3);
				    if (!list.length) {
				       return;
				    }
				    let num = 0,
	...
	//王濬
	/**暗筑楼船
	 * 出牌阶段限1次，你可以将1张手牌选择并转化为1张相同花色的装备牌，并获得1张相同花色的牌。回合开始时，若你的装备区牌数不是全场唯一最多，此技能的发动次数永久+1。
	 * */
	mjsanzhulouchuan: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**熔金断索
	 * 出牌阶段限1次，你可以烧毁一名装备区牌数小于你的其他角色的所有♦牌，并增加等量的出杀次数。
	 * */
	mjsrongjinduansuo: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	//东方朔
	/**奏牍三千
	 * 回合开始时，你可以令一名其他角色查看牌堆和你的手牌，然后可以从牌堆中找出并获得1张与你手牌中的1张牌完全相同的牌。
	 * 1.发动此技能时查看的牌堆是有序的。
	 * 2.完全相同的牌需要花色，点数，牌名均相同。
	 * 朔初入长安，至公车上书，凡用三千奏牍。公车令两人共持举其书，仅然能胜之。人主从上方读之，止，辄乙其处，读之二月乃尽。一《史记》
	 * 让旁人循着自己手中的文字，在牌堆中寻得同篇之章。
	 * */
	mjscoudusanqian: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**巧言善辩
	 * 出牌阶段限1次/应战，你可以将一名角色的1种类型的牌随机转化为相同类型的其他牌。
	 * 在装备区的装备也会被一起转化，但不会触发装备的打出装备效果。
	 * 刘向言少时数问长老贤人通于事及朔时者，皆曰朔口谐倡辩。--《汉书》
	 * 善辩者不必改其大类，只需换其名目，便能让一手牌另生用途。
	 * */
	mjsqiaoyanshanbian: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**答客难
	 * 限定，当你需要响应打出牌时，你可以将所有手牌当作要打出的牌打出，然后令一名角色随机添加和这些牌等量且类型相同的牌至手牌。
	 * 1.随机添加的牌的总数量与因此技能打出的手牌相同，类型一一对应。
	 * 2.响应打出牌并不要求一定在回合外。
	 * 其言专商鞅、韩非之语也，指意放荡，颇复诙谐，辞数万言，终不见用。朔因著论，设客难己，用位卑以自慰谕。-《汉书》
	 * 客难忽至，便以满手文章一并应答。
	 * */
	mjsxiaokenan: {
		nobracket: true,
	...
	//王戎
	/**道旁苦李
	 * 当有♣牌因弃置进入弃牌堆时，你可以获得这些牌，并交给一名其他角色。这些牌无法被打出或主动弃置，直到下一轮结束。
	 * 1.无法被主动弃置的牌在弃牌阶段即使大于手牌上限也无法被弃置。
	 * 2.牌在弃置过程中会先被弃置进入处理区，再从处理区进入弃牌堆。因此此技能发动时机点在刘备携民渡江和子婴末代秦王之后。
	 * */
	mjsdaopankuli: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**卖李钻核
	 * 其他角色打出获得过的你的牌，在即将进入弃牌堆时，改为由你获得。
	 * 1.其他角色获得过的你的牌一定是从你的区域直接获得的牌，若这些牌再次被其他角色获得则不会触发此技能。
	 * 2.这些牌只有因打出而即将进入弃牌堆时才会改为由你获得，若因弃置或其他动作即将进入弃牌堆，不会触发此技能。
	 * */
	mjsmailizuanhe: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**情之所钟
	 * 每个回合限1次，当你的手牌被弃置或销毁时，你可以手牌上限-1，然后随机弃置任意名其他角色各1张牌。
	 * */
	mjsqingzhisuozhong: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	//山涛
	/**八斗方醉
	 * 其他角色出牌阶段限1次，其可以查看牌堆顶的2张牌，并令你选择是否获得这2张牌，或者令你选择是否获得其2张牌。当你获得＞1张牌时，若这些牌的总点数≤8，添加1张酒到你的手牌，并且此技能发动次数+1；否则你弃置本回合所有因此技能获得的牌。
	 * */
	mjsbadoufangzui: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	/**悬丝积尘
	 * 当你即将获得＞1张牌前，你可以改为展示这些牌，然后将其中点数＞4的牌放入阁，并将其他牌按随机顺序放回牌堆。每个回合限1次，当你成为其他角色打出牌的目标时，你可以弃置阁中的1张牌，抵消此牌。
	 * */
	mjsxuansijichen: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	mjszaohuatonggong: {
		...
		getList() {
			game.initCharacterList();
			const banList = lib.skill.mjszaohuatonggong.banList;
		...
		async content(event, trigger, player) {
            ...
            event.result = await player
                ...
                .set("ai", button => {
                	...
                	const hp = get.character(name)?.hp || 1;
                    return skills.length + hp;
                })
                .forResult();
	...
	/**拆解机枢
	 * 当傀儡重伤或累计受到3次伤害时，你代替傀儡登场，并拆解傀儡，此时傀儡每剩余1点体力，你摸2张牌，并立即进行1个出牌阶段。
	 * */
	mjschaijiejishu: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		...
		filter(event, player) {
			...
			return event.name == "dying" || event._mjschaijiejishu;
		},
		async content(event, trigger, player) {
	...
	/**木革塑形
	 * 限定，你可以弃置2张♣牌，选择并制作1个新的傀儡，然后傀儡代替你登场。
	 * */
	mjsmugesuxing: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		...
		filter(event, player) {
			...
			return player.countCards("he", card => lib.skill.mjsmugesuxing.filterCard(card)) > 1;
		},
		filterCard(card) {
			return get.suit(card) == "club" && lib.filter.cardDiscardable(card, get.owner(card));
		},
		selectCard: 2,
		position: "he",
		check(card) {
	...
	//马钧
	/**水转百戏 
	 * 出牌阶段限1次，将至少1张♠牌洗回牌堆，然后选择一名角色，依次翻开牌堆顶等量张牌并自动对其打出，然后你获得其中无法被打出的牌。
	 * */
	mjsshuizhuanbaixi: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		...
		usable: 1,
		filter(event, player) {
			return player.hasCards("he", card => card.suit == "spade");
		},
		filterCard(card) {
		...
		async content(event, trigger, player) {
			...
			game.log(player, `将${get.cnNumber(cards)}张牌置入了牌堆`);
			const next = player.loseToDiscardpile(cards, ui.cardPile, "blank").set("log", false);
            next.insert_index = function () {
                return ui.cardPile.childNodes[get.rand(0, ui.cardPile.childNodes.length - 1)];
            };
            await next;
	        const result = await player
	        	.chooseTarget(`${mjs.prompt(event.name)}，选择一名角色，依次翻开牌堆顶等量张牌并自动对其打出，然后你获得其中无法被打出的牌。`, true)
	        	.set("ai", target => {
	        		const player = get.player();
	        		const att = get.attitude(player, target);
	        		return -att;
	        ...
	        if (result?.targets?.length) {
	        	...
	        	const cards = get.cards(event.cards.length);
	        	await game.cardsGotoOrdering(cards);
	            for (const card of cards) {
	            	//懒得写摆了
	            	/*const next = game.createEvent("useCardWithCardPile", false);
            		next.player = player;
            		next.target = target;
            		next.card = card;
            		next.setContent(lib.skill.mjsshuizhuanbaixi.useCardWithCardPile);
            		await next;*/
		...
		async useCardWithCardPile(event, trigger, player) {
			const { card, target } = event;
			event.compareWithCardPile = true;
			game.log("牌堆", "对", target, "使用", card);
		},
		ai: {
			order: 1,
			result: {
	...
	/**龙骨汲流
	 * 出牌阶段限1次，你可以将1张♣️牌选择并转化为1张装备牌。当你打出装备牌后，将弃牌堆中的♠️牌洗回牌堆，并且你的装备区每存在1张♣️牌，就获得1张♠️牌。
	 * 出牌阶段限1次，你可以将1张♣牌选择并转化为1张装备牌。当你打出装备牌后，将弃牌堆中的♠牌洗回牌堆，并且你的装备区每存在1张牌，就获得1张♠牌。
	 * */
	mjslonggujiliu: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		...
		usable: 1,
		filter(event, player) {
			return player.hasCards("he", card => get.suit(card) == "club");
		},
		filterCard(card) {
	    ...
	    delay: false,
	    async content(event, trigger, player) {
	    	const list = mjs.getVCardList("equip").randomGets(3);
	    	const result = await player
	    		.chooseButton(["龙骨汲流", [list, "vcard"]], true)
	    		.set("ai", button => {
	    			const player = get.player();
	    	...
	    	if (result?.links?.length) {
        		...
        		game.broadcastAll(
		            function (card, bool, player, name, suit, number) {
		...
		subSkill: {
			use: {
				...
				async content(event, trigger, player) {
					const cards = Array.from(ui.discardPile.childNodes).filter(card => card.suit == "spade");
			        if (cards.length) {
			        ...
			        const num = player.countCards("e", card => get.suit(card) == "club");
			        if (num > 0) {
			        	const cards = [];
	...
	/**巧思绝世
	 * 出牌阶段限1次，你可以将一名角色的1张牌的花色改为你选择的任意1种花色。
	 * */
	mjsqiaosijueshi: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	    ...
	    async content(event, trigger, player) {
	        if (event.cards?.length) {
		        ...
		        event.result = await player
		            .choosePlayerCard(get.translation(event.name), target, "he", true)
		            .set("ai", button => {
		                const player = get.player();
	        ...
	        if (event.result?.cards?.length) {
	            ...
	            const result = await player
	            	.chooseControl(mjs.suits)
	            	.set("prompt", `${mjs.prompt(event.name)}，选择改为任意1种花色`)
	            	.forResult();
	            game.broadcastAll(
	            	function (card, suit) {
	...
	mjsqiru: {
		...
		async content(event, trigger, player) {
			...
			const result = await target
				...
				.set("ai", card => {
					if (_status.event.getRand() < 0.35) {
	                	return Math.random();
	              	}
					return (get.number(card, false) == 1 ? 1.8 : 1) + Math.random();
				})
				.forResult();
			if (result?.cards?.length) {
				...
				const result2 = await player
					.chooseToDiscard(get.prompt(event.skill, target), `你可弃置任意张点数之和等于${num}的牌并对${get.translation(target)}造成1点伤害`, "he", [1, Infinity])
					.set("filterOk", () => {
				...
				if (result2?.bool) {
					...
					if (result2.cards?.length == 1) {
						const addSkill = mjs.addCreateSkills(event.name);
						const removeSkill = event.name;
						await player.changeSkills([addSkill], [removeSkill]);
					}
				}
			}
		},
	...
	mjssihuifuran: {
        ...
        ai: {
        	...
        	skillTagFilter(player, tag) {
	            if (player.countCards("h") !== 1 || player.getHandcardLimit() === 0) {
	              	return false;
	            }
	        },
        },
	...
	//卫玠
	/**风神秀异
	 * 每个回合限1次，当你即将受到伤害时，若你的手牌上限＞体力值，则改为减少等量的手牌上限。当你打出牌时，每回合每种类型的牌限1次，若所有其他角色手牌中都不存在与此牌相同牌名的牌，则你的手牌上限+1，并获得随机1张与其他角色手牌中相同牌名的牌。
	 * 当你即将受到伤害时，若你的手牌上限＞体力值，则改为减少等量的手牌上限。当你打出牌时，每回合每种类型的牌限1次，若所有其他角色手牌中都不存在与此牌相同牌名的牌，则你的手牌上限+1，并获得随机1张与其他角色手牌中相同牌名的牌。
	 * 当你即将受到伤害时，若你的手牌上限＞体力值，则改为减少等量的手牌上限。当你打出行动牌或战法牌时，若所有其他角色手牌中都不存在与此牌相同牌名的牌，则你的手牌上限+1，并获得随机1张与其他角色手牌中相同牌名的牌。
	...
	mjsfengshenxiuyi: {
		...
		filter(event, player) {
			if (event.name == "damage") {
				if (player.hasSkill("mjsfengshenxiuyi_used")) {
					return false;
				}
				return player.getHandcardLimit() > player.getHp(true);
			}
			if (player.hasStorage("mjsfengshenxiuyi_record", get.type2(event.card))) {
				return false;
			}
		...
		async content(event, trigger, player) {
			if (trigger.name == "damage") {
				...
				player.addTempSkill("mjsfengshenxiuyi_record");
				player.markAuto("mjsfengshenxiuyi_record", get.type2(trigger.card));
				lib.skill.mjsallmax.change(player, 1);
				const list = game.filterPlayer(current => current != player && current.hasCards("h"))
                    .reduce((list, target) => {
                        const nameList = target.getCards("h").reduce((list2, card2) => list2.add(card2.name), []);
                        list.addArray(nameList);
                        return list;
                    }, []);
                const card = get.cardPile2(card => {
	...
	mjszhuyuzaice: {
		...
		async cost(event, trigger, player) {
			const targets = game.filterPlayer(target => {
				return get.info("mjszhuyuzaice").getClosest(player, target) && target != player && target.hasCards("h");
			});
			if (!targets.length) {
				return;
			}
	...
	//乐广
	/**约言析理
	 * 回合结束时，若你在本回合的每个出牌阶段打出的牌数都≤2，直到你的下个回合开始，当其他角色主动打出战法牌后，你可以立即将1张牌当作同名战法牌打出，每回合每种牌限1次。
		...
		subSkill: {
			use: {
				...
				filter(event, player) {
					...
					if (player.hasStorage("mjsyueyanxili_used", event.card.name)) {
						return false;
					}
				...
				async content(event, trigger, player) {
					...
					event.set("onresult", result => {
		                player.addTempSkill("mjsyueyanxili_used");
		                player.markAuto("mjsyueyanxili_used", get.event().namex);
		            });
		            event.set("namex", trigger.card.name);
	...
	mjsmingjiaoyuedi: {
		...
		prompt2(event, player) {
			const num = game.filterPlayer(target => target.hasSkill("mjsmingjiaoyuedi", null, null, false)).length - 1;
			return `你可以摸${num}张牌，并进行1个额外的出牌阶段`;
		},
		async content(event, trigger, player) {
		...
		group: "mjsmingjiaoyuedi_addSkill",
		subSkill: {
			addSkill: {
				trigger: {
					global: "phaseEnd",
				...
				async content(event, trigger, player) {
					player.logSkill("mjsmingjiaoyuedi", trigger.player);
					await trigger.player.addSkills("mjsmingjiaoyuedi");
				},
			},
			record: {
	            trigger: {
	...
	//王元姬
	/**烛奸抑势
	 * 回合开始时，你可以获得一名其他角色的1张牌，并且当其发动“出牌阶段限1次”的技能后，你可以获得其2张牌，直到你的下个回合开始。
	...
	mjsduwuku: {
		...
		subSkill: {
			init: {
				...
				async content(event, trigger, player) {
					const cardList = [];
				  	const nameList = _status.cardPile.reduce((list, card) => list.add(card.name), []);
				  	for (const name of nameList) {
				  		const card = _status.cardPile.slice().filter(card => card.name == name).randomGet();
				  		if (card) {
				  			cardList.push(card);
				  		}
				  	}
	...
	mjslinggucunxun: {
		...
		filter(event, player) {
			...
			return (event.cards && event.cards?.some(card => card?.storage?.eternal_mjslinggucunxun_tag));
		},
		async content(event, trigger, player) {
			if (trigger.name == "die") {
				const cards = [];
	...
	mjsjianghudaochi: {
		...
		forced: false,
		filter(event, player) {
			return event.source && event.source?.isIn() && player.hasCards("he");
		},
		async cost(event, trigger, player) {
			event.result = await player
		...
		subSkill: {
			effect: {
				...
				async content(event, trigger, player) {
					...
					const map = player.getStorage("mjsjianghudaochi_effect", new Map());
					if (map?.has(target)) {
						const list = player.storage.mjsjianghudaochi_effect.get(target);
						player.storage.mjsjianghudaochi_effect.set(target, list.remove(trigger.player));
					}
					const cards = target.getCards("h", card => {
	...
	mjsyuanqishangqing: {
	    ...
	    removeMark(player, num) {
	    	player.removeMark("mjsyuanqishangqing", num, false);
	        game.log(player, "消耗了", "#y" + num, "点", "#g元气");
	    },
		async precontent(event, trigger, player) {
			const skillName = event.name.slice("pre_".length);
			const result = await player
			...
			if (result.bool && result.targets?.length) {
				...
				const result2 = await player
					...
					.set("prompt", `令${get.translation(target)}添加1张你选择的类型的牌至其手牌`)
					.set("ai", () => {
		...
		async content(event, trigger, player) {
			if (trigger?.name == "phaseZhunbei") {
				const num = 1 + player.countMark("mjsyuanqishangqing_add");
				lib.skill.mjsyuanqishangqing.addMark(player, num);
				player.addSkill("mjsyuanqishangqing_add");
				player.addMark("mjsyuanqishangqing_add", 1, false);
			} else {
				lib.skill.mjsyuanqishangqing.removeMark(player, num);
				const target = event.targets[0];
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					const player = get.player();
					const att = get.attitude(player, target);
					if (target == player && player.hasSkill("mjsyuanqishangqing", null, null, false)) {
						return att * 2;
					}
					return att;
				})
	...
	//陆机
	/**缘情绮靡
	 * 每个回合限1次，当一名角色打出战法牌选择目标时，若此牌名本轮没有被打出过，你可以为此牌增加另一名角色为目标。
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
	            .chooseTarget({
	                ...
	                prompt2: `为${get.translation(trigger.card)}增加另一名角色为目标`,
	                filterTarget(card, player, target) {
	                    return get.event().targets.includes(target);
	                },
	                targets,
	...
	/**陆才如海
	 * 回合开始时，你可以将1张牌当作本轮未被打出过的任意1张战法牌打出。然后直到你的下个回合开始，当一名其他角色打出本轮没有被打出过的战法牌后，你可以弃置1张牌，然后随机添加1张本轮未被打出过的战法牌到你的手牌。
	...
	mjslucairuhai: {
		...
		subSkill: {
			...
			effect: {
				...
				async cost(event, trigger, player) {
					event.result = await player
						.chooseToDiscard(get.prompt(event.skill), "你可以弃置1张牌，然后随机添加1张本轮未被打出过的战法牌到你的手牌。", "he", "chooseonly")
						.set("ai", card => {
							return 7 - get.value(card);
						})
						.forResult();
				...
				async content(event, trigger, player) {
			        ...
			        const name = list.map(info => info[2]).randomGet();
			        if (name) {
			        	const card = mjs.createCard(name);
						if (card) {
							await player.gain(card, "draw");
						}
			        }
	...
	/**华亭鹤唳
	 * 每轮结束时，你可以选择并添加1张你本局游戏打出过的战法牌到手牌，每种牌限1次。
	...
	mjshuatingheli: {
		...
		async cost(event, trigger, player) {
			const list = get.inpileVCardList(info => {
				...
				if (player.hasStorage("mjshuatingheli_record", info[2])) {
					return false;
				}
		...
		async content(event, trigger, player) {
			player.addSkill("mjshuatingheli_record");
			player.markAuto("mjshuatingheli_record", event.cost_data);
    ...
    //魏延
    /**子午奇谋
     * 每轮结束时，你可以将任意牌当作杀对一名其他角色打出，若此杀造成伤害，你摸2张牌。
    ...
    /**谁敢杀我
     * 限定，令所有其他角色依次对你打出1张杀，若未打出，则你获得其1张牌，并且出杀次数+1。
    ...
    mjsrongnadangzheng: {
	    ...
	    mod: {
	    	cardname(card, player) {
	            ...
	            if (get.itemtype(card) === "card" && card.name == "nu") {
		...
		filter(event, player) {
			...
			if (!player.hasCards("h", { name: "nu" })) return false;
    ...
    //羊祜
    /**轻裘缓带
     * 出牌阶段限1次，你可以收回自己的装备牌，然后手牌上限+1。回合结束时，若你的装备区的牌不是全场最多，你的装备上限+1，并且下个回合的回合开始阶段改为摸牌阶段；否则下个回合的摸牌阶段改为出牌阶段。
    ...
    mjsqingqiuhuandai: {
    	nobracket: true,
		audio: "ext:名将杀/audio/skill:4",
		phaseList: ["phaseZhunbei", "phaseDraw", "phaseUse", "phaseDiscard", "phaseJieshu"],
		enable: "phaseUse",
		usable: 1,
		filter(event, player) {
		...
		async content(event, trigger, player) {
	        ...
	        await player.gain(es, "gain2", "log");
	        lib.skill.mjsallmax.change(player, 1);
		},
		ai: {
			order() {
		...
		group: "mjsqingqiuhuandai_phaseChange",
		subSkill: {
			phaseChange: {
				trigger: {
					player: "phaseEnd",
				...
				locked: false,
				async content(event, trigger, player) {
					const isMaxEquip = function(player) {
					    const numberOfEquips = player.countCards("e");
				    	return game.filterPlayer().every((value) => {
					      	if (value.isOut() || value == player) {
					        	return true;
					      	}
					      	return value.countCards("e") < numberOfEquips;
					    });
					};
					if (!isMaxEquip(player)) {
						await player.mjsExpandEquip();
						const phasename = "phaseDraw";
				        game.log(player, "下回合的", "#g" + "回合开始阶段", "=>", "#g" + phasename);
				        player
				            .when("phaseBegin")
				            .then(async (event2, trigger2, player2) => {
				                player2.addTempSkill(`mjsqingqiuhuandai_${phasename}`);
				                player2.markAuto(`mjsqingqiuhuandai_${phasename}`, "phaseZhunbei");
				            });
					} else {
						const phasename = "phaseUse";
				        game.log(player, "下回合的", "#g" + "摸牌阶段", "=>", "#g" + phasename);
				        player
				            .when("phaseBegin")
				            .then(async (event2, trigger2, player2) => {
				                player2.addTempSkill(`mjsqingqiuhuandai_${phasename}`);
				                player2.markAuto(`mjsqingqiuhuandai_${phasename}`, "phaseDraw");
				            });
					}
				},
			},
    ...
    /**德冠四海
     * 出牌阶段限1次，你可以令所有角色展示2张手牌，若你展示的手牌总点数最大，随机添加1张装备牌到你的手牌。
    ...
    mjsdeguansihai: {
		...
		async content(event, trigger, player) {
	        ...
	        game.hideCardsFromPlayer(targets);
	        const isMaxNumberOfShowCards = player2 => {
	        	const resultp = map.get(player);
	        	const showCardsOfPlayer = resultp?.cards?.length ? [...resultp.cards] : [];
            	const numberOfShowCards = showCardsOfPlayer.reduce((sum, card) => sum + get.number(card, player2), 0);
            	return targets.every(target2 => {
			      	if (target2.isOut() || target2 == player2) {
			        	return true;
			      	}
			      	const resultt = map.get(target2);
			      	if (!resultt) {
			      		return true;
			      	}
			      	const showCardsOfTarget = resultt?.cards?.length ? [...resultt.cards] : [];
			      	return showCardsOfTarget.reduce((sum, card) => sum + get.number(card, target2), 0) <= numberOfShowCards;
			    });
            };
            if (isMaxNumberOfShowCards(player)) {
	...
	//刘据
	/**博望纳贤
	 * 每个回合限1次，当你失去最后的手牌时，其他角色可以交给你任意张牌；当你获得其他角色的牌时，随机添加其中1张牌的复制至手牌，每回合每种牌限1次。
    ...
    /**惧忿成戾
     * 限定，出牌阶段结束时，若你已受伤，你可以销毁所有手牌，摸2倍数量张牌，并获得1个额外的出牌阶段，直到此阶段结束时，若没有角色阵亡，你失去全部体力。
	...
	mjsshujunkeji: {
		...
		async content(event, trigger, player) {
			if (event.triggername == "phaseEnd") {
				...
				const num = Math.floor(player.countCards("h", "sha") / 2);
				const cards = [];
				while (cards.length < num) {
	...
	mjsduzhifenxue: {
	    ...
	    filter(event, player) {
	        if (event.name == "gameDraw") {
	            return player.hasCards("h", card => get.type2(card) == "trick");
	        }
	        return event.getg && event.getg(player)?.some(card => get.type2(card) == "trick");
	    },
	    async content(event, trigger, player) {
	    	const cards = (trigger?.getg?.(player) ?? player.getCards("h")).filter(card => get.type2(card) == "trick");
    		if (cards.length > 0) {
    			player.addMark(event.name, cards.length, false);
    		}
	...
	mjsbaiyidujiang: {
		...
		init(player, skill) {
			if (!_status._mjbaiyidujiang) {
				//1.11..5版本删除lib.element.player.updates改为lib.hooks.checkUpdate
                ...
                */
				_status._mjbaiyidujiang = function(player) {
					if (player.hasSkillTag("hide_hp", player)) {
						player.node.hp.style.setProperty("display", "none");
					}
					if (player.hasSkillTag("hide_hs", player)) {
				...
				lib.hooks.checkUpdate.push(_status._mjbaiyidujiang);
			}
	    ...
	    async cost(event, trigger, player) {
	        ...
	        event.result = await player
	            ...
	            .set("ai", target => {
	                ...
	                if (!player.additionalSkills?.mjsyuyin) {
	                	if (target == player) {
	                		return 5 * 11111;
	                	}
	                }
	    ...
	    async content(event, trigger, player) {
	        ...
	        const result = await player
	        	.chooseButton([
                    ...
                    [
                        dialog => {
							...
							dialog.add([list, "tdnodes"]);
							dialog.add([list, "tdnodes"]);
							if (!target.additionalSkills?.mjsyuyin) {
								const buttons = dialog.buttons.slice(list.length);
								for (const button of buttons) {
									button.style.setProperty("display", "none");
								}
							}
							dialog.add(`<div><div style="width:100%;text-align:center">选择令<span class="mj-playertext">${get.translation(target)}</span>${!target.additionalSkills?.mjsyuyin?.length ? "获得" : "切换"}<span class="mj-skilltext">余音</span></div></div>`);
							dialog.buttons.forEach(i => {
                                i.setNodeIntro(get.translation(i.link), get.skillInfoTranslation(i.link));
                            });
                            if (!target.additionalSkills?.mjsyuyin || !target.additionalSkills?.mjsyuyin?.length) {
	                        	return;
	                        }
	                        const skill = target.additionalSkills.mjsyuyin[0];
	                        const button = dialog.buttons.find(i => i.link == skill);
	                        if (button) {
	        	...
	        	.set("filterButton", button => {
	        		...
	        		return !ui.selected.buttons.some(buttonx => buttonx.link == button.link);
	        	})
	        	...
	        	.set("custom", {
	                replace: {
	                	window() {
	                        ...
	                        const skill = target.additionalSkills.mjsyuyin[0];
	                        const button = event.dialog.buttons.find(i => i.link == skill);
	                        if (button) {
	                        	button.classList.add("selected");
	                        	ui.selected.buttons.add(button);
	                        }
	...
	mjsyuyin: {
		...
		getMode: new Map([
            ["Do", {
                ...
                trigger: {
                    player: ["mjsyuyin_Do_init", "useCard", "respond"],
                },
                silent: true,
                popup: true,
                filter(event, player) {
                    ...
                    return get.type2(event.card) == "trick" && player.getHistory(event.name, evt => get.type2(evt.card) == "trick").indexOf(event) == 0;
                },
                async content(event, trigger, player) {
                    const trick = mjs.getTrick("random");
                    const card = mjs.createCard(trick);
            ...
            ["Re", {
                ...
                ai(player, target) {
                    ...
                    let eff = get.effect(target, { name: "guohe_copy2", position: "he" }, target);
                    return eff;
                },
            }],
            ["Mi", {
            ...
            ["Sol", {
                ...
                ai(player, target) {
                	...
                	const hs = target.countCards("h", "sha");
                	const eff = 0.1;
                    return 0.5 + hs * eff;
                },
            }],
	...
	mjspiqinzhiji: {
	    ...
	    async content(event, trigger, player) {
	        ...
	        while (true) {
	            if (current.hasCards("h", "mjsqu") || current == player) {
	...
	mjsguanzhongwoye: {
		...
		usable: 1,
		filter(event, player) {
			return player.hasCards("he") && game.hasPlayer(target => lib.skill.mjsguanzhongwoye.filterTarget(null, player, target));
		},
		filterTarget(card, player, target) {
			return target.hasCards("h", "mjsqu");
		},
		filterCard: true,
		position: "he",
		check(card) {
	...
	//贾诩
	/**算无遗策
	 * 你打出的战法牌无法被识破抵消；当战法牌即将对你生效时，你可以将1张牌当作识破打出。 
	...
	mjssuanwuyice: {
	    ...
	    filter(event, player) {
	    	if (event.name == "chooseToUse") {
	    		if (!player.hasCards("hs", { type: ["trick", "delay"] })) {
			        return false;
			    }
			    if (event.type != "wuxie") {
		        	return false;
	    ...
	    hiddenWuxie(player, info) {
	        ...
	        return player.hasCards("hs", { type: ["trick", "delay"] });
	    },
	},
	/**文和乱武
	...
	mjswenheluanwu: {
		...
		async content(event, trigger, player) {
			player.awakenSkill(event.name);
			player.$fullscreenpop("文和乱武", "donghan");
		...
		subSkill: {
			counter: {
				...
				async content(event, trigger, player) {
	                ...
	                while (player.countMark(event.name) >= 2) {
	...
	mjsfenggongrufa: {
		...
		ai: {
			order() {
				return get.order("mjsxialuxiangfeng") - 0.1;
			},
			result: {
				target(player, target) {
		...
		ai: {
	        effect: {
	            target(card, player, target, current) {
	              	if (card.name == "sha") {
	              		let num = player.hasSkill("mjsfeilianshi") ? 2 : 0.7;
	                	return [1, 0, 1, num];
	              	}
	              	let count = target.countMark("mjsyanmenzongmu_counter");
	              	if (count % 3 == 2) {
	              		let eff = (target.getHandcardLimit() + 1 - target.countCards("h")) * 0.6;
	              		return [1, eff];
	              	}
	            },
	        },
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", card => {
					...
					if (trigger && player.hasSkill("mjsyanmenzongmu")) {
						return 5 - get.value(card);
					}
					return 0;
				})
	...
	mjsliuwangchujun: {
	    ...
	    async cost(event, trigger, player) {
	        event.result = await player
	            ...
	            .set("ai", target => {
	                ...
	                if (target.hasSkill("mjsjuehaoshanwu")) {
	                	eff += 5;
	                }
	                return skills.length + eff;
	            })
	            .forResult();
	    },
	    async content(event, trigger, player) {
	    ...
	    subSkill: {
	        use: {
	        	...
	        	async cost(event, trigger, player) {
	        		event.result = await player
	        			.chooseTarget(`${mjs.prompt(event.skill)}，选择令1名其他角色进入连环状态`, lib.filter.notMe, true)
	        			.set("ai", target => {
	        				const player = get.player();
	...
	mjsshouliweiwang: {
		...
		popup: true,
		filter(event, player) {
			return player.hasCards("h", "mjsyibing");
		},
		check(event, player) {
	 ...
	 * 1.若回合结束时已经处于连环状态，此技能效果不会解除连环状态。
	 * 2.无法选择是否失去体力上限，一定会失去。
	 * */
	mjsfenshenjiumin: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
	...
	mjsqinianrenjia: {
		...
		filter(event, player) {
			...
			if (player.storage.mjsqinianrenjia_checkHistory) {
				return false;
			}
			...
			if (gains.length) {
				...
				const result2 = await player
					...
					.set("ai", target => {
						const player = get.player();
                        const att = get.attitude(player, target);
                        if (get.is.playerNames(target, "mjs_simaxiangru")) {
                            return att * 14;
                        }
                        return att;
					})
	...
	mjsdanglumaijiu: {
	    ...
	    filter(event, player) {
	        if (event.name == "gameDraw") {
	            return player.hasCards("h", { suit: "heart" });
	        }
	...
	mjsfengqiuhuang: {
		...
		async content(event, trigger, player) {
			...
			if (gains.length) {
				...
				const result2 = await player
					...
					.set("ai", target => {
						const player = get.player();
						const att = get.attitude(player, target);
						if (get.is.playerNames(target, "mjs_zhuowenjun")) {
							return att * 13;
						}
						return att;
					})
			...
			if (cards.length) {
				const result = await player.chooseBool(`${get.translation(event.name)}，是否将剩余牌按原顺序放回牌堆底`).forResult();
				if (result.bool) {
					game.addCardKnower(cards, player);
		            await game.cardsGotoPile(cards);
		        }
		...
		subSkill: {
			effect: {
				...
				filter(event, player) {
					...
					return event.targets?.length && player.hasMark("mjszizhedubi_effect");
				},
				async content(event, trigger, player) {
					player.logSkill("mjszizhedubi");
	...
	mjsyaomumen: {
		...
		async cost(event, trigger, player) {
			const skill = event.skill + "_history";
			if (player.storage[skill]) {
				event.result.bool = true;
			} else {
				event.result.bool = player.hasAllHistory("lose", evt => evt.cards2?.some(card => card.name == "mjsyugou"));
			}
			player.setStorage(skill, event.result.bool);
		},
		async content(event, trigger, player) {
			player.addMark(event.name, 1, false);
	...
	mjsqingguoqingcheng: {
		...
		async content(event, trigger, player) {
			...
			const result = await target
				...
				.set("ai", button => {
					const player = get.player();
					const eff = get.is.damageCard(button.link) ? 2.5 : 1;
					return player.getUseValue(button.link) + eff;
				})
				.forResult();
			if (result?.bool && result.links?.length) {
	...
	mjsshanshanlaichi: {
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				.chooseTarget(get.prompt(event.skill), "令一名其他角色摸牌至手牌上限，并且回复所有体力值。", lib.filter.notMe, true)
				.set("ai", target => {
					const player = get.player();
					const att = get.attitude(player, target);
					if (att <= 0) {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					const player = get.player();
					const att = get.sgnAttitude(player, target);
					const delt = player.countCards("h") - target.countCards("h");
					const draw = get.effect(target, { name: "draw" }, player, player);
					const save = player.getHp() + player.countCards("hs", card => player.canSaveCard(card, player));
					if (Math.abs(delt) > 1) {
						if (save <= 1 && get.damageEffect(player, player, player) <= 0) {
							return 0;
						}
					}
					if (att <= 0) {
						if (target.hasSkill("mjsmodaiqinwang") && target.hasFriend()) {
							return 0;
						}
						if (game.hasPlayer(current => {
							...
							return current.hasSkill("mjsjunshengbieli_effect") && current.getStorage("mjsjunshengbieli_effect").includes(target);
						})) {
							return (att * delt) / 2.45;
						}
					}
					if (delt > 0) {
						return draw * delt;
	...
	mjssheziquanyi: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					const player = get.player();
					const delt = target.getHandcardLimit() - target.countCards("h", card => lib.filter.cardDiscardable(card, target));
					const draw = get.effect(player, { name: "draw" }, player, player) * delt;
					return get.sgnAttitude(player, target) * draw;
				})
				.forResult();
		},
		async content(event, trigger, player) {
	              ...
	              if (get.tag(card, "damage") && target.hp > 1) {
	                ...
	                for (let i2 = 0; i2 < players.length; i2++) {
	                  if (get.attitude(target, players[i2]) > 0) {
	                    max = Math.max(players[i2].getHandcardLimit() - players[i2].countCards("h"), max);
	                  }
	                }
	                switch (max) {
	                  case 0:
	...
	mjshuangtianyaofa: {
		...
		subSkill: {
			init: {
				...
				filter(event, player) {
					if (event.name == "gameDraw") {
						return player.hasCards("h", { name: "sha" });
					}
	...
	mjsliaodihebian: {
		...
		getIndex(event, player) {
			return game.filterPlayer(function (target) {
				if (target == player || target.hasCards("h")) {
					return false;
				}
				return event?.getl?.(target)?.hs?.length;
			});
		...
		filter(event, player, triggername, target) {
			return target?.isIn() && lib.filter.targetEnabled({ name: "sha" }, player, target) && (player.hasSha() || (_status.connectMode && player.hasCards("h") > 0));
		},
		logTarget(event, player, triggername, target) {
			return target;
		},
	...
	mjschuqiwuqiong: {
		...
		usable: 1,
		filter(event, player) {
			if (_status.currentPhase == player || !player.hasCards("h")) {
				return false;
			}
			return get
				.inpileVCardList(info => {
	...
	mjsjianmie: {
		...
		async cost(event, trigger, player) {
			const targets = game.filterPlayer(target => {
				return target != player && target.hasCards("hej", cardx => lib.filter.cardDestuctible(cardx, target, event.skill));
			});
			if (!targets.length) {
				return;
			}
		...
		async content(event, trigger, player) {
			const target = event.targets[0];
			player.logSkill(event.name, target);
			if (!target.hasCards("hej", card => lib.filter.cardDestuctible(card, target, event.name))) {
				return;
			}
			const result = await player
	...
	mjsyitongliuhe: {
		...
		groupFiter(player) {
			return lib.filter.groupFilter(null, null, player);
		},
		async cost(event, trigger, player) {
			const list = [];
		...
		async content(event, trigger, player) {
			if (event.cost_data == "相同势力") {
				...
				const next = trigger.player.changeGroup("feichu", false);
	...
	mjshengsaosanjin: {
		...
		filter(event, player) {
			...
			return event.card.name == "sha" && event.targets?.some(target => target.hasCards("h"));
		},
		logTarget: "targets",
		async content(event, trigger, player) {
			for (const target of event.targets) {
	...
	mjslikangwancheng: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		audioname: ["mjs_baqing_yuepodanxin"],
		trigger: {
			global: "roundStart",
		},
		forced: true,
		locked: false,
		filter(event, player) {
			return game.hasPlayer(target => target.hasCards("h", "sha"));
		},
		async content(event, trigger, player) {
	...
	mjstugaodaihan: {
		...
		locked: false,
		filter(event, player) {
			return !player.hasCards("he", "mjschuanguoyuxi");
		},
		filterCard(card) {
	...
	mjsshiqinjinshen: {
		...
		async content(event, trigger, player) {
			...
			if (bool1 && bool2) {
				const list = ["摸1张牌", "回复1点体力"];
				const result = await player
		            .chooseTargetControl_mjs({
		                createDialog: [get.prompt2(event.name)],
		                choices: list,
		                control(targets) {
		        ...
		        if (result.bool && result?.targets?.length) {
		        	const target = result.targets[0];
		        	const index = result.control == "摸1张牌" ? 1 : 2;
		        	player.logSkill(event.name, target, null, null, [index]);
		        	await target[index == 1 ? "draw" : "recover"]();
		        }
		        return;
			}
			if (bool1) {
	...
	mjsrenhuifuxia: {
		...
		filter(event, player) {
			return event.player != player && player.hasCards("he");
		},
		async cost(event, trigger, player) {
			event.result = await player
	...
	mjsyuanbishanshe: {
		...
		filter(event, player) {
			if (event.name == "useCardToTarget") {
				return event.card.name == "sha" && !event.target.hasCards("h", "shan");
			}
	...
	mjsshizhikuanghan: {
		...
		async cost(event, trigger, player) {
			...
			if (!player.hasCards("he") || game.countPlayer(target => target != player) < 2) {
				return;
			}
			event.result = await player
	...
	mjsjieliangfenzi: {
		...
		usable: 1,
		filter(event, player) {
			return event.card?.name == "sha" && event.player.hasCards("he");
		},
		async cost(event, trigger, player) {
			event.result = await player
	...
	mjstongshuishangshi: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					return get.effect(target, "mjs_debuff_liuxue", get.player(), target);
				})
				.forResult();
		},
		async content(event, trigger, player) {
	...
	mjsganluzhaoqin: {
		...
		usable: 1,
		filter(event, player) {
			return event.card.name == "sha" && event.target.isIn() && event.target.hasCards("h");
		},
		check(event, player) {
	...
	mjsshimengjushou: {
		...
		filter(event, player) {
			if (event.name == "chooseToUse") {
				return player.getHp(true) == 1 && player.hasCards("hes");
			}
		...
		subSkill: {
			discard: {
				...
				filter(event, player, triggername, target) {
					...
					return target.hasCards("h", { name: "mjschi" });
				},
				logTarget(event, player, triggername, target) {
					return target;
				},
	...
	mjsbashidanjing: {
		...
		popup: false,
		filter(event, player) {
			return event.source?.isIn() && player.hasCards("he");
		},
		async cost(event, trigger, player) {
			event.result = await player
	...
	mjsqiaobianshanzhan: {
		...
		onremove(player, skill) {
			delete player.storage[skill];
			player.removeSkill("mjsqiaobianshanzhan_used");
		},
		filter(event, player) {
			return get
				.inpileVCardList(info => {
					const type = info[0], name = info[2];
					if (player.hasStorage("mjsqiaobianshanzhan", name)) {
						return false;
					}
					if (player.hasStorage("mjsqiaobianshanzhan_used", type)) {
						return false;
					}
					return ["basic", "trick"].includes(type);
				})
				.some(info =>
					player.hasCards("hs", cardx => {
						const type = info[0], name = info[2];
		...
		chooseButton: {
			...
			backup(links, player) {
				return {
					...
					log: false,
					async precontent(event, trigger, player) {
						player.logSkill("mjsqiaobianshanzhan");
	...
	mjsyingshibuzhen: {
		...
		usable: 1,
		filter(event, player) {
			return player.hasCards("he") && player.countExpansions("mjsyingshibuzhen");
		},
		async precontent(event, trigger, player) {
		...
		intro: {
	        ...
	        mark(dialog, content, player) {
	            var content = player.getExpansions("mjsyingshibuzhen");
	            if (content && content.length) {
	...
	mjssichengweiwu: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		audioname: ["mjs_caopi_weiwenqianding"],
		trigger: {
			player: "damageEnd",
		},
		silent: true,
	...
	mjszhidaohuanglong: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		audioname: ["mjs_weiqing_wangyezhubei"],
		trigger: {
			global: "phaseBefore",
			player: ["enterGame", "changeSkillsAfter"],
		},
	...
	mjsheyijiawei: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		audioname: ["mjs_huoqubing_hanhailieyun"],
		trigger: {
			player: "phaseEnd",
		},
		silent: true,
	...
	mjstianrenganying: {
		...
		async content(event, trigger, player) {
			...
			const result = await player
				.chooseToDiscard(get.prompt(event.name), "你可以弃置1张属性不同的手牌（相生/相克，则被生者/被克者下回合摸牌数+1/-1）", card => {
			...
			if (result?.bool && result.cards?.length) {
				...
				for (const relation of relations) {
					if (info.hasEffect(list, relation)) {
						...
						game.log(current, "下回合", "#y摸牌数", relation == "generate" ? "#y+1" : "#r-1");
					}
				}
			}
		},
		...
		subSkill: {
			...
			generates: {
				name: "相生",
				name: `<span data-nature=\"woodmm\">相生</span>`,
			},
			generated: {
				name: "被生",
				name: `<span data-nature=\"woodmm\">被生</span>`,
			},
			restricts: {
				name: "相克",
				name: `<span data-nature=\"firemm\">相克</span>`,
			},
			restrictd: {
				name: "被克",
				name: `<span data-nature=\"firemm\">被克</span>`,
			},
		},
	},
	/**春秋繁露
				...
				event.result = await target
					...
					.set(
						...
						(() => {
							...
							if (target.hasSkillTag("noChangeGroup", false, player)) {
								return true;
							}
							if (!player.hasFriend()) {
								return true;
		...
		ai: {
			...
			result: {
				target(player, target) {
					...
					if (att > 0) {
						if (target.hasSkillTag("noChangeGroup")) {
							return 0;
						}
						return att * target.getHandcardLimit() * target.getHp();
					}
					return att;
				},
			},
	...
	mjsquanyusigui: {
		...
		audio: "ext:名将杀/audio/skill:2",
		init: (player, skill) => {
	      	game.addGlobalSkill(skill + "_use");
	    },
	    onremove: (player, skill) => {
		...
		subSkill: {
			...
			use: {
				...
				filter(event, player) {
					...
					return player.hasCards("h", card => card.hasGaintag("mjsquanyusigui_tag"));
				},
				async content(event, trigger, player) {
	...
	mjsqianjinmaifu: {
		...
		check(event, player) {
			...
			if (att <= 0 && cards.length >= 8 && player.countCards("h", card => get.value(card) - 5) < 1) {
				return true;
			}
			if (cards.length <= 4) {
				return false;
	...
	mjsfabuagui: {
		...
		targetprompt2: target => {
	        if (get.event().skill == "mjsfabuagui") {
	        	return lib.skill.mjsfabuagui.getNum(target).toString();
	        }
    	},
		onChooseToUse(event) {
	        event.targetprompt2.add(lib.skill.mjsfabuagui.targetprompt2);
	    },
		getNum(player) {
		...
		prompt() {
			...
			return `你可以令所有角色${player.hasSkill("mjsfabuagui_rewrite") ? "摸牌" : "随机弃置牌"}`;
		},
		async content(event, trigger, player) {
			...
			use: {
				...
				init(player, skill) {
					...
					const cards = player.getStorage(skill).reduce((cards, target) => {
						const fake = target.isAlive() && target.hasCards("h") ? game.createFakeCards(target.getCards("h")) : [];
				...
				async content(event, trigger, player) {
					...
					if (["useCard", "respond"].includes(trigger.name)) {
						...
						const real = player.getStorage(event.name).reduce((cards, target) => {
							const hs = target.isAlive() && target.hasCards("h") ? target.getCards("h") : [];
	...
	mjsyishuyuguo: {
		...
		onremove(player) {
			if (!player.hasSkillTag("viewTop")) {
				player.removeSkill("mjsyishuyuguo_viewTop");
			}
		},
		trigger: {
			player: "useCard",
		...
		subSkill: {
			viewTop: {
				clickableFilter(player) {
			        return player.hasSkill("mjsyishuyuguo_viewTop", null, null, false);
			    },
			    clickable(player) {
			        if (player.isUnderControl(true)) {
			            const cards = lib.skill.mjsyishuyuguo_viewTop.getCards(player);
			            function createDialogWithControl(result) {
			                const dialog = ui.create.dialog("牌堆顶", "peaceDialog");
			                ...
			                if (_status.mjsyishuyuguo_viewTop) {
			                    _status.mjsyishuyuguo_viewTop.close();
			                }
			                _status.mjsyishuyuguo_viewTop = dialog;
			                dialog.open();
			            }
			    ...
			    getCards(player) {
			        let cards = [];
			        if (game.online) {
			            return game.requestSkillData("mjsyishuyuguo_viewTop", "getTopCards", 10000);
			        } else {
	...
	mjsyishuzhiguo: {
		...
		init(player) {
			lib.skill.mjsyishuyuguo.init(player, "mjsyishuyuguo");
		},
		onremove(player) {
			lib.skill.mjsyishuyuguo.onremove(player, "mjsyishuyuguo");
		},
		trigger: {
			player: "useCard",
		},
	...
	mjsjiangxianghe: {
		...
		popup: false,
		filter(event, player) {
			return game.hasPlayer(target => target != player && target.hasCards("he"));
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(get.prompt2(event.skill), (card, player, target) => {
	...
	mjszhanxiaolixin: {
		...
		filterTarget(card, player, target) {
			return target != player && target.hasCards("hej", card => lib.filter.cardDestuctible(card, target, "mjszhanxiaolixin"));
		},
		async content(event, trigger, player) {
			const [target] = event.targets;
			const result = await player
			...
			for (const target of targets) {
				...
				await target.draw();
				if (target.hasCards("he") && target != player) {
	...
	mjslilingzhihun: {
		...
		onremove(player, skill) {
			game.countPlayer2((current) => {
				if (current.hasStorage(skill + "_debuff", player)) {
					current.unmarkAuto(skill + "_debuff", player);
				}
			}, true);
		},
		trigger: {
		...
		subSkill: {
			debuff: {
				...
				mod: {
					targetEnabled(card, player, target) {
						...
						if (player.getStorage("mjslilingzhihun_debuff").some(current => {
							...
							if (!current.hasSkill("mjslilingzhihun")) {
								return false;
							}
	            ...
	            filter: (event, player) => {
	              	return player.hasStorage("mjslilingzhihun_debuff", event.player);
	            },
	            silent: true,
	            forceDie: true,
	            charlotte: true,
	...
	mjshezong: {
		...
		filterTarget(card, player, target) {
			...
			return target.hasCards("he", card => lib.filter.canBeStrengthened(card, target, "mjshezong"));
		},
		selectTarget: [0, 1],
		filterCard(card, player) {
			if (ui.selected.targets.length) {
	...
	mjsyinzhangshierqu: {
		...
		enable: "phaseUse",
		filter(event, player) {
			if (!player.hasCards("he", { suit: "spade" })) return false;
		...
		async content(event, trigger, player) {
			...
			if (!player.hasCards("h", { suit: suit })) {
				const card = get.cardPile(
					card => {
	...
	mjsshikesanqian: {
		...
		filter(event, player) {
			...
			return game.hasPlayer(target => target != player && target.hasCards("h"));
		},
		async content(event, trigger, player) {
	...
	mjsweisishi: {
		...
		subSkill: {
			...
			use: {
				...
				logTarget: "player",
				check(event, player) {
					return get.effect(event.player, "mjs_debuff_liuxue", player, event.player) > 0;
				},
				prompt2(event, player) {
	...
	mjsjieqiujiuren: {
		...
		filter(event, player) {
			...
			return (_status.connectMode && player.hasCards("he")) || player.hasCards("he", { type: "equip" });
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseCard(get.prompt2(event.skill, trigger.player), "he")
			...
			effect: {
				...
				filter(event, player) {
					return get
						...
						.getTargets(player)
						.some(target => target.hasCards("he"));
				},
				logTarget(event, player) {
					return get
						.info("mjsfenquanshiyi")
	...
	mjsjiaotusanku: {
		...
		subSkill: {
			effect: {
				...
				async cost(event, trigger, player) {
					const targets = game.filterPlayer(target => {
						if (!target.hasCards("h", card => card.hasGaintag(`mjsjiaotusanku_${player.playerid}`))) {
							return false;
						}
					...
					event.result = await player
						.chooseTarget({
							prompt: get.prompt(event.skill),
							prompt2: `你可以选择一名角色代替你成为${get.translation(trigger.card)}的目标`,
							filterTarget(card, player, target) {
								return get.event().targets.includes(target);
							},
							targets,
				...
				async content(event, trigger, player) {
					...
					const cards = target
						.getCards("h", card => {
							if (!lib.filter.canBeStrengthened(card, target, "mjsjiaotusanku_effect")) {
								return false;
							}
	...
	mjslianheng: {
		...
		async precontent(event, trigger, player) {
			const skillName = event.name.slice("pre_".length);
			const list = ["改变他人势力", "改变自身势力"].map((info, index) => [index + 1, info]);
			const result = await player
				.chooseButtonTarget({
					createDialog: [`###${get.prompt(skillName)}###将你的势力变为一名其他角色的势力，或者令一名其他角色的势力变为你的势力`, [list, "tdnodes"]],
					canHidden: false,
					filterTarget(card, player, target) {
						if (!get.info("mjsyitongliuhe").groupFiter(target)) {
							return false;
		...
		async content(event, trigger, player) {
			const target = event.targets[0];
			if (event.getParent(2).cost_data == 1) {
				const next = target.changeGroup(player.group);
				next.source = player;
				await next;
			} else {
	...
	mjszhongkoushuojin: {
		...
		filter(event, player) {
			...
			return (
				...
				game.hasPlayer(target => {
					return target != player && target.hasCards("hej", cardx => lib.filter.cardDestuctible(cardx, target, "mjszhongkoushuojin"));
				})
			);
		},
		async cost(event, trigger, player) {
		...
		async content(event, trigger, player) {
			const target = event.targets[0];
			player.logSkill(event.name, target);
			if (!target.hasCards("hej", card => lib.filter.cardDestuctible(card, target, "mjszhongkoushuojin"))) {
				return;
			}
			const result = await player
				.choosePlayerCard(target, "hej", true)
		...
		group: "mjszhongkoushuojin_counter",
		subSkill: {
			counter: {
				trigger: {
					global: ["useCard1", "respond"],
				...
				async content(event, trigger, player) {
					...
					player.storage[skill] ??= new Map();
					let num = 0;
					if (player.storage[skill].has(name)) {
						num = player.storage[skill].get(name);
					}
					num++;
					if (num % 3 == 0) {
			...
			const targets = game.filterPlayer(target => {
				return (
					...
					target.getSkills(null, false, false).some(skill => {
	                        ...
	                        return target.getRoundHistory("useSkill", evt => evt.skill == skill).length;
	                    })
				);
			});
			if (targets.length && !player.hasSkill("mjszhezhu_used", null, null, false)) {
				list.push("刻写技能");
			}
			if (player.getExpansions(skillName).length) {
				list.push("获得技能");
			}
			const { control } = await player
				.chooseControl(list, "cancel2")
				...
				if (control == "刻写技能") {
					...
					const result = await player
						.chooseButtonCard_mjs({
							createDialog: [
								get.translation(skillName),
								[
									dialog => {
										dialog.css({
			                                top: get.is.phoneLayout() ? "5%" : "45%",
			                            });
			                            const { targets, skillMap } = get.event();
			                            //算出来需要多少列，最多八列
			                            const num = 8;
			                            const column = Math.min(targets.length, num);
			                            if (column > 6) {
			                                dialog.css({
			                                    width: "100%",
			                                    left: 0,
			                            ...
			                            for (const target of targets) {
			                                const div = ui.create.div(".buttons", contentx);
			                                const button = ui.create.button(target.name, "character", div);
			                                const skills = skillMap.get(target);
			                                //让角色和技能按钮水平居中垂直排列
			                                div.css({
			                                    display: "flex",
			                                    flexDirection: "column",
			                                ...
			                                if (skills.length) {
			                                    //创建技能按钮
			                                    const buttons = ui.create.buttons(
			                                        skills.map(skill => [skill, get.translation(skill)]),
			                                        "tdnodes",
			                                        div
			                                    );
			                                    //丢进可选按钮中
	...
	mjswuguibaixiang: {
		...
		init(player, skill) {
			player.storage[skill] ??= new Map();
			game.addGlobalSkill("mjswuguibaixiang_global");
		},
		onremove(player, skill) {
			delete player.storage[skill];
			if (!game.hasPlayer(current => current.hasSkill("mjswuguibaixiang", null, null, false), true)) {
				game.removeGlobalSkill("mjswuguibaixiang_global");
			}
		},
		trigger: {
	...
	mjsshanyoufusu: {
		...
		async content(event, trigger, player) {
			...
			for (const target of event.targets) {
				...
				target
					.when(["useCard", "respond"])
					.filter(evt => get.suit(evt.card) == "club")
					.then(() => {
						player.recover();
					});
		...
		ai: {
			...
			result: {
				player(player) {
					...
					return player.getFriends().length - 1;
				},
			},
		},
		subSkill: {
	...
	mjsxianwenyumin: {
		...
		async content(event, trigger, player) {
			...
			trigger.source.addMark("mjsxianwenyumin_effect", 1, false);
		},
		subSkill: {
			effect: {
				trigger: {
	...
	mjshujiashibapai: {
		nobracket: true,
		audio: "ext:名将杀/audio/skill:2",
		audioname: ["mjs_caiwenji_zhenxueyanchou"],
		trigger: {
		...
		check(event, player) {
			...
			if (source && source.isIn()) {
				return get.effect(player, { name: "draw" }, player, player) + get.effect(source, { name: "guohe_copy" }, player, player);
			}
		...
		ai: {
			...
			effect: {
				target(card, player, target) {
					if (target.storage?.counttrigger?.mjshujiashibapai) {
						return;
					}
	...
	mjsbeifenlichou: {
		...
		ai: {
			...
			effect: {
				target(card, player, target) {
					if (!player.getSkills(null, false, false).length) {
						return;
					}
					if (get.tag(card, "damage")) {
						if (player.hasSkillTag("jueqing", false, target)) {
	...
	mjsguaguliaodu: {
        ...
        forced: false,
		filter(event, player) {
			return event.player?.isIn() && event.player.hasCards("h");
		},
		logTarget: "player",
		check(event, player) {
			...
			effect: {
				...
				async content(event, trigger, player) {
					...
					if (lib.cardPack.mjs.includes(name)) {
						...
						trigger.setContent(name == "sha" ? lib.skill.mjszhibeixicao_effect.contentx : "emptyEvent");
					}
				},
				contentx() {
					"step 0";
	...
	mjsfeishengtaixu: {
		...
		async content(event, trigger, player) {
			...
			lib.skill.mjsallmax.change(player, num2 - player.getHandcardLimit());
	...
	mjswushuangfeijiang: {
		...
		async content(event, trigger, player) {
			...
			for (const card of cards) {
				game.broadcastAll(function (card) {
					card.init([lib.suit.randomGet(), get.rand(1, 8), "sha"]);
				}, card);
		...
		subSkill: {
			init: {
				...
				async content(event, trigger, player) {
					player.logSkill("mjswushuangfeijiang", null, null, null, [get.rand(3, 4)]);
					trigger.directHit.addArray(game.filterPlayer());
				},
			},
		},
	...
	mjsyuanmensheji: {
		...
		filter(event, player) {
			...
			const evt = event.getParent();
			if (evt.directHit && evt.directHit?.includes(event.target)) {
				return false;
			}
	...
	//陈平
	/**六出奇计
	 * 出牌阶段开始时和应战，你可以将1张未选择过的专属战法牌添加到手牌，回合开始时，若每张专属战法牌都选择，重置此技能。
	...
	mjsliuchuqiji: {
		...
		async cost(event, trigger, player) {
			const list = lib.skill[event.skill].getList
				.filter(name => {
					return !player.hasStorage(event.skill, name);
				})
				.randomGets(3);
			const cards = [];
			for (const name of list) {
		...
		subSkill: {
			restore: {
				...
				locked: false,
				filter(event, player) {
					return player.getStorage("mjsliuchuqiji_used").containsAll(...lib.skill.mjsliuchuqiji.getList);
				},
				async content(event, trigger, player) {
					const skill = get.sourceSkillFor(event.name);
					player.logSkill(skill, null, null, null, [1, 2]);
					await player.removeSkills(skill);
					const createSkills = mjs.addCreateSkills(skills);
					await player.addSkills(createSkills);
				},
	...
	mjsguaxiweimen: {
		...
		filter(event, player) {
			...
			if (get.is.virtualCard(event.card) || get.is.convertedCard(event.card)) {
				return false;
			}
	...
	mjsshimianmaifu: {
		...
		intro: {
			markcount: "expansion",
			mark(dialog, content, player2) {
	            var content = player2.getExpansions("mjsshimianmaifu");
	            if (content && content.length) {
	              if (player2 == game.me || player2.isUnderControl()) {
	                dialog.addAuto(content);
	              } else {
	                return "共有" + get.cnNumber(content.length) + "张埋伏牌";
	              }
	            }
	        },
	        content(content, player2) {
	...
	mjszhansheqiyi: {
		...
		enable: "phaseUse",
		filter(event, player) {
			return player.hasCards("h", card => card.name != "sha");
		},
		limited: true,
		filterCard(card) {
			const player = get.player();
	...
	mjsdafengge: {
		...
		filter(event, player) {
			if (event.name == "gameDraw") {
				return player.hasCards("h", card => get.number(card) == 5);
			}
	...
	mjslinchaochengzhi: {
		...
		subSkill: {
			...
			destroy: {
				...
				onremove: true,
				filter(event, player) {
					if (!event.player.hasCards("he", card => lib.filter.cardDestuctible(card, event.player, "mjslinchaochengzhi_destroy"))) {
						return false;
					}
	...
	mjsbolangzhuiqin: {
		...
		mod: {
			aiOrder(player, card, num) {
	            if (!player.hasSkill("mjsyunchouweiwo")) {
					return;
				}
		...
		enable: "phaseUse",
		filter(event, player) {
			return player.hasCards("h", card => lib.skill.mjsbolangzhuiqin.filterCard(card));
		},
		filterCard(card) {
			return get.subtype(card) == "equip1";
		},
	...
	mjsyunchouweiwo: {
		...
		locked: false,
		filter(event, player) {
			if (!player.hasCards("h", { name: "sha" })) return false;
		...
		subSkill: {
			...
			use: {
				...
				filter(event, player) {
					...
					if (event.responded || event.mjsyunchouweiwo) {
			            return false;
			        }
			        if (player.hasSkill("mjsyunchouweiwo_blocker", null, null, false)) {
			        	return false;
			        }
				...
				async content(event, trigger, player) {
					...
					for (const target of targets) {
						...
						const chooseToRespondEvent = target.chooseToRespond("是否替" + get.translation(player) + "打出一张杀？", function(card) {
		                    if (get.name(card) != "sha") {
		                      return false;
		                    }
		                    return lib.filter.filterCard.apply(this, arguments);
	...
	mjschuwang: {
		...
		filter(event, player) {
			...
			return game.hasPlayer(current => {
				return event.targets?.some(target => {
					return current.inRange(target) && ((_status.connectMode && current.hasCards("hs")) || current.hasSha());
				});
			});
		},
		async content(event, trigger, player) {
			...
			const func = async target => {
				await target
					.chooseToUse("是否对" + get.translation(trigger.targets) + "打出一张杀？", function (card, player, event) {
						if (get.name(card) != "sha") {
							return false;
						}
						return lib.filter.filterCard.apply(this, arguments);
			...
			await game.doAsyncInOrder(game.filterPlayer(), func);
		},
	},
	/**先发制人
	...
	mjsxianfazhiren: {
		...
		filter(event, player) {
			...
			return event.player.isIn() && lib.filter.targetEnabled({ name: "sha" }, player, event.player) && (player.hasSha() || (_status.connectMode && player.hasCards("h")));
		},
		check(event, player) {
	...
	mjslangguyingshi: {
		...
		locked: false,
		async content(event, trigger, player) {
			const targets = game.filterPlayer(target => target != player && !target.hasCards("h", "sha"));
			if (targets.length) {
				lib.skill.mjsallmax.change(player, targets.length);
				await player.gainMaxHp(targets.length);
			}
			const targets2 = targets.filter(target => target != player && !target.hasCards("h", "shan"));
			if (targets2.length) {
	...
	mjsren: {
		...
		prompt() {
			const player = get.player();
			const counter = player.countMark("mjsren_counter");
			const num = Math.floor(counter / 2);
			return `所有角色已累计受到过${counter}点伤害，你可以摸${num}张牌。`;
		},
		async content(event, trigger, player) {
		...
		ai: {
			...
			result: {
				player(player) {
					...
					return get.effect(player, { name: "draw" }, player) * num > 0;
				},
			},
		},
	...
	/**百步穿杨
	 * 每个回合限1次，当你选择杀的目标时，若你与目标距离>1，则此杀对其强命，否则此杀伤害+1。
			...
			counter: {
				...
				async content(event, trigger, player) {
					...
					while (player.storage[event.name].get(target) >= 3) {
						...
						const next = game.createEvent("mjssangumaoluGive");
						next.player = player;
						next.target = target;
						next.setContent("emptyEvent");
						await next;
	...
	mjsdaojianqingyuan: {
		...
		usable: 1,
		filter(event, player) {
			return player.hasCards("he") && game.hasPlayer(target => lib.skill.mjsdaojianqingyuan.filterTarget(null, player, target));
		},
		filterCard: true,
		position: "he",
		check(card) {
	...
	mjsshijiuruming: {
		...
		viewAsFilter(player) {
			if (!player.hasCards("hs", { name: "sha" })) {
				return false;
			}
			return true;
		},
	...
	mjsbairenyexi: {
		...
		filter(event, player) {
			if (event.name == "gameDraw") {
				return player.hasCards("h", { suit: "spade" });
			}
			return event.getg && event.getg(player)?.some(card => get.suit(card) == "spade");
		},
		async cost(event, trigger, player) {
	...
	mjshuopolianying: {
		...
		ai: {
			order(item, player) {
				player = player || get.event().player;
				if (player.hasCards("hs", "mjsliehuofencheng")) {
	...
	mjstaozhiyaoyao: {
		...
		filter(event, player) {
			if (event.name == "gameDraw") {
				return player.hasCards("h", { suit: "club" });
			}
