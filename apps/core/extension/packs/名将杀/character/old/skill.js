...
const skills = {
        ...
        async content(event, trigger, player) {
            ...
            await player
                ...
                .set("oncard", (card, player) => {
                    player.addTempSkill("mjsoldlucairuhai_effect", { player: "phaseBegin" });
                });
        },
        subSkill: {
            backup: {
            ...
            effect: {
                ...
                async content(event, trigger, player) {
                    player.useSkill("mjsoldlucairuhai");
                },
            },
        },
    },
    ...
    mjsoldhuatingheli: {
        ...
        async cost(event, trigger, player) {
            const list = get.inpileVCardList(info => {
                ...
                if (player.hasStorage("mjsoldhuatingheli_effect", info[2])) {
                    return false;
                }
        ...
        async content(event, trigger, player) {
            player.addSkill("mjsoldhuatingheli_effect");
            player.markAuto("mjsoldhuatingheli_effect", event.cost_data);
        ...
        subSkill: {
            effect: {
                ...
                logTarget: "player",
                filter(event, player) {
                    return player.hasStorage("mjsoldhuatingheli_effect", event.card.name);
                },
                check(event, player) {
        ...
        async content(event, trigger, player) {
            ...
            player
                ...
                .then(async (event2, trigger2, player2) => {
                    player2.addTempSkill(`mjsnewqingqiuhuandai_${phasename}`);
                    player2.markAuto(`mjsnewqingqiuhuandai_${phasename}`, trigger.name);
                });
        },
        subSkill: {
            phaseUse: {
                ...
                filter(event, player) {
                    ...
                    if (event.phaseList[event.num].endsWith("mjsnewqingqiuhuandai")) {
                        return false;
                    }
                    const phasename = event.phaseList[event.num].split("|")[0];
                    return player.hasStorage("mjsnewqingqiuhuandai_phaseUse", phasename);
                },
                async content(event, trigger, player) {
                    const phasename = trigger.phaseList[trigger.num].split("|")[0];
                    game.log(player, "的", "#g" + phasename, "=>", "#g" + "phaseUse");
                    trigger.phaseList[trigger.num] = `phaseUse|mjsnewqingqiuhuandai`;
                    await game.delay();
                },
            },
            phaseDraw: {
                ...
                filter(event, player) {
                    ...
                    const phasename = event.phaseList[event.num].split("|")[0];
                    return player.hasStorage("mjsnewqingqiuhuandai_phaseDraw", phasename);
                },
                async content(event, trigger, player) {
                    const phasename = trigger.phaseList[trigger.num].split("|")[0];
                    game.log(player, "的", "#g" + phasename, "=>", "#g" + "phaseDraw");
                    trigger.phaseList[trigger.num] = `phaseDraw|mjsnewqingqiuhuandai`;
                    await game.delay();
                },
            },
        },
    ...
    mjsnewdeguansihai: {
        ...
        filter(event, player) {
            ...
            return !player.hasSkill("mjsnewdeguansihai_used");
        },
        logTarget: "targets",
        chooseToShow(target, player) {
            const next = target.chooseCard(`${mjs.prompt("mjsnewdeguansihai")}，请选择展示2张手牌`, 2, true);
        ...
        async content(event, trigger, player) {
            player.addTempSkill("mjsnewdeguansihai_used", "phaseUseAfter");
        ...
        async content(event, trigger, player) {
            ...
            if (gains.length) {
                ...
                const result2 = await player
                    .chooseTarget(`${mjs.prompt(event.name)}，将其中的♠牌交给一名其他角色`, true)
                    .set("ai", target => {
                        const player = get.player();
                        const att = get.attitude(player, target);
    ...
    mjsoldfengqiuhuang: {
        ...
        async content(event, trigger, player) {
            ...
            if (gains.length) {
                cards.removeArray(gains);
                const result2 = await player
                    .chooseTarget(`${mjs.prompt(event.name)}，将其中的♥牌交给一名角色`, true)
                    .set("ai", target => {
                        const player = get.player();
                        const att = get.attitude(player, target);
...
const skills3 = {
    ...
    mjsxuezhaoluanchen: {
        ...
        forceDie: true,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simayu")) {
                return false;
            }
            return true;
        },
        async content(event, trigger, player) {
            if (player.name2 && get.character(player.name2, 3).includes(event.name)) {
                await player.reinitCharacter(player.name2, "mjs_simalun");
            } else {
                await player.reinitCharacter(player.name1, "mjs_simalun");
            }
        },
    },
    /**愍（min）怀天下
     * 当有角色打出牌后，失去1点体力，令所有角色摸1张牌。 
     * */
    mjsminhuantianxia: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        async content(event, trigger, player) {
            ...
            await game.asyncDraw(game.filterPlayer());
        },
    },
    //司马衷
    /**何不食肉糜
     * 你无法打出牌。当其他角色受到伤害后，你随机销毁所有角色1张牌，然后回复1点体力。受伤，若贾南风在场，令贾南风进行1个出牌阶段，否则随机令一名其他司马氏角色进行1个出牌阶段。 
     * */
    mjshebushiroumi: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simazhong")) {
                return false;
            }
            return true;
        },
        async content(event, trigger, player) {
            if (trigger.player != player) {
                ...
                let source = game.findPlayer(target => get.is.playerNames(target, "mjs_jiananfeng"));
                if (!source?.isIn()) {
                    const targets = game.filterPlayer(target => {
                        if (target == player) {
                            return false;
                        }
                        return get.nameList(target).some(name => get.characterSurname(name).some(j => j[0] == "司马"));
                    });
                    if (targets.length) {
                        source = targets.randomGet();
                    }
                }
                const next = source.phaseUse();
                trigger.next.remove(next);
                trigger.getParent().next.push(next);
            }
        },
    ...
    //贾南风
    /**矫诏屠宗
     * 回合开始时，对司马玮打出牌堆顶的杀，直到其阵亡，并且令司马遹中毒+99。
     * */
    mjsjiaozhaotuzong: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_jiananfeng")) {
                return false;
            }
            return game.hasPlayer(target => get.is.playerNames(target, "mjs_simawei"));
        },
        async content(event, trigger, player) {
            const target = game.findPlayer(current => get.is.playerNames(current, "mjs_simawei"));
            if (!target?.isIn()) {
                return;
            }
            while (target.isIn()) {
                const card = get.cardPile2("sha");
                if (card && player.canUse(card, target, false, false)) {
            ...
            target.addSkill("mjs_debuff_zhongdu");
            target.addMark("mjs_debuff_zhongdu", 99);
        },
    },
    /**祸心藏毒
     * 当你打出牌后，令随机一名其他角色中毒+1。当你对其他角色造成伤害后，若目标有中毒状态，你可以查看并打出其1张手牌。  
     * */
    mjshuoxincangdu: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        filter(event, player) {
            ...
            return ["useCard", "respond"].includes(event.name) || event.player.hasMark("mjs_debuff_zhongdu");
        },
        async content(event, trigger, player) {
            if (["useCard", "respond"].includes(trigger.name)) {
                const target = game.filterPlayer(current => current != player).randomGet();
                if (!target?.isIn()) {
                    return;
                }
                target.addSkill("mjs_debuff_zhongdu");
                target.addMark("mjs_debuff_zhongdu", 1, false);
            } else {
                const target = trigger.player;
                const cards = target.getCards("h");
                if (!cards.length) {
                ...
                const result = await player
                    .chooseButton(["###祸心藏毒###是否使用其中一张牌", cards])
                    .set("filterButton", button => {
                        return get.event().cards.includes(button.link);
                    })
                    .set("cards", cards2)
    ...
    /**毒后乱晋
     * 回合结束时/阵亡，令场上中毒层数最多的一名其他角色失去与中毒层数等量的体力值。 
     * */
    mjsduhouluanjin: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        filter(event, player) {
            ...
            if (event.name == "phase" && player.isDead()) {
                return false;
            }
            return game.hasPlayer(target => target.hasSkill("mjs_debuff_zhongdu"));
        },
        async cost(event, trigger, player) {
            const targets = game.filterPlayer(target => {
                ...
                return !game.hasPlayer(current => {
                    ...
                    return current.countMark("mjs_debuff_zhongdu") > target.countMark("mjs_debuff_zhongdu");
                });
            });
            if (!targets.length) {
                return;
            ...
            event.result = await player
                .chooseTarget(`${mjs.prompt(event.skill)}发动，令场上中毒层数最多的一名其他角色失去与中毒层数等量的体力值。`, (card, player, target) => {
                    return get.event().targets.includes(target);
                })
                .set("ai", target => {
                    const player = get.player();
                    const num = target.countMark("mjs_debuff_zhongdu");
                    return get.effect(target, { name: "losehp" }, player, player) * num;
                })
                .set("targets", targets)
                .forResult();
        },
    ...
    mjsluanqi: {
        ...
        forceDie: true,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simaliang")) {
                return false;
            }
            return true;
        },
        async content(event, trigger, player) {
            const func = async target => {
                ...
                if (cards.length) {
                    await target.mjsWeakenCards(cards.randomGets(7), player);
                }
            };
            const targets = game.filterPlayer(target => target != player);
            await game.doAsyncInOrder(targets, func);
    ...
    //司马玮
    /**连诛
     * 登场，将弃牌堆洗回牌堆，然后对司马亮打出牌堆顶的杀，直到其阵亡。然后你每因此打出过1张杀，就随机对一名敌方角色打出牌堆顶的1张杀。
     * */
    mjslianzhu: {
        audio: "ext:名将杀/audio/skill:2",
        trigger: {
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simaying")) {
                return false;
            }
            if (event.name == "changeSkills") {
                return event.addSkill.includes("mjslianzhu");
            }
            return (event.name != "phase" || game.phaseNumber == 0);
        },
        async content(event, trigger, player) {
            ...
            const targets = game.filterPlayer(target => get.is.playerNames(target, "mjs_simaliang"));
            for (const target of targets) {
                while (target?.isIn()) {
                    const card = get.cardPile2("sha");
                    if (card && player.canUse(card, target, false, false)) {
                        const next = player.useCard(card, target, false);
                        next.set("_mjslianzhu", true);
                        await next;
                    } else {
                        break;
                    }
            ...
            let num = player.getHistory("useCard", evt => evt.getParent(event.name) == event && evt._mjslianzhu).length;
            while (num > 0) {
                const card = get.cardPile2("sha");
                if (card && player.getEnemies(target => player.canUse(card, target, false, false)).length) {
                    const target = player.getEnemies(target => player.canUse(card, enemie, false, false)).randomGet();
                    const next = player.useCard(card, target, false);
                    await next;
                } else {
                    break;
    ...
    //司马伦
    /**敕令相结
     * 出牌阶段开始时，你随机获得所有其他角色2张牌，并且司马冏登场。  
     * */
    mjschilingxiangjie: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        async content(event, trigger, player) {
            const func = async target => {
                const cards = target.getGainableCards(player, "he");
                if (cards.length) {
                    await player.gain(cards.randomGets(2), target, "giveAuto", "bySelf");
                }
            };
            const targets = game.filterPlayer(target => target != player);
            await game.doAsyncInOrder(targets, func);
    ...
    /**废后挟君
     * 登场，令贾南风中毒+99，令何不食肉糜的效果改为“你无法打出牌。当你受到伤害后，令司马伦进行1个出牌阶段”。 
     * 限定，当你即将受到伤害时，令场上中毒层数最多的一名其他角色失去与中毒层数等量的体力值，然后令司马衷代替你承担此伤害。 
     * */
    mjsfeihouxiejun: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simalun")) {
                return false;
            }
            if (event.name == "changeSkills") {
                return event.addSkill.includes("mjsfeihouxiejun");
            }
            return (event.name != "phase" || game.phaseNumber == 0);
        },
        async content(event, trigger, player) {
            const targets = game.filterPlayer(target => get.is.playerNames(target, "mjs_jiananfeng"));
            for (const target of targets) {
                target.addSkill("mjs_debuff_zhongdu");
                target.addMark("mjs_debuff_zhongdu", 99, false);
            }
        },
        group: "mjsfeihouxiejun_limited",
        subSkill: {
            limited: {
                limited: true,
                trigger: {
                ...
                async content(event, trigger, player) {
                    ...
                    const num = target.countMark("mjs_debuff_zhongdu");
                    await target.loseHp(num);
                    const source = game.filterPlayer(target => get.is.playerNames(target, "mjs_simazhong"));
                    if (source?.isIn()) {
                        trigger.cancel();
                        await source.damage(trigger.source, trigger.nature, trigger.num).set("card", trigger.card).set("cards", trigger.cards);
                    }
                },
            },
        },
    ...
    //司马冏
    /**倡义讨贼
     * 登场，召唤司马颙和司马颖，你与司马颙和司马颖分别依次对司马伦打出手牌中的杀。然后若司马伦没有阵亡，你与司马颙和司马颖退场。若司马伦阵亡，恢复何不食肉糜为初始效果。
     * */
    mjschangyitaozei: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
    ...
    /**移檄天下
     * 限定，每个回合开始时，你获得司马颙和司马颖的所有牌，并且令其所有属性改为1，你获得其下降的属性值，然后令司马颖退场。  
     * */
    mjsyixitianxia: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
    ...
    /**专擅之威
     * 当你打出牌后，随机削弱每个其他角色1张牌，然后随机弃置每个其他角色1张牌。当其他角色弃牌后，若其手牌中存在削弱牌，你令其中毒+1。   
     * */
    mjszhuanshanzhiwei: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
    ...
    mjstongzongjizhi: {
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simayong")) {
                return false;
            }
            return player.maxHp == 1;
        },
    ...
    /**敛势益兵
     * 当你的牌被其他角色获得或削弱后，你摸1张牌。   
     * */
    mjslianshiyibing: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        trigger: {
            player: ["mjsWeakenCardsAfter"],
            global: ["gainAfter","loseAsyncAfter"],
        },
        silent: true,
        popup: true,
    ...
    mjszhenranchaogang: {
        ...
        filter(event, player) {
            ...
            if (event.name == "changeSkills") {
                return event.addSkill.includes("mjszhenranchaogang");
            }
            return (event.name != "phase" || game.phaseNumber == 0);
        },
        async content(event, trigger, player) {
    ...
    mjsdukuiqianjun: {
        ...
        async content(event, trigger, player) {
            ...
            const targets = game.filterPlayer(target => target.hasSkill("mjs_debuff_zhongdu"));
            await game.doAsyncInOrder(targets, func);
        },
    },
    //司马乂（yi） 
    /**三王共谋
     * 登场，立即进行1个出牌阶段，此阶段结束时，司马颙和司马颖登场。  
     * */
    mjssanwanggongmou: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simayixijin")) {
                return false;
            }
            if (event.name == "changeSkills") {
                return event.addSkill.includes("mjssanwanggongmou");
            }
            return (event.name != "phase" || game.phaseNumber == 0);
        },
        async content(event, trigger, player) {

            player.when("phaseUseEnd")
                .filter(evt => evt._extraPhaseReason == "mjssanwanggongmou")
                .then(async (event, trigger, player) => {
                    const nameList = ["mjs_simayong", "mjs_simaying"];
    ...
    /**阊阖厉诛
     * 当你打出牌时，对司马冏造成此牌点数的伤害，若司马冏已阵亡，则改为随机对一名其他角色造成此牌点数次1点伤害。   
     * */
    mjschanghelizhu: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
    ...
    //司马越 
    /**诸王终焉
     * 登场，依次对司马乂，司马颖，司马颙造成其体力值2倍的火焰伤害。然后令司马衷中毒+99。  
     * */
    mjszhuwangzhongyan: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        ...
        locked: false,
        filter(event, player) {
            if (!get.is.playerNames(player, "mjs_simayue")) {
                return false;
            }
            if (event.name == "changeSkills") {
                return event.addSkill.includes("mjszhuwangzhongyan");
            }
            return (event.name != "phase" || game.phaseNumber == 0);
        },
        async content(event, trigger, player) {
            const nameList = ["mjs_simayixijin", "mjs_simaying", "mjs_simayong"];
            for (const name of nameList) {
                const targets = game.filterPlayer(target => get.is.playerNames(target, name));
                for (const target of targets) {
                    if (!target.isIn()) {
                        continue;
                    }
                    const num = target.getHp(true) * 2;
            ...
            await game.doAsyncInOrder(game.filterPlayer(target => get.is.playerNames(target, "mjs_simazhong")), func);
        },
    },
    /**权噬四海
     * 当你打出牌后，令所有中毒的角色失去1点体力，然后令所有有削弱牌的角色随机弃置1张牌，并且中毒+1。回合结束时，此技能的所有数字+1。
     * */
    mjsquanshisihai: {
        nobracket: true,
        audio: "ext:名将杀/audio/skill:2",
        trigger: {
            player: ["useCardAfter", "respondAfter", "phaseEnd"],
        },
        silent: true,
        popup: true,
        locked: false,
        ...
        async content(event, trigger, player) {
            if (["useCard", "respond"].includes(trigger.name)) {
                const num = 1 + player.countMark(event.name);
                for (const target of game.filterPlayer(target => target.hasSkill("mjs_debuff_zhongdu"))) {
                    await target.loseHp(num);
                }
                for (const target of game.filterPlayer(target => target.hasCards("he", card => get.is.mjsWeakenedCard(card)))) {
                    const cards = target.getCards("h", card => {
                        return lib.filter.cardDiscardable(card, target, "mjsquanshisihai");
                    });
                    if (cards.length > 0) {
                        await target.discard(cards.randomGets(num)).set("discarder", target);
                    }
                    target.addSkill("mjs_debuff_zhongdu");
                    target.addMark("mjs_debuff_zhongdu", num, false);
                }
            } else {
                player.addMark(event.name, 1, false);
            }
