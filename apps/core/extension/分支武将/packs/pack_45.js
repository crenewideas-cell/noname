// Extracted from card/standard.js; legacy startup and engine/UI patches are not executed.
               ...
               cixiong_skill: {
                    ...
                    content: function () {
                         ...
                         trigger.target.chooseToDiscard('弃置一张手牌，或令' + get.translation(player) + '摸一张牌').set('ai', function (card) {
                              ...
                              if (get.attitude(trigger.target, trigger.player) > 0) {
                                   return false;
                              }
               ...
               _wuxie: {
                    ...
                    content: function () {
                         ...
                         if (trigger.name == 'phaseJudge') {
                              if (get.itemtype(card) == 'card' && card.viewAs) card = get.autoViewAs({
                                   name: card.viewAs
                              }, [card]);
                    ...
                    ai: {
                         ...
                         equipValue: function (card, player) {
                              if (!game.hasPlayer(function (current) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he')) return current != player && get.distance(player, current) <= 1 && get.attitude(player, current) < 0;
                                   return current != player && get.distance(player, current) <= 2 && get.attitude(player, current) < 0;
                              })) return 1;
                              if (player.hasSha() && _status.currentPhase == player) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he') && game.hasPlayer(function (current) {
                                        return current != player && get.distance(player, current) <= 1 && get.attitude(player, current) < 0 && player.hasCard(function (card) {
                                        ...
                                        if (player.getEquip('zhuge')) {
                                             ...
                                             }) <= 1 && player.countUsed('sha', true) == 0) return 0;
                                             return 10;
                                        }
                                   ...
                                   if (player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he') && player.getCardUsable('sha') == 0 && game.hasPlayer(function (current) {
                                        return current != player && get.distance(player, current) <= 2 && get.attitude(player, current) < 0 && player.hasCard(function (card) {
                                             return get.name(card) == 'sha' && get.effect(current, card, player, player) > 0
                                        }) && player.canUse('sha', current);
                                   })) {
                                        if (player.getEquip('zhuge')) {
                              ...
                              if (_status.currentPhase == player) {
                                   ...
                                   if (!game.hasPlayer(function (current) {
                                        return get.attitude(player, current) < 0 && get.effect(current, {
                                             name: 'sha'
                                        }, player, player);
                                   })) return 0;
                              }
                    ...
                    ai: {
                         equipValue: function (card, player) {
                              ...
                              if (!game.hasPlayer(function (current) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he')) return current != player && get.distance(player, current) <= 3 && get.attitude(player, current) < 0;
                              ...
                              if (!player.hasCard({
                                   ...
                                   for (var j = 0; j < game.players.length; j++) {
                                        ...
                                        if (game.hasPlayer(function (current) {
                                             ...
                                             if (game.hasPlayer(function (current) {
                                                  return current == target1 && player.getCardUsable('sha') >= 1 && player.hasCard(function (card) {
                                                       return get.name(card) == 'sha' && get.effect(current, card, player, player) > 0
                                                  }) && he > 2;
                                             })) {
                                   ...
                                   for (var j = 0; j < game.players.length; j++) {
                                        ...
                                        if (game.hasPlayer(function (current) {
                                             ...
                                             if (game.hasPlayer(function (current) {
                                                  return current == target2 && player.getCardUsable('sha') >= 1 && player.hasCard(function (card) {
                                                       return get.name(card) == 'sha' && get.effect(current, card, player, player) > 0
                                                  }) && he > 2;
                                             })) {
                                                  if (player.getEquip('zhuge') && player.countCards('h', function (card) {
                    ...
                    ai: {
                         equipValue: function (card, player) {
                              if (!game.hasPlayer(function (current) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he')) return current != player && get.distance(player, current) <= 2 && get.attitude(player, current) < 0;
                              ...
                              if (!player.hasCard({
                                   ...
                                   for (var j = 0; j < game.players.length; j++) {
                                        ...
                                        if (game.hasPlayer(function (current) {
                                             ...
                                             if (game.hasPlayer(function (current) {
                                                  return current == target2 && player.canUse('sha', current) && player.hasCard(function (card) {
                                                       return get.name(card) == 'sha' && get.effect(current, card, player, player) > 0
                                                  });
                                             })) return 6;
                                             return 4;
                         ...
                         basic: {
                              order: function (item, player) {
                                   var ssdy = game.countPlayer(function (current) {
                                        return get.attitude(current, player) > 0 && current.hp < 3;
                                   });
                                   var ssdr = game.countPlayer(function (current) {
                                        return get.attitude(current, player) <= 0 && current.hp < 3;
                                   });
                                   if (ssdy > 0 && ssdr == 0) return 11;
                                   return 0.5;
                              },
                              useful: [3, 1],
                              value: 0,
               ...
               nanman: {
                    ...
                    ai: {
                         wuxie: function (target, card, player, viewer, status) {
                              ...
                              if (viewer == player && player.countCards('hs', 'sha') > 0) return 0;
                         ...
                         result: {
                              "target_use": function (player, target) {
                                   ...
                                   if (get.mode() == 'identity') {
                                        if (target.isZhu && (nh <= 2 || target.hp <= 2)) {
                                             if (player.identity == 'nei' && (get.population('zhong') > 0 || get.population('fan') > 0)) {
                                                  return -1;
                                             }
                                             return -100;
                                        }
               ...
               wanjian: {
                    ...
                    ai: {
                         wuxie: function (target, card, player, viewer, status) {
                              ...
                              if (viewer == player && player.countCards('hs', 'shan') > 0) return 0;
                              let att = get.attitude(viewer, target);
                              if (status * att < 0) return 0;
                              let evt = _status.event.getParent('useCard'), t = target, has = 0;
                              if (viewer == target) return;
                    ...
                    content: function () {
                         ...
                         if (event.directHit) {
                              ...
                              var next = event.turn.chooseToRespond({
                                   name: event.respondCard
                              });
                              if (event.shaRequired > 1) {
                                   next.set('prompt2', '共需打出' + event.shaRequired + '张' + get.translation(event.respondCard))
                              }
                              next.set('ai', function (card) {
                                   ...
                                   if (event.shaRequired > 1 && player.countCards('h', event.respondCard) < event.shaRequired) return 0;
                              ...
                              if (result.bool) {
                                   ...
                                   if (event.turn == target) {
                                        ...
                                        if (event.shaRequired > 0) event.goto(2);
                                             ...
                                             event.turn = player;
                                             if (event.turn.isDead()) event.finish();
                                             else event.goto(1);
                                        }
                                   } else {
                    ...
                    ai: {
                         ...
                         result: {
                              ...
                              player: function (player, target, card) {
                                   ...
                                   if (get.damageEffect(target, target, player) > 0 && get.attitude(player, target) > 0) {
                                        return 0;
                                   }
               ...
               jiedao: {
                    ...
                    content: function () {
                         ...
                         if (event.directfalse || result.bool == false) {
                              ...
                              if (cards.length > 1) {
                                   target.chooseButton(['选择一张武器牌交给' + get.translation(player), target.getCards('e', {
                                        subtype: 'equip1'
                                   })], 1, true);
                              }
                              else {
                         ...
                         "step 2"
                         if (result.bool) {
                              player.gain(result.links, target, 'give', 'bySelf');
                         }
                    },
                    ai: {
                         wuxie: function (target, card, player, viewer) {
