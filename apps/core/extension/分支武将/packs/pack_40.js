// Extracted from card/extra.js; legacy startup and engine/UI patches are not executed.
...
"skill": {
               ...
               muniu_skill: {
                    ...
                    check: function(card) {
                         ...
                         if (card.name == 'jsrg_ying') return 0;
                         if (nh>player.hp){
                         if (card.name == 'wuxie') return 15;
               ...
               jiu2: {
                    ...
                    filter: function(event, player) {
                         ...
                         if (event.name == 'useCard') return (event.card && (event.card.name == 'sha')&&event.jiu);
                         return true;
                    },
                    forced: true,
                    popup: false,
               ...
               huogong: {
                    ...
                    content: function() {
                         ...
                         if (target.countCards('h') == 0) {
                              event.finish();
                              return;
                         } else if (target.countCards('h') == 1) event._result = {
               ...
               tiesuo: {
                    ...
                    ai: {
                         wuxie: function(target, card, player, viewer) {
                              ...
                              if (player.countCards('hs', 'wuxie') == 1) return 0;
                         },
                         basic: {
                              useful: 4,
                              value: 4,
                         ...
                         result: {
                              target:function(player,target){
                              var hzs = game.hasPlayer(function(current){
								return current.isLinked();
							});
						var hzdy = game.hasPlayer(function(current){
							...
							if (player.countCards('hs', {
                                             ...
                                             name: 'huogong',
                                        }) == 0 && !player.getEquip('zhuque')) {
                                        return 0;
                                        }
							}
							if(hzdy<2&&hzs<2&&target.isLinked()) return 0;
							if(game.countPlayer()<=3&&hzs==1&&player.isLinked()) return 0;
							}
							if(target.isLinked()){
							     if(game.countPlayer()<=3&&hzs==1&&hzdy==1) return 0;
                    ...
                    ai: {
                         equipValue: function(card, player) {
                              if (!game.hasPlayer(function(current) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he')) return current != player && get.distance(player, current) <= 4 && get.attitude(player, current) < 0;
               ...
               guding: {
                    ...
                    ai: {
                         equipValue: function(card, player) {
                              ...
                              if ((player.hasSkill('repojun') || player.hasSkill('pojun')) && game.hasPlayer(function(current) {
                                   if (!player.hasCard({
                                        subtype: 'equip4'
                                   }, 'he')) return current != player && get.distance(player, current) <= 2 && get.attitude(player, current) < 0;
                                   return current != player && get.distance(player, current) <= 3 && get.attitude(player, current) < 0;
                                   ...
                                   if (player.hasCard({
                                        name: 'zhuge'
                                   }) && player.getCardUsable('sha') == 0 && player.countCards('h', function(card) {
                                        return get.name(card) == 'sha' && player.hasValueTarget(card);
                                   }) >= 2) return 4.5;
                                   return 12;
                              }
