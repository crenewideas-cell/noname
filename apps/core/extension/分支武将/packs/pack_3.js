// Extracted from character/collab.js; legacy startup and engine/UI patches are not executed.
export default function(lib, game, ui, get, ai, _status) {
return {
"character": {
			boss_wuyi:['male','shu',4,['boss_benxi']],
        },
"characterSort": {
			collab:{
				collab_olympic:[],
...
"dynamicTranslate": {
            boss_benxi: function(player) {
                if (player.storage.boss_benxi == true) return '锁定技，转换技，当你失去手牌后，阳：随机念一句包含“wuyi”的技能台词；<span class="bluetext">阴：获得上次念台词的技能直到你下回合开始。若你已有该技能，改为对其他角色各造成1点伤害。</span>';
                return '锁定技，转换技，当你失去手牌后，<span class="bluetext">阳：随机念一句包含“wuyi”的技能台词；</span>阴：获得上次念台词的技能直到你下回合开始。若你已有该技能，改为对其他角色各造成1点伤害。';
            },
        },
"translate": {
			old_lingju:'SP灵雎',
            ...
            nezha: '哪吒',
            dcsantou: '三头',
            dcsantou_info: '锁定技，防止你受到的所有伤害。<br/>若你本回合已防止过该伤害来源的伤害且你体力值大于等于3，你失去1体力；<br/>若防止的伤害为属性伤害且你体力值为2，你失去1体力；<br/>若防止的伤害为红色牌造成的伤害且你体力值为1，你失去1体力。',
            dcfaqi: '法器',
			dcfaqi2:'法器',
			dcfaqi_info:'出牌阶段，当你使用装备牌后，视为使用一张普通锦囊牌。（每回合每种牌名限一次）',
			boss_wuyi:'地主·吴懿',
			boss_wuyi_ab:'吴懿',
			boss_benxi:'奔袭',
			boss_benxi_info:'锁定技，转换技，当你失去手牌后，阳：随机念一句包含“wuyi”的技能台词；阴：获得上次念台词的技能直到你下回合开始。若你已有该技能，改为对其他角色各造成1点伤害。',

			collab_olympic:'OL·伦敦奥运会',
			collab_tongque:'OL·铜雀台',
			sp_zhongdan:"OL·忠胆英杰",
			collab_duanwu:'新服·端午畅玩',
			collab_decade:'新服·斗地主',
			collab_tianshu:'OL·天书乱斗',
        },
"skill": {
        boss_benxi:{
            mark:true,
            ...
            marktext:"☯",
                intro:{
                    content:function(storage,player,skill){
                        if(player.storage.boss_benxi == true) return '获得上次念台词的技能直到你下回合开始。若你已有该技能，改为对其他角色各造成1点伤害。';
                        return '随机念一句包含“wuyi”的技能台词';
                    },
                },
				trigger:{
					player:"loseAfter",
				...
				content:function(){
					if(!player.storage.boss_benxi){
                        var keys = Object.keys(lib.skillSet.boss_benxi);
                        var skill = keys[Math.floor(Math.random() * keys.length)];
						if(lib.skillSet.boss_benxi[skill].length>=1&&lib.skillSet.boss_benxi[skill][0].length>0){
							game.playAudio(lib.skillSet.boss_benxi[skill][0]);
						}
						else if(lib.skill[skill].audio) game.trySkillAudio(skill);
						if(lib.skillSet.boss_benxi[skill].length>=2&&lib.skillSet.boss_benxi[skill][1].length>0) player.chat(lib.skillSet.boss_benxi[skill][1]);
						player.storage.boss_benxi2=skill;
                        player.changeZhuanhuanji('boss_benxi');
					}
					else{
						if(!player.hasSkill(player.storage.boss_benxi2)) player.addTempSkill(player.storage.boss_benxi2,{player:'phaseBegin'});
					else{
                            game.players.forEach(p => {
							if(p!=player) p.damage();
                            });
                        }
                        player.changeZhuanhuanji('boss_benxi');
                    }
			...
			dcbianzhuang:{
				...
				content:function(){
                    ...
                    player.chooseControl(skills).set('dialog', [
                        '选择获得一个技能并“变装”', [characters, 'character']]);
                    'step 1'
            ...
            dcsantou: {
                ...
                forced: true,
				init: function (player) {
                    if (!player.storage.dcsantou) player.storage.dcsantou = [];
                },
				content: function () {
                    'step 0'
                    trigger.cancel();
                    'step 1'
					if (player.hp == 1 && get.color(trigger.getParent())=='red') {
                        player.loseHp();
                    }
                    if (player.hp == 2 && trigger.nature != null) {
                        player.loseHp();
                    }
                    if (player.hp >= 3 && player.storage.dcsantou.contains(trigger.source)) {
                        player.loseHp();
                    }
                    if (!player.storage.dcsantou.contains(trigger.source)) {
                        player.storage.dcsantou.push(trigger.source);
                    }
                },
                group: ['dcsantou_clear', 'dcsantou_balance'],
                subSkill: {
                    clear: {
