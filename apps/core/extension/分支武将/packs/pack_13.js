// Extracted from character/offline.js; legacy startup and engine/UI patches are not executed.
...
"characterSort": {
			offline:{
				...
				offline_luanwu:[],
				offline_yongjian:[],
				offline_piracyE:['jiangfei','chendong'],
				offline_piracyS:[],
				offline_piracyK:[],
				offline_vtuber:[],
				//offline_others:[""],
			...
			jsrgpingtao:{
				...
				content:function(){
					...
					if(result.bool){
						...
						player.useCard({name:'sha',isCard:true},target,false).set('addCount', true);
					}
				},
				ai:{
					expose:0.15,
			...
			jsrgjizhao:{
				...
				content:function(){
					...
					player.chooseTarget(get.prompt2('jsrgjizhao')).set('ai',target=>{
						var player=_status.event.player;
						if (target.getCards('h',function(card){
						     return game.hasPlayer(function(current){
						          return target.canUse(card,current)
						     })
						})&&get.attitude(player,target)>0) return get.attitude(player,target);
						if(target.countCards('j')&&get.attitude(player,target)>0) return get.attitude(player,target)*2;
						return 0;
					});
					'step 1'
					if(result.bool){
