// Extracted from character/sp2.js; legacy startup and engine/UI patches are not executed.
export default function(lib, game, ui, get, ai, _status) {
return {
"character": {
			lijueguosi: ['male', 'qun', 4, ['dcxiongsuan']],
...
"characterSort": {
			sp2: {
				...
				sp_guandu: [],
				sp_qihuan: [],
				sp_binglin: [ "sp_mifangfushiren", 'dc_zhaoyan'],
				sp_danqi: [ 'qinyilu', 'guānning'],
				sp_fenghuo: [],
				sp_huangjin: [],
				sp_fadong: [],
				sp_xuzhou: [],
...
"characterTitle": {
			chunyuqiong: '#b对决限定武将',
			sp_xuyou: '#g4v4限定武将',
			zhangchangpu: 'DC·系列专属',
			dc_fuwan: 'DC·系列专属',
		},
"characterIntro": {
			lijueguosi: '李傕，字稚然。北地郡泥阳县人，郭汜，又名郭多，是凉州张掖人。二人本来为董卓部下，董卓被杀以后，二人听贾诩献策，联兵将攻向长安，击败吕布，杀死王允等人，占领长安，把持朝廷大权。',
			dc_fuwan: '伏完，琅琊东武人，东汉末年大臣，大司徒伏淇六世孙，大司农伏质之子，汉献帝伏皇后之父。<br>伏完成年后袭爵不其侯，娶桓帝女阳安长公主为妻，历任侍中、执金吾、辅国将军、中散大夫、屯骑校尉。<br>建安五年，曹操杀死董承，并下今处死其女董贵人。<br>伏皇后感到害怕，于是给伏完写信，诉说曹操威逼朝廷、献帝心存怨恨之事，希望伏完能谋划诛杀曹操，伏完不敢有所举动。',
...
"dynamicTranslate": {
			...
			chuaili: function (player) {
				if (!player.hasSkill('piaoping', null, null, false)) return '锁定技，当你成为其他角色使用黑色牌的目标后，若你的〖漂萍〗：处于阴状态，则你将〖漂萍〗转换至阳状态；处于阳状态，则你令“托献”可使用次数+1，然后若“托献”可使用次数超过3，〖惴栗〗于本回合内失效。';
				if (player.storage.piaoping) return '锁定技，当你成为其他角色使用黑色牌的目标后，若你的〖漂萍〗：<span class="bluetext">处于阴状态，则你将〖漂萍〗转换至阳状态；</span>处于阳状态，则你令“托献”可使用次数+1，然后若“托献”可使用次数超过3，〖惴栗〗于本回合内失效。';
				return '锁定技，当你成为其他角色使用黑色牌的目标后，若你的〖漂萍〗：处于阳状态，则你将〖漂萍〗转换至阴状态；<span class="bluetext">处于阴状态，则你令“托献”可使用次数+1，然后若“托献”可使用次数超过3，〖惴栗〗于本回合内失效。</span>';
			},
			dcdouzhen: function (player) {
				var str = '锁定技，①转换技，你的回合内，';
				if (player.countMark('dcdouzhen') % 2) str += '阳：当你使用非转化且对应的实体牌为一张黑色基本牌的【决斗】时，你获得目标角色各一张牌并获得1枚“☯”；<span class="bluetext">阴：当你使用或打出非转化且对应的实体牌为一张红色基本牌的【杀】时，你获得1枚“☯”。</span>②若你的“☯”数为：偶数，你的黑色基本牌均视为【决斗】；<span class="bluetext">奇数，你的红色基本牌均视为无次数限制的普【杀】。</span>';
				else str += '<span class="bluetext">阳：当你使用非转化且对应的实体牌为一张黑色基本牌的【决斗】时，你获得目标角色各一张牌并获得1枚“☯”；</span>阴：当你使用或打出非转化且对应的实体牌为一张红色基本牌的【杀】时，你获得1枚“☯”。</span>②若你的“☯”数为：<span class="bluetext">偶数，你的黑色基本牌均视为【决斗】；</span>奇数，你的红色基本牌均视为无次数限制的普【杀】。';
				return str;
			},
		},
"perfectPair": {},
"translate": {
			...
			haomeng: '郝萌',
			xiongmang: '雄莽',
			xiongmang_info: '你可将任意张花色各不相同的手牌当做目标数上限为X的【杀】使用（X为此【杀】对应的实体牌数）。此【杀】使用结算结束后，若你未造成过渠道为此牌的伤害，则你减1点体力上限。若此【杀】造成了伤害，此阶段你使用【杀】的次数上限+1。',
			yanfuren: '严夫人',
			channi: '谗逆',
			...
			chuaili: '惴栗',
			chuaili_info: '锁定技，当你成为其他角色使用黑色牌的目标后，若你的〖漂萍〗：处于阳状态，则你将〖漂萍〗转换至阴状态；处于阴状态，则你令“托献”可使用次数+1，然后若“托献”可使用次数超过3，〖惴栗〗于本回合内失效。',
			fengfang: '冯方',
			dcditing: '谛听',
			...
			sunlang: '孙狼',
			dctingxian: '铤险',
			dctingxian_info: '每回合限一次。当你使用【杀】指定最后一个目标后，你可以摸X张牌，然后可令此【杀】对其中至多X个目标无效（X为你装备区的牌数+1）。',
			dcbenshi: '奔矢',
			dcbenshi_info: '锁定技，①你的攻击范围+1。②由你使用的【杀】的牌面信息中的“使用目标”产生的规则改为“攻击范围内的所有角色”。',
			sunhuan: '孙桓',
			dcniji: '逆击',
			dcniji_info: '当你成为基本牌或锦囊牌的目标后，你可以摸一张牌。每个结束阶段，你弃置所有本回合以此法摸到的牌。弃牌前你可以先使用其中一张牌。',
			dc_fuwan: '伏完',
			dcmoukui: '谋溃',
			dcmoukui_info: '当你使用【杀】指定目标后，你可以选择任意项：1.摸一张牌；2.弃置其中一个目标角色一张牌。若你两项都选，则此【杀】无效或被抵消后该角色弃置你一张牌。',
			guānning: '关宁',
			dcxiuwen: '修文',
			...
			dclvecheng: '掠城',
			dclvecheng_info: '出牌阶段限一次。你可以选择一名其他角色，你于本回合对其使用当前手牌中的【杀】无次数限制。然后回合结束时，其展示所有手牌，若其中有【杀】，其可以选择对你依次使用其中所有的【杀】。',
			dczhongji: '螽集',
			dczhongji_info: '当你使用牌时，若你没有与此牌花色相同的手牌且手牌数小于体力上限，你可以将手牌摸至体力上限并弃置X张牌（X为本回合发动〖螽集〗的次数）。',
			dc_mengda: '孟达',
			dclibang: '利傍',
			...
			dc_jsp_guanyu: '魏关羽',
			dcdanji: '单骑',
			dcdanji_info: '觉醒技，准备阶段，若你区域内牌数大于体力值，你减1点体力上限，将体力回复至体力上限并摸等量张牌，然后获得〖马术〗和〖怒嗔〗。',
			dcnuchen: '怒嗔',
			...
			dchumei: '狐魅',
			dchumei_info: '出牌阶段各限一次，你可以选择一名体力值不大于X的角色，令其：①摸一张牌。②交给你一张牌。③回复1点体力。（X为你本阶段造成伤害数）',
			lijueguosi: '李傕郭汜',
			dcxiongsuan: '凶算',
			dcxiongsuan_info: '出牌阶段限一次，你可以弃置一张手牌并对一名角色造成1点伤害，然后摸三张牌。若该角色不为你，你失去1点体力。',
...
"skill": {
			dcxiongsuan: {
				...
				content: function () {
					target.damage();
					player.draw(3);
					if (target != player) player.loseHp();
				},
				ai: {
					order: 8,
					expose: 0.2,
					...
					result: {
						...
						target: function (player, target) {
							if (get.attitude(player, target) < 0) return target.maxHp - target.hp - 0.4;
							if (target == player && player.hp >= 3) return 1.8;
							return -1;
						}
					}
				},
			...
			dcjichou: {
				...
				filter: function (event, player) {
					...
					player.getHistory('useCard', e => {
						if (e.card && e.card.name) {
							if (list.contains(e.card.name)) check = true;
							else list.push(e.card.name);
						}
					});
				...
				intro: {
					content: function (storage, event, player) {
						if (!storage || storage.length == 0) return '当前没有通过【急筹】给出过牌';
						return '已通过【急筹】给出' + get.translation(storage);
					}
				},
				init: function (player) {
					if (!player.storage.dcjichou) player.storage.dcjichou = [];
				},
				content: function () {
					'step 0'
					var cu = [];
					player.getHistory('useCard', e => {
						if (e.card && e.card.name && e.cards) cu.push(e.cards[0]);
					});
					event.list = [];
					for (let j = 0; j < ui.discardPile.childNodes.length; j++) {
						var card = ui.discardPile.childNodes[j];
						if (cu.includes(card)) event.list.push(card);
					}
					event.given = [];
					event.new = 0;
					if (event.list.length == 0) event.finish();
					'step 1'
					player.chooseButton(['选择一张牌交给一名角色', event.list], 1).set('ai', function (button) {
						var player = _status.event.player, card = button.link;
						return player.getUseValue(card);
					});
					'step 2'
					if (result.bool) {
						event.cc = result.links[0];
						player.chooseTarget(true, '将' + get.translation(event.cc) + '交给一名角色', function (card, player, target) {
							return !event.given.includes(target)
					...
					'step 3'
					if (result.bool) {
						result.targets[0].gain(event.cc, 'gain2').giver = player;
						if (!player.storage.dcjichou.includes(get.translation(get.name(event.cc)))) {
							player.storage.dcjichou.push(get.translation(get.name(event.cc)));
							event.new++;
						}
						event.given.add(result.targets[0]);
						var index = event.list.indexOf(event.cc);
						if (index !== -1) {
							event.list.splice(index, 1);
						}
						if (event.list.length > 0 && event.given.length < game.players.length) event.goto(1);
					}
					'step 4'
					player.draw(event.new);
				},
			},
			dcmouli: {
				forced: true,
				...
				filter: function (event, player) {
					return player.storage.dcjichou.length > 4;
				},
				content: function () {
					player.awakenSkill('dcmouli');
					player.gainMaxHp();
					player.recover();
					player.addSkill('dczifu');
				}
			},
			dczifu: {
				forced: true,
				...
				content: function () {
					'step 0'
					event.drawNum = Math.min(player.maxHp - player.countCards('h'), 5);
					player.draw(event.drawNum);
					'step 1'
					if (event.drawNum > 0 && player.countCards('h') > 0) {
						var list = [];
						for (var i of player.getCards('h')) {
							if (!list.includes(i.name)) list.push(i.name)
						}
						var num = list.length;
						player.chooseCard('h', num, '保留每种牌名的手牌各一张', true, function (card) {
							for (var j = 0; j < ui.selected.cards.length; j++) {
								if (ui.selected.cards[j].name == card.name) return false;
							}
							return true;
						}).set('ai', function (card) {
							return get.value(card);
						}).set('complexCard', true).set('maxNum', num);
					}
					else event.finish()
					'step 2'
			...
			dchumei: {
				...
				filterTarget: function (card, player, target) {
					if (target.hp > _status.event.dchumei_num) return false;
			...
			dctongye: {
				...
				forced: true,
				filter: function (event, player) {
					if (game.countGroup() > 5) return false;
					if (event.name == 'die') return true;
					return event.name != 'phase' || game.phaseNumber == 0;
				},
				content: function () {
					player.removeMark('dctongye_draw', player.countMark('dctongye_draw'), false);
					player.removeMark('dctongye_sha', 3, false);
					player.removeMark('dctongye_range', 3, false);
					player.removeMark('dctongye_handcard', 3, false);
					...
					if (num <= 3) {
						...
						player.addMark('dctongye_draw', 1, false);
			...
			dcdanji: {
				...
				filter: function (event, player) {
					return player.countCards('hej') > player.hp;
				},
				content: function () {
					'step 0'
					player.loseMaxHp();
			...
			dclvecheng: {
				...
				content: function () {
					...
					player.addGaintag(cards, 'dclvecheng');
				...
				subSkill: {
					xiongluan: {
						...
						content: function () {
							'step 0'
							player.removeGaintag('dclvecheng');
						...
						mod: {
							cardUsableTarget: function (card, player, target) {
								if (card.cards && card.cards[0].gaintag.contains('dclvecheng') && player.getStorage('dclvecheng_xiongluan').contains(target)) return Infinity;
							},
						},
					}
				}
			...
			dczhongji: {
				...
				filter: function (event, player) {
					...
					return (!lib.suit.contains(suit) || !player.countCards('h', { suit: suit })) && player.countCards('h') < player.maxHp;
				},
				check: function (event, player) {
			...
			chuaili: {
				...
				content: function () {
					if (player.storage.piaoping == true) {
						...
						if (player.countMark('tuoxian') > 3) player.addTempSkill('chuaili_blocker');
					}
					game.delayx();
				},
				ai: { combo: 'piaoping' },
			...
			xiongmang: {
				...
				onuse: function (links, player) {
					...
					player.addTempSkill('xiongmang_buff');
				},
				ai: {
				...
				subSkill: {
					...
					buff: {
						...
						content: function () {
							player.addMark('xiongmang_sha', 1, false);
							player.addTempSkill('xiongmang_sha');
						},
					},
					sha: {
						onremove: true,
						...
						mod: {
							cardUsable: function (card, player, num) {
								if (card.name == 'sha') return num + player.countMark('xiongmang_sha');
							},
						},
			...
			reliewei: {
				...
				trigger: { global: "dying" },
				filter: function (event, player) {
					return player == _status.currentPhase || player.getHistory("useSkill", evt => evt.skill == "reliewei").length < player.hp;
				},
				frequent: true,
				content: function () {
					player.draw();
			...
			pingjian: {
				...
				content: function () {
					...
					var skills = [];
					var map = [];
					allList = allList.filter(o => typeof o === 'string');
					allList.randomSort();
