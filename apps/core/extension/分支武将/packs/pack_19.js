// Extracted from character/shiji.js; legacy startup and engine/UI patches are not executed.
...
"characterSort": {
            shiji: {
                mobile_shijizhi: [ "sp_sunshao"],
                mobile_shijixin: [],
                mobile_shijiren: [],
                mobile_shijiyong: [],
                mobile_shijiyan: [ 'sp_lvfan', 'sp_zhujun'],
            },
        },
"characterFilter": {},
"characterTitle": {},
...
"translate": {
            ...
            dbzhuifeng: '椎锋',
            dbzhuifeng_info: '魏势力技，每回合限两次，你可以失去1点体力并视为使用一张【决斗】(若你因此死亡，此【决斗】依然生效)。当你因此【决斗】而受到伤害时，你防止此伤害并令此技能失效直到出牌阶段结束。',
            dbchongjian: '冲坚',
...
"skill": {
                    ...
                    if (!result.bool) {
                        ...
                        if (!cards.length || !game.hasPlayer((current) => current != player && current != target)) event.finish();
                            ...
                            player.chooseTarget('请选择一名角色', '令其获得' + get.translation(cards) + '，且视为对' + get.translation(target) + '使用一张火【杀】', function (card, player, target) {
                                ...
                                if (val <= 0) return 0;
                                return val + target.canUse({
                                    name: 'sha',
                                    nature: 'fire',
                                    isCard: true
                                }, target2, false) ? get.effect(target2, {
                                    name: 'sha',
                                    nature: 'fire',
                                    isCard: true
                                }, target, player) : 0;
            ...
            sptaoluan: {
                ...
                content: function () {
                    ...
                    if (trigger.player.isIn() && player.canUse({
                        name: 'sha',
                        nature: 'fire',
                        isCard: true
                    }, trigger.player, false)) list.push(1);
            ...
            dbquedi: {
                ...
                usable: 1,
                filter: function (event, player) {
                    return (event.card.name == 'sha' || event.card.name == 'juedou') && event.targets.length == 1 && (event.target.countGainableCards(player, 'h') > 0 || player.hasCard(function (i) {
                ...
                content: function () {
                    ...
                    player.chooseControl(list).set('choiceList', [
                        ...
                        '背水！减1点体力上限并执行所有选项',]).set('prompt', get.prompt('dbquedi', target)).set('ai', function () {
                            ...
                            var bool2 = player.hasCard(function (i) {
                                return get.type(i, player) == 'basic' && lib.filter.cardDiscardable(i, player, 'dbquedi') && get.value(card, player) < 7;
                            }, 'h') && !target.hasSkillTag('filterDamage', null, {
                                player: player,
                                card: card,
                            });
                            if (bool1 && bool2 && (target.hp <= 2 || (player.isDamaged() && player.maxHp > 3) || player.maxHp > 4)) return '背水！';
                            if (bool1) return '选项一';
                            if (bool2) return '选项二';
                            return 'cancel2';
                        });
            ...
            dbzhuifeng: {
                ...
                ai: {
                    ...
                    result: {
                        player: function (player, target) {
                            if (!player.hasFriend() && player.hp == 1) return -999;
                        },
                    },
                },
                ...
                subSkill: {
                    self: {
                        ...
                        filter: function (event, player) {
                            ...
                            return evt.name == 'dbzhuifeng' && evt.player == player;
                        },
                        content: function () {
                            trigger.cancel();
            ...
            jianzhan: {
                ...
                content: function () {
                    ...
                    if (!targets[0].canUse('sha', targets[1])) event._result = { index: 1 };
                        ...
                        var evt = _status.event.getParent();
                        var eff = get.effect(evt.targets[1], {
                            name: 'sha',
                            isCard: true
                        }, evt.targets[0], evt.targets[0]);
                        if (eff > 0) return 0;
                    ...
                    result: {
                        target: function (player, target, card, isLink) {
                            ...
                            if (evt && evt.card == card && evt.customArgs && evt.customArgs.tiaojiyanmei_map) {
                                var map = evt.customArgs.tiaojiyanmei_map, num1 = map.average,
