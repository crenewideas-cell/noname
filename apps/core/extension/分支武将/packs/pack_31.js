// Extracted from character/yingbian.js; legacy startup and engine/UI patches are not executed.
export default function(lib, game, ui, get, ai, _status) {
return {
"character": {
			chengjichengcui: ['male', 'jin', 6, ['tousui','chuming'], ['doublegroup:jin:wei']],
		},
"characterSort": {
			yingbian:{
				yingbian_pack1:[],
				yingbian_pack2:['chengjichengcui'],
				yingbian_pack3:[],
				yingbian_pack4:[],
				yingbian_pack5:[],
			},
...
"translate": {
			...
			tousui: '透髓',
			tousui_info: '你可以将任意张牌置于牌堆底，视为使用一张需要等量张【闪】抵消的【杀】。',
			chuming: '畜鸣',
			chuming_info: '锁定技，当你对其他角色造成伤害时或受到其他角色造成的伤害时，若没有造成伤害的实体牌，此伤害+ 1；否则其于此回合结束时将造成伤害的牌当【借刀杀人】或【过河拆桥】对你使用。',
...
"skill": {
			...
			weishu:{
				...
				forced:true,
				filter:function(event,player){
					return (event.getParent().name=='chongzhu'||event.getParent().name=='draw')&&event.getParent(2).name!='weishu'&&event.getParent('phaseDraw').player!=player;
				},
				content:function(){
					'step 0'
					...
					effect:{
						...
						content:function(){
							...
							if(target.isIn()&&player.canUse(card,target,null,true)){
								player.useCard(card,target,cards).set('addCount',true);
							}
							'step 1'
			...
			recaiwang:{
				audio:'caiwang',
				inherit:'caiwang',
				group:[/*'recaiwang_hand','recaiwang_equip','recaiwang_judge'*/'recaiwang_all'],
			},
			recaiwang_hand:{
				audio:'caiwang',
			...
			recaiwang_all:{
				...
				prompt:"你可以将一张【毒】当作任意基本牌使用",
				chooseButton:{
					dialog:function(event,player){
						var list=[];
						if(player.countCards('h')==1) list.push(['基本','','shan']);
						if(player.countCards('e')==1) list.push(['锦囊','','wuxie']);
						if(player.countCards('j')==1) list.push(['基本','','sha']);
						console.log(list)
						return ui.create.dialog('才望',[list,'vcard']);
					},
					check:function(button){
						var player=_status.event.player;
					...
					prompt:function(links,player){
						return '将一张【毒】当做'+(get.translation(links[0][3])||'')+get.translation(links[0][2])+'使用';
					},
					mod:{
			...
			tousui: {
				...
				filterTarget: function (_card, player, target) {
				...
				content: function () {
					if (!player.storage.tousui) player.storage.tousui = cards.length;
					...
					player.useCard({name: "sha",isCard: true},targets).set('addCount', true);
				},
				group: 'tousui_sha',
				subSkill: {
					sha: {
						...
						popup: false,
						filter: function (event, player) {
							return event.getParent(2).name == 'tousui' && !event.getParent().directHit.contains(event.target);
						},
						logTarget: "target",
						content: function () {
							...
							if (typeof map[id].shanRequired == 'number') {
								map[id].shanRequired += player.storage.tousui - 1;
							}
							else {
								map[id].shanRequired = player.storage.tousui;
							}
						},
						ai: {
							"directHit_ai": true,
							skillTagFilter: function (player, tag, arg) {
								if (arg.card.name != 'sha' || arg.target.countCards('h', 'shan') > player.storage.tousui) return false;
							},
						},
						sub:true,
					},
			...
			chuming: {
				...
				forced: true,
				filter: function (event, player) {
					return !player.hasSkill('chuming_over');
				},
				content: function () {
					if (get.itemtype(trigger.cards) != 'cards') {
						...
						player.addTempSkill('chuming_over', { player: 'damageEnd' });
						target.addToExpansion(trigger.cards, 'gain2').gaintag.add('chuming');
					}
				},
				group: 'chuming_aida',
				subSkill: {
					aida: {
						...
						filter: function (event, player) {
							var targets = game.filterPlayer(function (current) {
								return current.getExpansions('chuming').length > 0;
							});
						...
						content: function () {
							...
							if (event.targets.length) {
								event.cards = event.targets[0].getExpansions('chuming');
								var list = [];
								...
								if (player.getEquip(1))
									list.push(['锦囊', '', 'jiedao']);
								if (list.length) {
									event.targets[0].chooseButton(['畜鸣：将' + get.translation(event.cards) + '视为对' + get.translation(player) + '使用【借刀杀人】或【过河拆桥】', [list, 'vcard']], true).set('filterButton', function (button) {
										return event.targets[0].hasUseTarget({ name: button.link[2], isCard: true }, player, true);
									}).set('ai', function (button) {
										return event.targets[0].getUseValue({ name: button.link[2], isCard: true });
									});
								}
								else event._result = { bool: false };
							}
							'step 1'
							if (result.bool) {
								event.targets[0].useCard({ name: result.links[0][2], isCard: true }, event.cards, player);
