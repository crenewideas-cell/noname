import { loadScript } from '../compat.js';
import {
	lib,
	get,
	_status,
...
export let PRECONTENT = async function(config) {
	//-----Q-----END-----
	// @ts-ignore
	...
	// @ts-ignore
	if ((lib.config.qhly_currentViewSkin == 'decade' || lib.config.qhly_currentViewSkin == 'shousha') && !lib
		.config['extension_千幻聆音_qhly_decadeCloseDynamic']) window._qhlyThunder = true; //用于判断是否打开的是千幻聆音雷修版
	// @ts-ignore
	...
	// @ts-ignore
	await loadScript(lib.qhly_path + 'data/sanguoskininfo.js');
	// @ts-ignore
			...
			Object.keys(lib.qhly_skinChange[character]).forEach(skin => {
				...
				Object.keys(lib.qhly_skinChange[character][skin]).forEach(key => {
					if (keys.includes(key)) {
						value.add(key);
						// @ts-ignore
						if (typeof lib.qhly_skinChange[character][skin][key] !=
							'string') {
							str += (character + '的皮肤“' + skin + '”中的“' + key +
								'”属性填写错误，请检查！');
							alert(str);
							alerted = true;
						}
						if (key != 'source') {
							// @ts-ignore
							lib.qhly_skinChange[character][skin][key + '1'] = lib
								.qhly_skinChange[character][skin][key];
							// @ts-ignore
							if (key == 'audio') lib.qhly_skinChange[character][skin]
								[key + '2'] = skin + '/';
							else {
								// @ts-ignore
								lib.qhly_skinChange[character][skin][key + '2'] =
									character + '/' + skin + '.jpg';
							}
						}
					}
				...
				if (value.length < 3 && !alerted) {
					str += (character + '的皮肤“' + skin +
						'”中的属性填写不全，请检查至少包含source、audio、image这3条属性！');
					alert(str);
				}
	...
	if (lib.qhly_viewskin[lib.config.qhly_currentViewSkin] && lib.qhly_viewskin[lib.config.qhly_currentViewSkin]
		...
		if (typeof hasJs == 'string') {
			// @ts-ignore
			await loadScript([lib.qhly_path + "model/diy.js", hasJs]);
		} else if (hasJs) {
			// @ts-ignore
			var vsJsPath = lib.qhly_path + 'theme/' + lib.config.qhly_currentViewSkin + "/code/" + lib.config
				.qhly_currentViewSkin + ".js";
			await loadScript(vsJsPath);
		}
	}
	// @ts-ignore
	...
	// @ts-ignore
	// The modern lobby opens before arena content, so initialize once here.
	window.qhly_inPercontent = true;
	try { window.qhly_extension_package.content(config, window.qhly_extension_package); }
	finally { window.qhly_inPercontent = false; }
};
