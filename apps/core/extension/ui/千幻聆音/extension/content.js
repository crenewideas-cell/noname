...
export let CONTENT = function(config, pack) {
	...
	lib.arenaReady.push(function() {
		...
		// @ts-ignore
		if (window.decadeUI && window.duilib) {
			// @ts-ignore
			...
			if (!lib.config.qhly_mentionDynamic2) {
				...
				if ((viewConfig == 'decade' || viewConfig == 'shousha') && lib.config[
			...
			// @ts-ignore
			if (game.qhly_hasExtension('皮肤切换') && window.skinSwitch && lib.config[window.skinSwitch
					.configKey.useDynamic]) {
				// @ts-ignore
				window.dynamicExt = window.skinSwitch;
				skinSwitch.dynamic.transformDst = function(player, isPrimary, dstInfo, extraParams = {
					isOrigin: false,
					huanfuEffect: null
				}) {
					...
					if (dstInfo.name == null || dstInfo.name === avatar.name) {
						if (dstInfo.action) {
							skinSwitch.postMsgApi.changeAvatarAction(player, isPrimary, dstInfo,
								isOrigin)
						}
						if (dstInfo.skin) {
						...
						if (dstInfo.background) {
							player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
								'extension/十周年UI/assets/dynamic/' + dstInfo.background + '")';
						}
				...
				EpicFX.playDynamic = (player, character, character2, play, transform, cutdybg) => {
					if (!cutdybg) cutdybg = ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena
						.dataset.newDecadeStyle == 'on';
					let res2 = {
						status: false,
						zhu: "default",
						fu: "default"
					...
					if (player.dynamic && !play)
						player.stopDynamic();
					// @ts-ignore
					var showDynamic = (player.dynamic || CUR_DYNAMIC < MAX_DYNAMIC) && duicfg
						.dynamicSkin;
					// @ts-ignore
					if (showDynamic && _status.mode != null) {
						...
						for (var i = 0; i < avatars.length; i++) {
							...
							else if (lib && lib.config && lib.config.qhly_skinset && lib.config
							...
							var editArgument1 = 'dynamic',
								editArgument2 = 'beijing';
							skin.zhu = character == avatars[i];
							// @ts-ignore
							var skinCopy = game.qhly_deepClone(skin);
							let hide = [];
							if (lib.character[avatars[i]] && lib.character[avatars[i]][4]) hide = lib
								.character[avatars[i]][4];
							let isHide;
							if (hide.length > 0 && hide[0] == "hiddenSkill" || get.mode() ==
								'guozhan') {
								isHide = true;
							}
							...
							if (skinCopy.transform) {
								...
								if (skinCopy.transform.skillName) {
									// @ts-ignore
									if (!lib.skill[skinCopy.transform.skillName]) EpicFX
										.setTransformSkill(skinCopy.transform.skill);
									if (player.hp !== undefined) {
										player.addSkill(skinCopy.transform.skillName);
										// @ts-ignore
										if (EpicFX.filterSkills.indexOf(skinCopy.transform.skillName) ==
											-1) {
											// @ts-ignore
										...
										player._inits.push(function(p) {
											...
											// @ts-ignore
											if (EpicFX.filterSkills.indexOf(skinCopy.transform
													.skillName) == -1) {
												// @ts-ignore
												EpicFX.filterSkills.push(skinCopy.transform
													.skillName);
											}
										})
									}
							...
							// @ts-ignore
							if (lib.config['extension_千幻聆音_qhly_editDynamic'] && lib.qhly_skinEdit[game
									.qhly_getRealName(avatars[i])] && lib.qhly_skinEdit[game
									.qhly_getRealName(avatars[i])][editSkin] && lib.qhly_skinEdit[game
									.qhly_getRealName(avatars[i])][editSkin]['player'] && lib
								.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin]['player'][
									editArgument1
								] && lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][editSkin][
									'player'
								][editArgument1][theme]) {
								// @ts-ignore
								var resetDynamic = lib.qhly_skinEdit[game.qhly_getRealName(avatars[i])][
									editSkin
								].player[editArgument1][theme];
							...
							player.playDynamic(skinCopy, i == 1, cutdybg, lib.config[
								'extension_千幻聆音_ignoreClips']);
							...
							if (!skinCopy.dynamicBackground) player.$dynamicWrap.style.backgroundImage =
								`url(${lib.assetURL}extension/十周年UI/assets/dynamic/${skinCopy.background}`;
							if (!increased) {
								increased = true;
								// @ts-ignore
							...
							function getBackPos(me) {
								...
								if (me) {
									...
									return {
										...
										// @ts-ignore
										y: decadeUI.get.bodySize().height - rect.bottom + (rect.height /
											2),
										width: rect.width,
										height: rect.height
									}
					...
					if (res) {
						...
						if (action == "gongji" || action == "jineng") {
							if (me) {
								...
								let pos = getGongJiPos();
								// @ts-ignore
								if (get.itemtype(player) != 'player') sprite.scale = sprite.scale *
									player.offsetHeight * 0.0035;
								sprite.x = pos.x;
								sprite.y = pos.y;
								if (sprite.mirror && pos.isLeft) {
									// @ts-ignore
						...
						EpicFX.player.renderer.postMessage({
							...
							sprite: sprite,
							// @ts-ignore
							skin: EpicFX.player.getSkinState(player.dynamic.id, player.dynamic
								.primary.name)
						});
						if (player.dynamic.renderer.postMessage) player.dynamic.renderer.postMessage({
							message: "HIDE",
								...
								if (player && player.dynamic) {
									...
									if (EpicFX.attackings.length) {
										...
										for (let i = 0; i < EpicFX.attackings.length; i++) {
											// @ts-ignore
											let temp = EpicFX.attackings[i];
											if (temp.player == data.player && temp.zhu == data
												.zhu) {
												// @ts-ignore
				...
				EpicFX.playAction = (player, action) => {
					// @ts-ignore
					let res = EpicFX.canAction(false, player.dynamic.primary, player.dynamic.deputy,
						action);
					if (!res.ok) return;
		...
		// @ts-ignore
		if (window.decadeUI && !lib.config['extension_千幻聆音_qhly_decadeCloseDynamic'] && (lib.config
				.qhly_currentViewSkin == 'decade' || lib.config.qhly_currentViewSkin == 'shousha')) {
						...
						set: function(d) {
							...
							if (newInit.indexOf('playDynamic') == -1) this.qhly_init = d;
								...
								if (confirm(
									...
									game.saveConfig('extension_千幻聆音_qhly_currentViewSkin', lib
										.config.qhly_currentViewSkin);
									game.reload();
								} else {
							...
							set: function(d) {
								...
								if (newInit.indexOf('playDynamic') == -1) this.qhly_init = d;
									...
									if (confirm(
										...
										game.saveConfig('qhly_currentViewSkin', lib.config
											.qhly_currentViewSkin);
										game.saveConfig('extension_千幻聆音_qhly_currentViewSkin',
											lib.config.qhly_currentViewSkin);
										game.reload();
									} else {
										game.saveConfig('qhly_mentionDynamic', true);
									}
	...
	function qhly_init(character, character2, skill) {
		...
		if (window.qhly_newDynamicExt && lib.config['extension_EpicFX_skinEffects']) {
			// @ts-ignore
			let res = EpicFX.playDynamic(this, character, character2, undefined, undefined, ui.arena.dataset
				.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on');
			// @ts-ignore
			...
			if (showDynamic && _status.mode != null) {
				...
				for (var i = 0; i < avatars.length; i++) {
					...
					if (lib && lib.config && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && lib
						.config.extensions && lib.config.extensions.includes('千幻聆音') && lib.config[
							'extension_千幻聆音_enable']) {
						skin = null;
						var namex = avatars[i];
						// @ts-ignore
						var value = game.qhly_getSkin(namex);
					...
					if (!this.doubleAvatar && dynamicExt) {
						...
						if (this == game.me) {
							// @ts-ignore
							if (dynamicExt.selectSkinData && dynamicExt.selectSkinData.value) dynamicExt
					...
					if (lib.config['extension_千幻聆音_qhly_editDynamic'] && lib.qhly_skinEdit[realName] && lib
						...
						.qhly_skinEdit[realName][skinName].player.dynamic && lib.qhly_skinEdit[realName][skinName]
						.player.dynamic[theme]) {
						// @ts-ignore
						...
						skinCopy.angle = resetDynamic.angle;
						// @ts-ignore
						if (skinCopy.beijing && lib.qhly_skinEdit[realName][skinName].player.beijing && lib
							.qhly_skinEdit[realName][skinName].player.beijing[theme]) {
							// @ts-ignore
					...
					// @ts-ignore
					this.playDynamic(skinCopy, i == 1, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena
						.dataset.newDecadeStyle == 'on', lib.config['extension_千幻聆音_ignoreClips']);
					// 修改3 start  此处修改是因为动皮和背景加载需要时间, 在动皮加载好之前用一个默认背景代替, 防止武将框黑一片
					// @ts-ignore
					if (i == 0 || !this.dynamic.primary) {
						if (skinCopy.background) {
							// @ts-ignore
							this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
								'extension/十周年UI/assets/dynamic/' + skinCopy.background + '")';
						} else {
							// @ts-ignore
							this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
		...
		this.node.avatar.addEventListener(lib.config.touchscreen ? "touchend" : "mouseup", function(e) {
			if (!this._qhlyClickTime || Date.now() - this._qhlyClickTime > 400 || this.parentNode.classList
				.contains('selectable') || this.parentNode.classList.contains('target') || this.parentNode
				.isUnseen(0)) return;
		...
		this.node.avatar2.addEventListener(lib.config.touchscreen ? "touchend" : "mouseup", function(e) {
			if (!this.parentNode.doubleAvatar || !this._qhlyClickTime || Date.now() - this._qhlyClickTime >
				400 || this.parentNode.classList.contains('selectable') || this.parentNode.classList
				.contains('target') || this.parentNode.isUnseen(1)) return;
			timer = setTimeout(function() {
				// @ts-ignore
				if (lib.config['extension_千幻聆音_qhly_playerwindow']) game.qhly_playerWindow(this);
			}.bind(this), 220);
	...
	function qhly_reinit(from, to, maxHp, online) {
		...
		if (this.name2 == from) {
			...
			if (window.qhly_newDynamicExt) {
				...
				// @ts-ignore
				data.state = EpicFX.playDynamic(this, undefined, to, true, undefined, ui.arena.dataset
					.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on');
			}
			...
			if (window.qhly_newDynamicExt) {
				...
				// @ts-ignore
				data.state = EpicFX.playDynamic(this, to, undefined, true, undefined, ui.arena.dataset
					.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle == 'on');
			}
		} else {
			return this;
		...
		// @ts-ignore
		if (window.qhly_newDynamicExt && lib.config['extension_EpicFX_skinEffects']) EpicFX
			.initSkinPackOrRefreshAll(this, data);
		if (online) {
			return;
		}
		...
		if (!window.qhly_newDynamicExt) {
			// @ts-ignore
			var skin = game.qhly_getDynamicSkin(null, to),
				skinName;
			// @ts-ignore
			...
			if (this.doubleAvatar) {
				...
				if (skin) {
					// @ts-ignore
					this.playDynamic(skinCopy, deputy, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena
						.dataset.newDecadeStyle == 'on', lib.config['extension_千幻聆音_ignoreClips']);
					// @ts-ignore
					decadeUI.CUR_DYNAMIC++;
				}
				...
				if (skin) {
					// @ts-ignore
					this.playDynamic(skinCopy, false, ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena
						.dataset.newDecadeStyle == 'on', lib.config['extension_千幻聆音_ignoreClips']);
					// @ts-ignore
					decadeUI.CUR_DYNAMIC++;
					// @ts-ignore
					this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
						'extension/十周年UI/assets/dynamic/' + skin.background + '")';
				}
				var forces = 'qun';
	...
	function qhly_playdynamic(animation, deputy, cutdybg, ignoreClip) {
		...
		if (this.doubleAvatar) {
			...
			// @ts-ignore
			if (window.qhly_newDynamicExt && animation.dybg && cutdybg) animation.dybg.clip = {
				x: 0,
				y: 0,
				width: [0, 1],
				height: [0, dybgxishu],
			...
			else if (animation.player && animation.player.beijing && cutdybg) animation.player.beijing.clip = {
				x: 0,
				y: 0,
				width: [0, 1],
				height: [0, dybgxishu],
		...
		if (window.qhly_newDynamicExt && lib.config['extension_EpicFX_skinEffects']) {
			...
			switch (num) {
				...
				case 0:
					...
					// @ts-ignore
				case 1:
					game.qhly_changeDynamicSkin(this, undefined, this.name2, true);
					break;
				default: {
					// @ts-ignore
					game.qhly_changeDynamicSkin(this, undefined, this.name1);
	...
	lib.skill._qhlyChangeSkin = {
		...
		filter: function(event, player) {
			// @ts-ignore
			return lib.qhly_skinChange[game.qhly_getRealName(player.name1)] || lib.qhly_skinChange[game
				.qhly_getRealName(player.name2)];
		},
		direct: true,
		forced: true,
		charlotte: true,
		content: function() {
			for (let i = 0; i < 2; i++) {
				...
				// @ts-ignore
				if (lib.qhly_skinChange[playerName] && lib.qhly_skinChange[playerName][game.qhly_earse_ext(
						skin)] && lib.qhly_skinChange[playerName][game.qhly_earse_ext(skin)].source && lib
					.qhly_skinChange[playerName][game.qhly_earse_ext(skin)].source.indexOf('hp_') == 0) {
					// @ts-ignore
					...
					// @ts-ignore
					if (lib.qhly_skinChange[playerName][game.qhly_earse_ext(skin)].phase && trigger.name ==
						'recover' || trigger.name == 'phase' && player._qhly_gonnaChange[i] != 1) continue;
					// @ts-ignore
					if (trigger.name == 'phase') player._qhly_gonnaChange[i] = 0;
					// @ts-ignore
					game.qhly_checkPlayerImageAudio(playerName, skin, player, function() {
						// @ts-ignore
						player.node['avatar' + (i ? '2' : '')].qhly_origin_setBackgroundImage(player
							._qhly_skinChange[i]);
						// @ts-ignore
						...
						// @ts-ignore
						if (window.decadeUI && !game.qhly_hasExtension('皮肤切换') && !game
							.qhly_hasExtension('EpicFX')) game.qhly_changeDynamicSkin(player,
	...
	lib.skill._qhlyChageKillingSkin = {
		...
		filter: function(event, player) {
			...
			// @ts-ignore
			return lib.qhly_skinChange[game.qhly_getRealName(event.source.name1)] || lib.qhly_skinChange[
				game.qhly_getRealName(event.source.name2)];
		},
		direct: true,
		forced: true,
		charlotte: true,
	...
	lib.skill._qhlyCheckSkin = {
		...
		content: function() {
			...
			if (lib.qhly_skinChange[player.name1] || lib.qhly_skinChange[player.name2]) {
				...
				for (var i = 0; i < num; i++) {
					// @ts-ignore
					game.qhly_setCurrentSkin(game.qhly_getRealName(player['name' + (i + 1)]), game
						.qhly_getSkin(game.qhly_getRealName(player['name' + (i + 1)])));
				}
			}
		}
	}
	...
	game.qhly_changeSkillSkin = function(player, skill) {
		...
		for (let i = 0; i < 2; i++) {
			...
			// @ts-ignore
			if (lib.qhly_skinChange[playerName] && lib.qhly_skinChange[playerName][game.qhly_earse_ext(skin)] &&
				lib.qhly_skinChange[playerName][game.qhly_earse_ext(skin)].source && lib.qhly_skinChange[
					playerName][game.qhly_earse_ext(skin)].source == skill) {
				// @ts-ignore
	...
	game.qhly_getShoushajinjie = function(name) {
		...
		var dengjie = 0,
			current = lib.config.qhly_shoushadengjie[name],
			need = 190000;
		// @ts-ignore
		...
		need -= current;
		// @ts-ignore
		if (current == 0 && !_status.qhly_shoushajinjie[dengjie]) current = _status.qhly_shoushajinjie[dengjie -
			1]
	...
	var qhly_mvp = function(result) {
		...
		var mvpplayers = game.players.concat(game.dead),
			list = [];
	...
	game.qhly_getSkinLevel = function(name, skin, decade, dongtai) {
		...
		switch (lib.config.qhly_level[name + '_' + skin]) {
			case '稀有':
				return decade ? 'xiyou' : '稀有';
			case '史诗':
				return decade ? 'shishi' : '史诗';
			case '精良':
				return decade ? 'xiyou' : '精良';
			case '传说':
				return decade ? 'chuanshuo' : '传说';
			case '精品':
				return decade ? 'xiyou' : '稀有';
			case '绝版':
				return decade ? 'jueban' : '传说';
			case '动态':
				return decade ? 'dongtai' : '传说';
			case '普通':
				return decade ? 'putong' : '稀有';
			case '限定':
				return decade ? 'xianding' : '传说';
			default:
				return decade ? 'dongtai' : '原画';
		}
	}
	// @ts-ignore
	...
	play) { //str主体，如果为字符串是名称中带有str的所有玩家，name指定更换为某种动皮，character玩家姓名id，character2是否为副将
		...
		var nodes;
		var increased;
		if (typeof str == 'object') nodes = [
			[str, character2 ? 'name2' : 'name1']
		];
		// @ts-ignore
		...
		for (var dy = 0; dy < nodes.length; dy++) {
			...
			if (_status.mode != null) {
				...
				if (name) {
					...
					if (lib.config.qhly_skinset.djtoggle && lib.config.qhly_skinset.djtoggle[character] && lib
						.config.qhly_skinset.djtoggle[character][value]) value = null;
					for (var i of Object.keys(skins)) {
				...
				if (nodeType == 'player') {
					...
					if (window.dynamicExt && window.dynamicExt != window.skinSwitch) {
						...
						if (window.dynamicExt == window.eng) {
							if (lib.config['extension_EngEX_dynamicSkin']) {
								...
								else if (lib.config['extension_EngEX_dynamicSkin'][character]) delete lib
									.config['extension_EngEX_dynamicSkin'][node];
								game.saveConfig('extension_EngEX_dynamicSkin', lib.config[
									'extension_EngEX_dynamicSkin']);
							} else {
								if (skin) {
					...
					if (skin && node.campBack && name) {
						// @ts-ignore
						if (!['史诗', '传说'].includes(game.qhly_getSkinLevel(character, name))) node.campBack
							.setAttribute("data-pinzhi", '史诗');
						// @ts-ignore
				...
				if (game.qhly_getPlayerStatus(node, character2, character) == 2) {
					if (skin.special && skin.special.condition && (skin.special.condition.lowhp || skin.special
							.condition.juexingji)) {
						var value;
						...
						if (skin.special[special]) value = skin.special[special].name ? skin.special[special]
							.name : skin.name.slice(0, skin.name.lastIndexOf('/'));
						value = value.split('/');
				...
				// 动态背景切割 4 -> 判断提前
				if (nodeType == 'player') {
					cutdybg = ui.arena.dataset.dynamicSkinOutcrop == 'on' && ui.arena.dataset.newDecadeStyle ==
						'on';
				} else {
					cutdybg = lib.config.qhly_lutou && node.className.indexOf('decade') < 0 && node.className
						.indexOf('qh-image') < 0;
				}
				...
				var themeType = ui.arena.dataset.newDecadeStyle == 'on' ? 'decade' : 'decade';
				...
				// @ts-ignore
				if (lib.config['extension_千幻聆音_qhly_editDynamic'] && lib.qhly_skinEdit[game.qhly_getRealName(
						character)] && lib.qhly_skinEdit[game.qhly_getRealName(character)][editSkin] && lib
					...
					skinCopy.angle = resetDynamic.angle;
					// @ts-ignore
					if ((skinCopy.beijing || skinCopy.dynamicBackground) && lib.qhly_skinEdit[game
							.qhly_getRealName(character)][editSkin][nodeType][editArgument2] && lib
						.qhly_skinEdit[game.qhly_getRealName(character)][editSkin][nodeType][editArgument2][
							theme
						]) {
						// @ts-ignore
						var resetBeijing = lib.qhly_skinEdit[game.qhly_getRealName(character)][editSkin][
							nodeType
						][editArgument2][theme];
						if (skinCopy.dynamicBackground) {
							if (typeof skinCopy.dynamicBackground === 'string') {
					...
					if (skinCopy.beijing) skinCopy.beijing.scale = (skinCopy.beijing.scale || 1) * skinCopy
						.qhly_resizeRatio;
					if (skinCopy.dynamicBackground) {
						if (typeof skinCopy.dynamicBackground === 'string') {
							skinCopy.dybg = {
							...
							skinCopy.dybg.loop = true;
							skinCopy.dybg.zhu = skinCopy.zhu;
							skinCopy.dybg.scale = (skinCopy.dybg.scale || skinCopy.scale) * skinCopy
								.qhly_resizeRatio;
						}
					}
				}
				...
				if (window.qhly_newDynamicExt) {
					...
					node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
						'extension/十周年UI/assets/dynamic/' + skinCopy.background + '")';
					// @ts-ignore
					game.qhly_checkYH(node);
					// player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/十周年UI/assets/dynamic/' + skinCopy.background + '")';
					if (!skinCopy.dynamicBackground) {}
					node.$dynamicWrap.style.backgroundImage =
						`url(${lib.assetURL}extension/十周年UI/assets/dynamic/${skinCopy.background}`;
					if (!increased) {
						increased = true;
						// @ts-ignore
					...
					if (skinCopy.background) {
						if (node.doubleAvatar && ui.arena.dataset.newDecadeStyle == 'on') {
							...
							if (character2) {
								background1 = node.dynamic.primary && node.dynamic.primary != null && node
									.dynamic.primary.background ? (lib.assetURL +
										'extension/十周年UI/assets/dynamic/' + node.dynamic.primary.background) :
									'';
								background2 = lib.assetURL + 'extension/十周年UI/assets/dynamic/' + skinCopy
									.background;
							} else {
								background1 = lib.assetURL + 'extension/十周年UI/assets/dynamic/' + skinCopy
									.background;
								background2 = node.dynamic.deputy && node.dynamic.deputy != null && node.dynamic
									.deputy.background ? (lib.assetURL + 'extension/十周年UI/assets/dynamic/' +
										node.dynamic.deputy.background) : '';
							}
							node.$dynamicWrap.style.backgroundImage = 'url("' + background1 + '"),url("' +
								background2 + '")';
							node.$dynamicWrap.style['background-size'] = '50% 100%';
							node.$dynamicWrap.style['background-position'] = 'left, right';
						} else node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
							'extension/十周年UI/assets/dynamic/' + skinCopy.background + '")';
					} else {
						node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
							'extension/皮肤切换/images/card/card.png")'
	...
	game.qhly_checkPlayerImageAudio = function(name, skin, player, callbacka, callbackb, force) {
		...
		// @ts-ignore
		var pkgPath = (pkg.isExt && game.qhly_getRealName(name) != name && skin) ? DEFAULT_PACKAGE.skin
			.standard : pkg.skin.standard;
		// @ts-ignore
		if (lib.qhly_skinChange[name] && lib.qhly_skinChange[name][skinName] && lib.qhly_skinChange[name][
				skinName
			].source) {
			...
			if (lib.qhly_skinChange[name][skinName].source.indexOf('hp_') == 0) {
				...
				if (player.hp <= hp) {
					...
					if (game.qhly_getPlayerStatus(player, avatar2) != 2) {
						imageChanged = true;
						// @ts-ignore
						player._qhly_skinChange[avatar2 ? 1 : 0] = pkgPath + lib.qhly_skinChange[game
							.qhly_getRealName(name)][skinName].image1;
					}
					// @ts-ignore
					if (!window.qhly_audio_redirect[name + '-' + skinName] || window.qhly_audio_redirect[name +
							'-' + skinName] != lib.qhly_skinChange[name][skinName].audio1) {
						audioChanged = true;
						// @ts-ignore
						window.qhly_audio_redirect[name + '-' + skinName] = lib.qhly_skinChange[name][skinName]
							.audio1;
					}
					...
					if (player._qhly_gonnaChange && player._qhly_gonnaChange[avatar2 ? 1 : 0]) {
						...
						// @ts-ignore
						if (!window.qhly_audio_redirect[name + '-' + skinName] || window.qhly_audio_redirect[
								name + '-' + skinName] != lib.qhly_skinChange[name][skinName].audio1) {
							audioChanged = true;
							// @ts-ignore
							window.qhly_audio_redirect[name + '-' + skinName] = lib.qhly_skinChange[name][
								skinName
							].audio1;
						}
					} else {
						...
						if (game.qhly_getPlayerStatus(player, avatar2) != 1) {
							...
							// @ts-ignore
							player._qhly_skinChange[avatar2 ? 1 : 0] = pkgPath + lib.qhly_skinChange[game
								.qhly_getRealName(name)][skinName].image2;
						}
						// @ts-ignore
				...
				if (game.qhly_getPlayerStatus(player, avatar2) == 1) {
					// @ts-ignore
					if (!player._qhly_skinChange[avatar2 ? 1 : 0] || player._qhly_skinChange[avatar2 ? 1 : 0] !=
						pkgPath + lib.qhly_skinChange[game.qhly_getRealName(name)][skinName].image2) {
						imageChanged = true;
						// @ts-ignore
						player._qhly_skinChange[avatar2 ? 1 : 0] = pkgPath + lib.qhly_skinChange[game
							.qhly_getRealName(name)][skinName].image2;
					...
					// @ts-ignore
					if (!player._qhly_skinChange[avatar2 ? 1 : 0] || player._qhly_skinChange[avatar2 ? 1 : 0] !=
						pkgPath + lib.qhly_skinChange[game.qhly_getRealName(name)][skinName].image1) {
						imageChanged = true;
						// @ts-ignore
						player._qhly_skinChange[avatar2 ? 1 : 0] = pkgPath + lib.qhly_skinChange[game
							.qhly_getRealName(name)][skinName].image1;
	...
	game.qhly_getPlayerStatus = function(player, avatar2, name) {
		...
		// @ts-ignore
		else if (!name && get.itemtype(player) == 'player') name = game.qhly_getRealName(player['name' + (
			avatar2 ? '2' : '1')]);
		...
		if (player) {
			...
			// @ts-ignore
			if (lib.qhly_skinChange[name] && lib.qhly_skinChange[name][game.qhly_earse_ext(game.qhly_getSkin(
					name))]) {
				if (bg) {
					for (var s of support) {
						...
						if (index > 0) {
							...
							if (frIndex > 0) {
								...
								// @ts-ignore
								if (lib.qhly_skinChange[name][game.qhly_earse_ext(game.qhly_getSkin(name))]
									.image1 && str == lib.qhly_skinChange[name][game.qhly_earse_ext(game
										.qhly_getSkin(name))].image1) num = 2;
							}
						}
					}
				}
		...
		if (this.doubleAvatar) {
			...
			if (avatar2) { //副将
				...
				decadeUI.animation.playSpine({
					name: window.qhlyUI.assets[huanfuType].name,
					action: str
				}, {
					scale: currentScale,
					y: [0, 0.5],
				...
				// @ts-ignore
				decadeUI.animation.playSpine({
					name: window.qhlyUI.assets.pinzhi.name,
					action: 'play' + act
				}, {
					scale: 0.3,
					x: [0, 0.75],
			...
			// @ts-ignore
			if (huanfuType == 'decade') decadeUI.animation.playSpine({
				name: window.qhlyUI.assets.pinzhi.name,
				action: 'play' + act
			}, {
				scale: 0.5,
	...
	game.qhly_checkYH = function(player, group) {
		...
		if (lib.config['extension_十周年UI_newDecadeStyle'] == "osn") return;
		// @ts-ignore
	...
	game.playShoushaAvatar = function(node, flip, name) {
		...
		// @ts-ignore
		if (!mainPlayer || !node.dynamic || !node.dynamic.primary || node.dynamic.primary.name != _status
			.currentTexiao || _status.bigEditing) {
			// @ts-ignore
				...
				if (res) {
					...
					renderer.onmessage = function(e) {
						if (e.data) {
							...
							renderer.onmessage = function(e) {
								if (e.data) {
									game.playAudio("..", "extension", "EngEX/audio/effect", res.dynamic
										.name + ".mp3");
									// @ts-ignore
									// @ts-ignore
									// @ts-ignore
					...
					var tempX = node.dynamic.primary.x,
						chukuangX = tempX;
					var tempY = node.dynamic.primary.y,
						chukuangY = tempY;
					var tempA = node.dynamic.primary.angle || 0,
						chukuangA = tempA;
					var tempS = node.dynamic.primary.scale || 1,
						chukuangS = tempS;
					// @ts-ignore
					...
					// @ts-ignore
					if (lib.qhly_skinEdit[realName] && lib.qhly_skinEdit[realName][skin] && lib.qhly_skinEdit[
							realName][skin].bigAvatar && lib.qhly_skinEdit[realName][skin].bigAvatar.chukuang &&
						lib.qhly_skinEdit[realName][skin].bigAvatar.chukuang[theme]) {
						// @ts-ignore
			...
			if (!document.getElementById('qhlySkinChangebutton1')) {
				...
				for (let i = 0; i < 2; i++) {
					...
					buttons[i].listen(function() {
						...
						// @ts-ignore
						window.qhly_audio_redirect[game.qhly_getRealName(name) + '-' + skinName] = lib
							.qhly_skinChange[game.qhly_getRealName(name)][skinName]['audio' + index];
						// @ts-ignore
						game.qhly_setCurrentSkin(name, skin, function() {
							// @ts-ignore
							game.qhly_setOriginSkin(name, skin, state.mainView.avatarImage,
								state, this.id.slice(20) == '1');
						}.bind(this))
						// @ts-ignore
						if (this.id == 'qhlySkinChangebutton1') game.qhly_setPlayerStatus(state.mainView
							.avatarImage, undefined, 2);
						// @ts-ignore
						else game.qhly_setPlayerStatus(state.mainView.avatarImage, undefined, 1);
						if (lib.config.qhly_currentViewSkin == 'shousha') state.mainView.page.skin
							.refresh(name, state);
						// @ts-ignore
						if (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[
								name] && !lib.config.qhly_skinset.djtoggle[name][skinName]) game
							.qhly_changeDynamicSkin(state.mainView.avatarImage, skinName, name);
						// @ts-ignore
						if (state.mainView.avatarImage.dynamic && state.mainView.avatarImage.dynamic
							.primary != null) _status.currentTexiao = state.mainView.avatarImage.dynamic
							.primary.name;
					})
				}
				// @ts-ignore
				...
				if (button && button.classList.contains('sel')) {
					// @ts-ignore
					window.qhly_audio_redirect[game.qhly_getRealName(name) + '-' + skinName] = lib
						.qhly_skinChange[game.qhly_getRealName(name)][skinName].audio1;
				} else {
					// @ts-ignore
	...
	// @ts-ignore
	if (!String.prototype.replaceAll) String.prototype.replaceAll = function(s1, s2) {
	...
	window.qhly_openPluginWindow = function() {
		...
		for (var plugin of plugins) {
			(function(plugin) {
				...
				ph += ("<br><img id='qhly_pluginwindow_plugin_" + count + "'/>" +
					"<span id='qhly_pluginwindow_plugin_text_" + count +
					"' style='bottom:10px;'>插件启用</span>");
		...
		for (var num in map) {
			(function(num) {
				...
				// @ts-ignore
				var isOpen = lib.config.qhly_disabledPlugins ? (!lib.config.qhly_disabledPlugins.includes(
					game.qhly_getPluginId(plugin))) : true;
				// @ts-ignore
	...
	game.qhly_setOriginSkin = function(name, skin, node, state, skin2) {
		...
		var realName = game.qhly_getRealName(name);
		// @ts-ignore
		var originFrom = (state.pkg.isExt && realName != name && skin) ? DEFAULT_PACKAGE.skin.standard : state
			.pkg.skin.standard;
		// @ts-ignore
		var originFrom1 = (state.pkg.isExt && realName != name && skin) ? DEFAULT_PACKAGE.skin.origin : state
			.pkg.skin.origin;
		var originFrom2;
		var position = '50%';
		// @ts-ignore
		...
		if (skin) {
			if (skin2 && setTrue) {
				originFrom2 = realName + '/' + skin.substr(0, skin.length - 4) + '/' + skin.substr(0, skin
					.length - 4) + '2';
			} else {
		...
		image.onload = function() {
			...
			// @ts-ignore
			if (lib.qhly_skinEdit[realName] && lib.qhly_skinEdit[realName][skin] && lib.qhly_skinEdit[
					realName][skin][positionArgument]) {
				// @ts-ignore
		...
		image.onerror = function() {
			...
			if (skin2 && setTrue) {
				// @ts-ignore
				node.qhly_origin_setBackgroundImage(originFrom + lib.qhly_skinChange[realName][game
					.qhly_earse_ext(skin)
				].image1);
				node.style.backgroundSize = 'cover';
	...
	game.qhly_formatDS = function(obj, namex) {
		...
		var bianshenFlag = false,
			texiaoFlag = false;
		for (var key of keys) {
			if (key == 'dynamicBackground') obj.beijing = {
				name: obj.dynamicBackground
			};
			...
			if (key == 'transform' && (obj.transform.low && obj.transform.high || obj.transform.juexingji) && !
				bianshenFlag) {
				bianshenFlag = true;
	...
	game.qhly_setDoubleGroup = function(state) {
		...
		if (lib.config.qhly_doubleGroup && lib.config.qhly_doubleGroup[state.name]) {
			if (lib.config.doubleGroupCharacter && lib.config.doubleGroupCharacter.includes(state.name) || get
				.is.double(state.name)) {
				dg = true;
		...
		switch (group) {
			...
			case 'ye':
				...
				break;
			default:
				if (state.pkg.ssborder) path = state.pkg.ssborder;
		...
		if (path) {
			...
			if (border) {
				url1 =
					`url('${state.pkg.ssborder || 'shousha/border2/'}${group}_zi.png'),url('${state.pkg.ssborder || 'shousha/border2/'}${border}_frame.png')`;
			} else {
	...
	game.qhly_refreshSShp = function(state) {
		...
		switch (group) {
			...
			case 'ye':
				...
				break;
			default:
				hpBorder = state.pkg.sshpBorder && state.pkg.sshpBorder[group];
		}
		var HP = 0,
			MAXHP = 0,
			hpNode; //血量 空血量
		...
		if (state.mainView.skinTypeGuozhan) { //显示国战血量
			...
			if (HP <= 5) {
				...
				for (var i = 0; i < HP.length; i++) {
					...
					// @ts-ignore
					hpNumber.style.backgroundImage = 'url(' + lib.qhly_path + 'theme/shousha/hp_num_' + HP
						.substr(i, 1) + '.png)';
					// @ts-ignore
					if (HP == 'I') {
			...
			if (HP > 5 || MAXHP > 5) {
				...
				if (MAXHP) {
					...
					for (var i = 0; i < MAXHP.length; i++) {
						var hpNumber = ui.create.div('.qh-hpnumber', hpNode);
						// @ts-ignore
						hpNumber.style.backgroundImage = 'url(' + lib.qhly_path + 'theme/shousha/hp_num_' +
							MAXHP.substr(i, 1) + '.png)';
				...
				if (!MAXHP) {
					...
					hpNode.style.cssText =
						`width:1.5em;height:1.5em;margin-bottom:0.4em;background-image:url(${lib.qhly_path}theme/shousha/hp_num_x.png)`
				}
				hpNode = ui.create.div('.qh-hptext', state.mainView.hp);
				if (hpBorder) {
	...
	if (ui && ui.css && ui.css.fontsheet && ui.css.fontsheet.sheet && ui.css.fontsheet.sheet.insertRule) {
		// @ts-ignore
		ui.css.fontsheet.sheet.insertRule("@font-face {font-family: 'qh_heiti';src: url('" + lib.qhly_path +
			"font/heiti.woff2');}", 0);
		// @ts-ignore
		ui.css.fontsheet.sheet.insertRule("@font-face {font-family: 'qh_zhunyuan';src: url('" + lib.qhly_path +
			"font/zhunyuan.woff2');}", 0);
		// @ts-ignore
		ui.css.fontsheet.sheet.insertRule("@font-face {font-family: 'qh_youyuan';src: url('" + lib.qhly_path +
			"font/youyuan.woff2');}", 0);
		// @ts-ignore
		ui.css.fontsheet.sheet.insertRule("@font-face {font-family: 'qh_songhei';src: url('" + lib.qhly_path +
			"font/songhei.woff2');}", 0);
		// @ts-ignore
		ui.css.fontsheet.sheet.insertRule("@font-face {font-family: 'qh_weili';src: url('" + lib.qhly_path +
			"font/weili.woff2');}", 0);
	} else {
		// @ts-ignore
		if (!ui.qhlycss) ui.qhlycss = lib.init.sheet();
		// @ts-ignore
		ui.qhlycss.sheet.insertRule("@font-face {font-family: 'qh_heiti';src: url('" + lib.qhly_path +
			"font/heiti.woff2');}", 0);
		// @ts-ignore
		ui.qhlycss.sheet.insertRule("@font-face {font-family: 'qh_zhunyuan';src: url('" + lib.qhly_path +
			"font/zhunyuan.woff2');}", 0);
		// @ts-ignore
		ui.qhlycss.sheet.insertRule("@font-face {font-family: 'qh_youyuan';src: url('" + lib.qhly_path +
			"font/youyuan.woff2');}", 0);
		// @ts-ignore
		ui.qhlycss.sheet.insertRule("@font-face {font-family: 'qh_songhei';src: url('" + lib.qhly_path +
			"font/songhei.woff2');}", 0);
		// @ts-ignore
		ui.qhlycss.sheet.insertRule("@font-face {font-family: 'qh_weili';src: url('" + lib.qhly_path +
			"font/weili.woff2');}", 0);
	}
	// Current core stores the selected portrait; retain its switch and save.
	...
	if (get.mode() == 'guozhan' && game.getFileList && lib.config.qhly_guozhan !== false) {
		var cpath = lib.config.qhly_originSkinPath == 'extension/千幻聆音/sanguolutouskin/' ?
			'extension/千幻聆音/sanguolutouskin' : 'extension/千幻聆音/sanguoskin';
		// @ts-ignore
	...
	game.qhly_chooseDialog = function(title, detail, initValue, list, onok, onclose) {
		...
		belowButton += '<img src="' + lib.qhly_path + 'image/qhly_ok2.png" id="qhly_choose_okbutton' + id +
			'"/>&nbsp;&nbsp;&nbsp;&nbsp;';
		// @ts-ignore
		belowButton += '<img src="' + lib.qhly_path + 'image/qhly_cancel2.png" id="qhly_choose_cancelbutton' +
			id + '"/>';
	...
	game.qhly_editDialog = function(title, detail, initValue, onok, onclose) {
		...
		belowButton += '<img src="' + lib.qhly_path + 'image/qhly_ok2.png" id="qhly_edit_okbutton' + id +
			'"/>&nbsp;&nbsp;&nbsp;&nbsp;';
		// @ts-ignore
		belowButton += '<img src="' + lib.qhly_path + 'image/qhly_cancel2.png" id="qhly_edit_cancelbutton' +
			id + '"/>';
		content.innerHTML = text;
		below.innerHTML = belowButton;
		document.body.appendChild(dialog);
	...
	game.qhly_parseConfig = function(obj) {
		...
		if (['checkboxList', '复选框'].includes(obj.type)) {
			...
			for (var item of items) {
				...
				checkboxRef[id] = item;
				str += "<p><span style='display:inline-block;height:30px;'><img id='" + id + "'/><span id='" +
					id + "_text' style='display:inline-block;position:relative;bottom:25%;'>";
	...
	window.DEFAULT_PACKAGE = {
		...
		skin: {
			standard: (lib.config.qhly_originSkinPath ? lib.config.qhly_originSkinPath :
				'extension/千幻聆音/sanguoskin/'), //皮肤图片在千幻聆音扩展内的位置。
			origin: 'extension/千幻聆音/sanguoyuanhua/', //皮肤原画在千幻聆音扩展内的位置。
		},
		//ssborder: 'shousha/',//手杀边框
		audioOrigin: '',
		audio: resourceRoot + 'sanguoaudio/', //皮肤配音文件在千幻聆音扩展内的位置。
	};
	...
	game.qhly_bigEdit = function(state, bg) {
		...
		if (editDynamic) {
			...
			var dynamicX = [...temp.daiji.x],
				dynamicY = [...temp.daiji.y],
				dynamicS = temp.daiji.scale,
				dynamicA = temp.daiji.angle;
			if (focus.dynamic.primary.beijing) {
				...
				var beijingX = [...temp.beijing.x],
					beijingY = [...temp.beijing.y],
					beijingS = temp.beijing.scale,
					beijingA = temp.beijing.angle;
			...
			if (focus.dynamic.primary.gongji) {
				...
				var chukuangX = [...temp.chukuang.x],
					chukuangY = [...temp.chukuang.y],
					chukuangS = temp.chukuang.scale,
					chukuangA = temp.chukuang.angle;
			}
		}
		...
		for (var i = 0; i < 8; i++) {
			...
			buttons[i].addEventListener(lib.config.touchscreen ? 'touchstart' : 'mousedown', function() {
				...
				switch (this.id) {
					...
					case 'qhly_bigedit4': {
						if (!focus.dynamic.primary.beijing && !focus.dynamic.primary
							.dynamicBackground) {
							alert('无可调节的动态背景');
							return;
						}
					...
					case 'qhly_bigedit6': {
						if (editDynamic) {
							...
							// @ts-ignore
							if (!lib.qhly_skinEdit[name][skin][editObject][editArgument1][theme]) lib
								.qhly_skinEdit[name][skin][editObject][editArgument1][theme] = {};
							// @ts-ignore
							lib.qhly_skinEdit[name][skin][editObject][editArgument1][theme].x = temp
								.daiji.x;
							// @ts-ignore
							lib.qhly_skinEdit[name][skin][editObject][editArgument1][theme].y = temp
								.daiji.y;
							// @ts-ignore
							lib.qhly_skinEdit[name][skin][editObject][editArgument1][theme].scale = temp
								.daiji.scale / temp.qhly_resizeRatio;
							// @ts-ignore
							lib.qhly_skinEdit[name][skin][editObject][editArgument1][theme].angle = temp
								.daiji.angle;
							...
							if (focus.dynamic.primary.beijing || focus.dynamic.primary.dybg) {
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin][editObject][editArgument2]) lib
									.qhly_skinEdit[name][skin][editObject][editArgument2] = {};
								// @ts-ignore
								...
								lib.qhly_skinEdit[name][skin][editObject][editArgument2][theme].x = temp
									.beijing.x;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin][editObject][editArgument2][theme].y = temp
									.beijing.y;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin][editObject][editArgument2][theme].scale =
									temp.beijing.scale / temp.qhly_resizeRatio;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin][editObject][editArgument2][theme].angle =
									temp.beijing.angle;
							...
							if (focus.dynamic.primary.gongji && editObject != 'player') {
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin].bigAvatar) lib.qhly_skinEdit[name][
									skin
								].bigAvatar = {};
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin].bigAvatar.chukuang) lib
									.qhly_skinEdit[name][skin].bigAvatar.chukuang = {};
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme]) lib
									.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme] = {};
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].x = temp
									.chukuang.x;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].y = temp
									.chukuang.y;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].scale = temp
									.chukuang.scale;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].angle = temp
									.chukuang.angle;
						...
						// @ts-ignore
						game.qhly_readFileAsText("extension/千幻聆音/data/skineditmodel.txt", function(ret,
							str) {
							if (ret) {
								str = str.replace("_REPLACE_OBJECT_", strobj);
								// @ts-ignore
								game.qhly_writeTextFile(str, "extension/千幻聆音", "skinEdit.js",
									function(err) {
										if (!err) {
											console.log("保存成功");
										} else {
					...
					case 'qhly_bigedit7': {
						if (editDynamic) {
							...
							// @ts-ignore
							if (!lib.qhly_skinEdit[name][skin][editObject].dynamic[theme]) lib
								.qhly_skinEdit[name][skin][editObject].dynamic[theme] = {};
							// @ts-ignore
							...
							if (focus.dynamic.primary.beijing || focus.dynamic.primary.dybg) {
								...
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin][editObject].beijing) lib
									.qhly_skinEdit[name][skin][editObject].beijing = {};
								// @ts-ignore
								if (!lib.qhly_skinEdit[name][skin][editObject].beijing[theme]) lib
									.qhly_skinEdit[name][skin][editObject].beijing[theme] = {};
								// @ts-ignore
							...
							if (focus.dynamic.primary.gongji) {
								...
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].scale =
									chukuangS;
								// @ts-ignore
								lib.qhly_skinEdit[name][skin].bigAvatar.chukuang[theme].angle =
									chukuangA;
			...
			if (event) {
				if (event.touches && event.touches.length) {
					if (event.touches.length == 2) {
						focus.mousegesture(event.touches[0].clientX, event.touches[0].clientY, event.touches[1]
							.clientX, event.touches[1].clientY);
		...
		function mousedownEvent(event) {
			if (event) {
				if (event.touches && event.touches.length) focus.mousedown(event.touches[0].clientX, event
					.touches[0].clientY);
	...
	lib.extensionMenu['extension_千幻聆音']['qhly_currentViewSkin'] = {
		...
		"init": lib.config.qhly_currentViewSkin === undefined ? 'xuanwujianghu' : lib.config
			.qhly_currentViewSkin,
	...
	lib.extensionMenu['extension_千幻聆音']['qhly_modemusicconfig'] = {
		"name": "模式BGM",
		"intro": "设置当前模式的BGM。",
		"init": lib.config['qhly_modemusicconfig_' + get.mode()] ? lib.config['qhly_modemusicconfig_' + get
			.mode()] : 'system',
		// @ts-ignore
		"item": lib.qhly_bgmConfigs,
		onclick: function(item) {
	...
	// @ts-ignore
	if (lib.qhly_viewskin[lib.config.qhly_currentViewSkin] && lib.qhly_viewskin[lib.config.qhly_currentViewSkin]
		.onchange) {
		// @ts-ignore
	...
	game.qhly_playAudioPlus = function(...args) {
		...
		if (lib.config.qhly_volumnAudio) {
			var num = lib.config.qhly_volumnAudio['audio' + path];
			if (num !== undefined) {
				num = parseInt(num);
				if (!isNaN(num)) {
					audio.volume = num / 8;
		...
		audio.onended = ev => {
			...
			if (_status.qh_volmode) {
				// @ts-ignore
				game.qhly_openVolumnDialog('audio' + path);
				// @ts-ignore
		...
		Promise.resolve().then(async () => {
			let resolvedPath;
			if (parsedPath.startsWith('db:')) resolvedPath = get.objectURL(await game.getDB('image',
				parsedPath.slice(3)));
	...
	game.playAudio = function(...args) {
		...
		if (originPath.length) {
			...
			if (lib.config.qhly_notbb && lib.config.qhly_notbb != 'none' && !_status.qhly_previewAudio) {
				...
				// @ts-ignore
				while (keySkill.length && keySkill[keySkill.length - 1].charCodeAt() >= '0'.charCodeAt() &&
					keySkill[keySkill.length - 1].charCodeAt() <= '9'.charCodeAt()) {
	...
	HTMLDivElement.prototype.setBackgroundImage = function(name) {
		...
		if (this.classList.contains('qh-must-replace') || (!this.classList.contains('qh-not-replace') && (lib
				.config.qhly_forceall || (this.classList.contains('avatar') || this.classList.contains(
					'avatar2'))))) {
			//判断当前的div是否是人物avatar。
				...
				if (lib.config.qhly_skinset.skin[cname]) {
					...
					if (opath) {
						...
						if (skinPackage.isLutou) {
							// @ts-ignore
							if (lutouPrefix + cname != game.qhly_earse_ext(opath) && prefix + cname != game
								.qhly_earse_ext(opath)) {
								return false;
							}
							// @ts-ignore
	...
	HTMLDivElement.prototype.setBackground = function(name, type, ext, subfolder) {
		...
		if (type == 'character' && (this.classList.contains('qh-must-replace') || (!this.classList.contains(
				'qh-not-replace') && (lib.config.qhly_forceall || (this.classList.contains('avatar') ||
				this.classList.contains('avatar2') || this.classList.contains('primary-avatar') ||
				this.classList.contains('deputy-avatar') || this.classList.contains('button')))))) {
			let that = this;
				...
				if (lib.config.qhly_skinset.skin[cname]) {
					...
					game.qhly_checkFileExist(destpath, function(s) {
						if (s) {
							...
							// @ts-ignore
							if (lib.config.qhly_noSkin == 'origin') that
								.qhly_origin_setBackgroundImage(prefix + realName + '.jpg'); //原画
							// @ts-ignore
							else that.qhly_origin_setBackgroundImage(
	...
	game.qhly_getSkinFile = function(name, skin) {
		...
		if (skinPackage.replaceAvatarDestination) {
			...
			if (r) return r;
			// @ts-ignore
		} else if (lib.qhly_skinChange[realName] && _status.qhly_replaceSkin && _status.qhly_replaceSkin[
				realName] && _status.qhly_replaceSkin[realName][skin]) {
			// @ts-ignore
				...
				if (lib.characterPack['mode_' + mode] && lib.characterPack['mode_' + mode][name]) {
					if (mode == 'guozhan') {
						...
						if (name.indexOf('gz_shibing') == 0) {
							...
							if (lib.config.mode_config.guozhan.guozhanSkin && lib.character[name] && lib
								.character[name][4].includes('gzskin')) gzbool = true;
							name = name.slice(3);
						}
					} else {
						modeimage = mode;
	...
	game.qhly_foundPackageExt = function(name) {
		...
		if (cp) {
			...
			return {
				...
				skin: {
					standard: ((lib.config.qhly_extSkinPath == undefined || lib.config.qhly_extSkinPath ==
							'default') ? 'extension/' + extNameInPic + '/skin/image/' : lib.config
						.qhly_extSkinPath),
		...
		game.qhly_getSkinList(name, function(ret, list) {
			...
			if (window.decadeUI) {
				...
				if (dynamicSkinList.length) {
					...
					for (var i of skinList) {
						// @ts-ignore
						if (i.skinId && i.skinId != null) duibiList.push(i.skinId.substring(0, i.skinId
							.lastIndexOf('.')));
					}
					// @ts-ignore
	...
	game.qhly_writeImageFile = function(str, path, filename, callback) {
		if (lib.device) {
			...
			reader.onload = () => {
				...
				game.ensureDirectory(path, function() {
					// @ts-ignore
					lib.node.fs.writeFile(__dirname + '/' + path + '/' + filename, Buffer.from(arr),
						null, callback);
				});
			}
		}
	...
	game.qhly_dom2image = function(from, name, node, path, state) {
		...
		text.innerHTML +=
			'长按动皮可进行调整\t\t<button id=qh-d2icreate>生成</button>\t\t<button id=qh-d2icancel>取消</button>';
		...
		btn1.onclick = function() {
			...
			window.qhly_d2i.toBlob(bg4).then(function(url) {
				...
				game.qhly_writeImageFile(url, path + realName + '/', fileName + '.jpg', function() {
					...
					parent.dynamicToggle && parent.dynamicToggle.setAttribute('toggle',
						true);
					if (state) {
						if (state.pkg.isExt) {
							// @ts-ignore
							if (realName != name && fileName != '经典形象') path =
								DEFAULT_PACKAGE.skin.standard;
						}
						if (state.mainView.dynamicToggle) state.mainView.dynamicToggle
							.setAttribute('toggle', true);
						// @ts-ignore
						if (game.qhly_skinIs(name, fileName + '.jpg')) game
							.qhly_setOriginSkin(name, fileName + '.jpg', state.mainView
								.avatarImage, state, game.qhly_getPlayerStatus(state
									.mainView.avatarImage, null, state.name) == 2);
					}
					// @ts-ignore
					if (from && game.qhly_skinIs(name, fileName + '.jpg')) from
						.qhly_origin_setBackgroundImage(file);
					// @ts-ignore
					if (_status.qhly_skinListCache && _status.qhly_skinListCache[name])
						delete _status.qhly_skinListCache[name];
					exit();
				})
			});
		}
	...
	game.qhly_getViewSkills = function(name) {
		...
		for (var skill of skills) {
			...
			if (!info || info.nopop || !get.translation(skill + '_info') || !skillInfo(skill)) continue;
			viewSkills.add(skill);
			if (info.derivation) {
				if (typeof info.derivation === 'string' && skillInfo(info.derivation)) viewSkills.add(info
					.derivation);
				else {
					for (var s of info.derivation) {
						if (skillInfo(s)) viewSkills.add(s);
					}
				}
			}
		}
	...
	game.qhly_setCurrentSkin = function(name, skin, callback, save) {
		...
		if (skin) {
			if (game.getFileList) {
				let path;
				// @ts-ignore
				if (window.qhly_audio_redirect && window.qhly_audio_redirect[realName + "-" + game
						.qhly_earse_ext(skin)]) {
					// @ts-ignore
					path = skinPackage.audio + realName + "/" + window.qhly_audio_redirect[realName + "-" + game
						.qhly_earse_ext(skin)];
				} else {
					...
					if (success) {
						...
						game.getFileList(path, function(folders, files) {
							...
							if (skinPackage2.isExt) {
								...
								delete lib.config.qhly_skinset.audioReplace["../" +
									extAudioPath + 'victory'];
								delete lib.config.qhly_skinset.audioReplace["../" + skinPackage2
									.audio + name + '/victory'];
							} else {
							...
							for (let file of files) {
								...
								if (!skinPackage.isExt || realName != name) {
									if (file == realName) {
										arr.push(diePath);
										if (skinPackage2.isExt) lib.config.qhly_skinset
											.audioReplace[diePath] = "../" + path + "/" +
											realName;
										else lib.config.qhly_skinset.audioReplace[diePath] =
											"../" + path + "/" + realName;
									} else if (file == 'victory') {
										arr.push("victory/" + name);
										if (skinPackage2.isExt) lib.config.qhly_skinset
											.audioReplace["../" + path + '/victory'] = "../" +
											path + '/victory';
										else lib.config.qhly_skinset.audioReplace["victory/" +
											name] = "../" + path + "/victory";
									} else {
										let skills = [],
											file2 = '',
										...
										// @ts-ignore
										if (lib.qhly_skinShare[name] && lib.qhly_skinShare[name]
											.skills) {
											if (!get.character(name).isNull) {
												// @ts-ignore
										...
										if (skinPackage2.isExt) {
											if (file2) {
												lib.config.qhly_skinset.audioReplace["../" +
														extAudioPath + skillAudio + fileNum] =
													"../" + path + "/" + file;
											} else lib.config.qhly_skinset.audioReplace["../" +
													extAudioPath + file] = "../" + path + "/" +
												file;
										} else {
											if (file2) {
												lib.config.qhly_skinset.audioReplace["skill/" +
														skillAudio + fileNum] = "../" + path +
													"/" + file;
											} else lib.config.qhly_skinset.audioReplace[
												"skill/" + file] = "../" + path + "/" + file;
										}
									}
								} else {
									...
									if (file == name) {
										// @ts-ignore
										var diePathOrigin = game.qhly_getDieAudioOriginalPath(
											name);
										arr.push(diePathOrigin);
										lib.config.qhly_skinset.audioReplace[diePathOrigin] =
											"../" + path + "/" + file;
									} else {
										arr.push("../" + audioOrigin + file);
										lib.config.qhly_skinset.audioReplace["../" +
											audioOrigin + file] = "../" + path + "/" + file;
									}
						...
						//delete lib.config.qhly_skinset.audioReplace['die/' + name];
						// @ts-ignore
						delete lib.config.qhly_skinset.audioReplace[game.qhly_getDieAudioOriginalPath(
							name)];
						//delete lib.config.qhly_skinset.audioReplace['victory/' + name];
						lib.config.qhly_skinset.skinAudioList[name] = arr;
						lib.config.qhly_skinset.skin[name] = skin;
			...
			if (list) {
				for (let m of list) {
					if (skinPackage2.isExt) delete lib.config.qhly_skinset.audioReplace["../" + extAudioPath + m
						.slice(6)];
					delete lib.config.qhly_skinset.audioReplace[m];
				}
			}
			if (skinPackage2.isExt) {
				...
				lib.config.qhly_skinset.audioReplace["../" + skinPackage2.audio + name + '/victory'] = "../" +
					skinPackage.audio + realName + "/victory";
			} else {
				delete lib.config.qhly_skinset.audioReplace['die/' + name];
				delete lib.config.qhly_skinset.audioReplace['victory/' + name];
				lib.config.qhly_skinset.audioReplace["victory/" + name] = "../" + skinPackage.audio + realName +
					'/victory';
			}
	...
	game.qhly_getSkillAudioName = function(skill, player) { //获取技能播放的技能语音名
		...
		if (info.audioname2 && info.audioname2[player.name]) {
			...
			info = skillInfo(audioname);
		}
		var audioinfo = info.audio;
		if (typeof audioinfo == 'string' && skillInfo(audioinfo)) {
			audioname = audioinfo;
			audioinfo = skillInfo(audioname).audio;
		}
						...
						if (audioinfo[2] == 'true') {
							...
							if (audioinfo[2]) {
								if (typeof which == 'number') {
									// @ts-ignore
									return "../extension/" + audioinfo[1] + "/" + audioname + (which %
										audioinfo[2] + 1);
								} else {
									// @ts-ignore
									return "../extension/" + audioinfo[1] + "/" + audioname + Math.ceil(
										audioinfo[2] * Math.random());
								}
							}
						}
					}
					...
					if (audioinfo.length == 3) {
						...
						if (audioinfo[2] == 'true') {
							...
							if (audioinfo[2]) {
								if (typeof which == 'number') {
									// @ts-ignore
									game.playAudio('..', 'extension', audioinfo[1], audioname + (which %
										audioinfo[2] + 1));
								} else {
									//4VrLPyXM/UwVl3SXOMoDpBLQcoJHwBtPcxBNF1VM6oxC7qONebCO4KekZdetP8Zs
									// @ts-ignore
									game.playAudio('..', 'extension', audioinfo[1], audioname + Math.ceil(
										audioinfo[2] * Math.random()));
								}
							}
						}
	...
	lib.qhly_getEventPosition = function(event) {
		...
		// @ts-ignore
		return {
			x: x / game.documentZoom,
			y: y / game.documentZoom
		};
	};
	// @ts-ignore
	...
	game.qhly_open_small_dragon = function(name, from, ingame) {
		...
		// @ts-ignore
		game.qhly_playQhlyAudio('qhly_voc_click3', null, true);
		var baseHeight = (ui.window || document.documentElement).clientHeight * 0.7;
		...
		var state = {
			...
			opacity: function(view) {
				...
				if (angle < this.fadeAngleAreaStart) {
					return Math.abs((angle - this.fadeAngleAreaStart) / (this.visibleAngleStart - this
						.fadeAngleAreaStart)) / 2;
				}
				return Math.abs((angle - this.fadeAngleAreaEnd) / (this.visibleAngleEnd - this
					.fadeAngleAreaEnd)) / 2;
			},
			// @ts-ignore
			// @ts-ignore
		...
		dragonwin.listen(function(e) {
			...
			if (time - state.preclicktime < 250) {
				...
				_status.qhly_open = false;
				// @ts-ignore
				game.qhly_open_new(name, lib.config.qhly_doubledefaultpage ? lib.config
					.qhly_doubledefaultpage : 'skill', ingame);
			}
		...
		if (lib.config.qhly_dragonlocation == 'drag' || lib.config.qhly_dragonlocation == 'head') {
			...
			var dragMouseMove = function(e) {
				if (state.isDragging) {
					...
					var pos = lib.qhly_getEventPosition(e);
					// @ts-ignore
					if (lib.qhly_computeDistance(pos.x, pos.y, state.beginMousePosition.x, state
							.beginMousePosition.y) > 10) {
			...
			var dragMouseUp = function(e) {
				if (state.isDragging) {
					...
					//dragonhead.style.left = newx.toFixed(3)+"px";
					//dragonhead.style.top = newy.toFixed(3)+"px";
					game.saveConfig('qhly_dragonlocationValue', {
						x: dragonwin.style.left,
						y: dragonwin.style.top
					});
	...
	game.qhly_open_small = function(name, from, ingame) {
		...
		try {
				...
				refreshSkins: function() {
					for (var i = 0; i < this.skinViews.length; i++) {
						...
						if (game.qhly_skinIs(name, skin)) {
							...
							skinView.belowText.style.textShadow =
								'.2rem 0rem .5rem red,-.2rem 0rem .5rem red,0rem .2rem .5rem red,0rem -.2rem .5rem red';
						} else {
							skinView.style.filter = "saturate(40%)";
							skinView.belowText.style.textShadow =
								'.2rem 0rem .5rem blue,-.2rem 0rem .5rem blue,0rem .2rem .5rem blue,0rem -.2rem .5rem blue';
						}
					}
				},
			...
			game.qhly_getSkinList(name, function(ret, list) {
				...
				viewState.skinTotalWidth = (viewState.skinPerWidth + viewState.skinGap) * skinList
					.length - viewState.skinGap;
				for (var i = 0; i < skinList.length; i++) {
					var skin = skinList[i];
					var skinView = ui.create.div('.qh-skinchange-skin', content);
					// @ts-ignore
					viewState.skinViews.push(skinView);
					skinView.style.left = Math.round((viewState.skinPerWidth + viewState.skinGap) * i) +
						"px";
					...
					if (i != skinList.length - 1) {
						...
						border.style.left = Math.round((viewState.skinPerWidth + viewState.skinGap) *
							i + viewState.skinPerWidth) + "px";
					}
					if (skin) {
						// @ts-ignore
	...
	game.qhly_open_small_decade = function(name, from, ingame) {
		try {
			...
			if (lib.config.qhly_lutouType && lib.config.qhly_lutouType == 'shousha') dialog.classList.add(
				'shousha');
			...
			// @ts-ignore
			if (lib.config.qhly_autoChangeSkin == 'close' || !lib.config.qhly_autoChangeSkin) autoskin
				.setAttribute('data-auto', false);
			// @ts-ignore
			...
			var exitListener = function() {
				...
				for (var i = 0; i < viewState1.skinViews.length; i++) {
					// @ts-ignore
					if (viewState1.skinViews[i].dynamic && viewState1.skinViews[i].dynamic.renderer
						.postMessage) {
						// @ts-ignore
				...
				if (cPlayer && cPlayer.name2) {
					for (var i = 0; i < viewState2.skinViews.length; i++) {
						// @ts-ignore
						if (viewState2.skinViews[i].dynamic && viewState2.skinViews[i].dynamic.renderer
							.postMessage) {
							// @ts-ignore
			...
			var viewState1 = {
				...
				refresh: function() {
					...
					// @ts-ignore
					if (this.skinTotalWidth + this.offset > 665 && this.cover.getAttribute(
							'data-visible') == '1') this.rArrow.setAttribute('data-visiable', true);
					else {
					...
					// @ts-ignore
					if (this.offset < 0 && this.cover.getAttribute('data-visible') == '1') this.lArrow
						.setAttribute('data-visiable', true);
					else {
				...
				handleMouseMove: function(x, y) {
					if (this.isTouching) {
						...
						// @ts-ignore
						if (this.skinTotalWidth + this.tempoffset > 665 && this.cover.getAttribute(
								'data-visible') == '1') this.rArrow.setAttribute('data-visiable', true);
						// @ts-ignore
						else this.rArrow.setAttribute('data-visiable', false);
						// @ts-ignore
						if (this.tempoffset < 0 && this.cover.getAttribute('data-visible') == '1') this
							.lArrow.setAttribute('data-visiable', true);
						// @ts-ignore
			...
			if (cPlayer && cPlayer.name2) {
				var viewState2 = {
					...
					refresh: function() {
						...
						// @ts-ignore
						if (this.skinTotalWidth + this.offset > 665 && this.cover.getAttribute(
								'data-visible') == '2') this.rArrow.setAttribute('data-visiable', true);
						else {
						...
						// @ts-ignore
						if (this.offset < 0 && this.cover.getAttribute('data-visible') == '2') this
							.lArrow.setAttribute('data-visiable', true);
						else {
					...
					handleMouseMove: function(x, y) {
						if (this.isTouching) {
							...
							// @ts-ignore
							if (this.skinTotalWidth + this.tempoffset > 665 && this.cover.getAttribute(
									'data-visible') == '2') this.rArrow.setAttribute('data-visiable',
								true);
							// @ts-ignore
							else this.rArrow.setAttribute('data-visiable', false);
							// @ts-ignore
							if (this.tempoffset < 0 && this.cover.getAttribute('data-visible') == '2')
								this.lArrow.setAttribute('data-visiable', true);
							// @ts-ignore
							else this.lArrow.setAttribute('data-visiable', false);
							return true;
						}
					...
					handleMouseUp: function(x, y) {
						if (this.isTouching) {
							...
							if (x && y) {
								...
								if (this.tempoffset > 0) {
									this.tempoffset = 0;
								} else if (this.skinTotalWidth - this.visibleWidth() < -this
									.tempoffset) {
									this.tempoffset = -(this.skinTotalWidth - this.visibleWidth());
								}
							}
			...
			game.qhly_getSkinList(namex, function(ret, list) {
				...
				if (window.decadeUI) {
					...
					if (dynamicSkinList) {
						if (lib.config['extension_千幻聆音_qhly_decadeDynamic'] == 'three' &&
							dynamicSkinList.length > 3) playZhuDynamic = false;
						// @ts-ignore
						// @ts-ignore
						// @ts-ignore
				...
				viewState1.skinTotalWidth = (viewState1.skinPerWidth + viewState1.skinGap) * skinList
					.length - viewState1.skinGap + 20;
				for (let i = 0; i < skinList.length; i++) {
					...
					// @ts-ignore
					campName.style.backgroundImage = 'url("' + lib.qhly_path +
						'image/decoration/name_' + campWrap.getAttribute('data-camp') + '.png")';
					...
					skinView.style.left = Math.round((viewState1.skinPerWidth + viewState1.skinGap) *
						i + 16) + "px";
					...
					// @ts-ignore
					if (!skin && dynamicSkinList.includes('经典形象.jpg')) skinView.dynamicToggle
						.setAttribute('toggle', true);
					if (skin && lib.config.qhly_skinset.djtoggle[namex]) {
						// @ts-ignore
						if (lib.config.qhly_skinset.djtoggle[namex][skin.substring(0, skin.lastIndexOf(
								'.'))]) skinView.dynamicToggle.classList.add('jing');
					}
					// @ts-ignore
					skinView.dynamicToggle.addEventListener(lib.config.touchscreen ? 'touchend' :
						'click',
						function() {
							if (this.classList.contains('jing')) {
								this.classList.remove('jing');
								// @ts-ignore
								if (playZhuDynamic) game.qhly_changeDynamicSkin(this.parentNode,
									this.parentNode.belowText.innerText, namex);
								//game.qhly_changeDynamicSkin(cPlayer, this.parentNode.belowText.innerText, namex);
								if (lib.config.qhly_skinset.djtoggle[namex] && lib.config
									.qhly_skinset.djtoggle[namex][this.parentNode.belowText
										.innerText
									]) delete lib.config.qhly_skinset.djtoggle[namex][this
									.parentNode.belowText.innerText
								];
							} else {
								this.classList.add('jing');
								if (playZhuDynamic && this.parentNode.stopDynamic) this.parentNode
									.stopDynamic();
								//cPlayer.stopDynamic(true, false);
								if (!lib.config.qhly_skinset.djtoggle[namex]) lib.config
									.qhly_skinset.djtoggle[namex] = {};
								lib.config.qhly_skinset.djtoggle[namex][this.parentNode.belowText
									.innerText
								] = true;
							}
							// @ts-ignore
					...
					if (skin) {
						...
						// @ts-ignore
						if (singleDynamic.includes(str) && lib.config['extension_千幻聆音_qhly_dom2image'])
							skinView.toImageBtn.setAttribute('single', true); //打开快照
						// @ts-ignore
						...
						// @ts-ignore
						if (dynamicSkinList.includes('经典形象.jpg') && playZhuDynamic) game
							.qhly_changeDynamicSkin(skinView, '经典形象', namex);
					}
						...
						view.listen(function() {
							...
							game.qhly_setCurrentSkin(namex, skin, function() {
								...
								if (lib.config[
									...
									cPlayer) cPlayer.playChangeSkinEffect(false);
								// @ts-ignore
								game.qhlySyncConfig();
								if (lib.config.qhly_smallwinclosewhenchange) {
									exitListener();
			...
			if (cPlayer && cPlayer.name2) {
				...
				game.qhly_getSkinList(namey, function(ret, list) {
					...
					if (window.decadeUI) {
						...
						if (dynamicSkinList) {
							if (lib.config['extension_千幻聆音_qhly_decadeDynamic'] == 'three' &&
								dynamicSkinList.length > 3) playFuDynamic = false;
							// @ts-ignore
							// @ts-ignore
							// @ts-ignore
							dynamicSkinList.forEach(function(value, index, array) {
					...
					viewState2.skinTotalWidth = (viewState2.skinPerWidth + viewState2.skinGap) *
						skinList.length - viewState2.skinGap + 20;
					for (let i = 0; i < skinList.length; i++) {
						var skin = skinList[i];
						...
						skinView.style.left = Math.round((viewState2.skinPerWidth + viewState2
							.skinGap) * i + 16) + "px";
						...
						if (skin && lib.config.qhly_skinset.djtoggle[namey]) {
							// @ts-ignore
							if (lib.config.qhly_skinset.djtoggle[namey][skin.substring(0, skin
									.lastIndexOf('.'))]) skinView.dynamicToggle.classList.add('jing');
						}
						// @ts-ignore
						skinView.dynamicToggle.addEventListener(lib.config.touchscreen ? 'touchend' :
							'click',
							function() {
								if (this.classList.contains('jing')) {
									this.classList.remove('jing');
									// @ts-ignore
									if (playFuDynamic) game.qhly_changeDynamicSkin(this.parentNode,
										this.parentNode.belowText.innerText, namey);
									//game.qhly_changeDynamicSkin(cPlayer, this.parentNode.belowText.innerText, namey, true);
									if (lib.config.qhly_skinset.djtoggle[namey] && lib.config
										.qhly_skinset.djtoggle[namey][this.parentNode.belowText
											.innerText
										]) delete lib.config.qhly_skinset.djtoggle[namey][this
										.parentNode.belowText.innerText
									];
								} else {
									this.classList.add('jing');
									if (playFuDynamic && this.parentNode.stopDynamic) this
										.parentNode.stopDynamic();
									//if(decadeUI.CUR_DYNAMIC) decadeUI.CUR_DYNAMIC--;
									//cPlayer.stopDynamic(false, true);
									//if(decadeUI.CUR_DYNAMIC) decadeUI.CUR_DYNAMIC--;
									if (!lib.config.qhly_skinset.djtoggle[namey]) lib.config
										.qhly_skinset.djtoggle[namey] = {};
									lib.config.qhly_skinset.djtoggle[namey][this.parentNode
										.belowText.innerText
									] = true;
								}
								// @ts-ignore
						...
						if (skin) {
							var str = skin.substring(0, skin.lastIndexOf('.'));
							// @ts-ignore
							if (bothSkin.includes(str)) skinView.dynamicToggle.setAttribute('toggle',
								true);
							// @ts-ignore
							if (singleDynamic.includes(str) && lib.config[
									'extension_千幻聆音_qhly_dom2image']) skinView.toImageBtn.setAttribute(
								'single', true); //打开快照
							// @ts-ignore
							...
							skinView.belowText.innerHTML = "经典形象";
							// @ts-ignore
							if (dynamicSkinList.includes('经典形象.jpg') && playFuDynamic) game
								.qhly_changeDynamicSkin(skinView, '经典形象', namey);
						}
						var skinQua = ui.create.div('.qhly-skinQua-decade', skinView);
						// @ts-ignore
						...
						(function(name, skin, view) {
							view.listen(function() {
								...
								game.qhly_setCurrentSkin(namey, skin, function() {
									...
									//if (view.dynamicToggle && view.dynamicToggle.classList && !view.dynamicToggle.classList.contains('jing')) {
									// @ts-ignore
									game.qhly_changeDynamicSkin(namey, null, null,
										true);
									//}
									if (lib.config[
											'extension_千幻聆音_qhly_decadeChangeEffect'
	...
	game.qhly_open_small_shousha = function(name, from, ingame) {
		try {
			...
			zhuskinBut.listen(function() {
				...
				cover.setAttribute('data-visible', 1);
				// @ts-ignore
				if (Math.round(viewState1.skinTotalWidth) >= viewState1.visibleWidth()) cover
					.setAttribute('data-overstep', true);
				// @ts-ignore
			...
			fuskinBut.listen(function() {
				...
				cover.setAttribute('data-visible', 2);
				// @ts-ignore
				if (Math.round(viewState2.skinTotalWidth) >= viewState2.visibleWidth()) cover
					.setAttribute('data-overstep', true);
				// @ts-ignore
				else cover.setAttribute('data-overstep', false);
				// @ts-ignore
			...
			var viewState1 = {
				...
				refresh: function() {
					...
					content1.style.left = Math.round(this.offset) + "px";
					// @ts-ignore
					if (this.skinTotalWidth + this.offset > this.visibleWidth() && this.cover
						.getAttribute('data-visible') == '1') this.rArrow.setAttribute('data-visiable',
						true);
					else {
						if (rTimer1) clearInterval(rTimer1);
						// @ts-ignore
				...
				handleMouseMove: function(x, y) {
					if (this.isTouching) {
						...
						this.content1.style.left = Math.round(this.tempoffset) + "px";
						// @ts-ignore
						if (this.skinTotalWidth + this.tempoffset > this.visibleWidth() && this.cover
							.getAttribute('data-visible') == '1') this.rArrow.setAttribute(
							'data-visiable', true);
						// @ts-ignore
						else this.rArrow.setAttribute('data-visiable', false);
						// @ts-ignore
			...
			if (cPlayer && cPlayer.name2) {
				var viewState2 = {
					...
					refresh: function() {
						...
						// @ts-ignore
						if (this.skinTotalWidth + this.offset > this.visibleWidth() && this.cover
							.getAttribute('data-visible') == '2') this.rArrow.setAttribute(
							'data-visiable', true);
						else {
							if (rTimer2) clearInterval(rTimer2);
							// @ts-ignore
					...
					handleMouseMove: function(x, y) {
						if (this.isTouching) {
							...
							this.content2.style.left = Math.round(this.tempoffset) + "px";
							// @ts-ignore
							if (this.skinTotalWidth + this.tempoffset > this.visibleWidth() && this
								.cover.getAttribute('data-visible') == '2') this.rArrow.setAttribute(
								'data-visiable', true);
							// @ts-ignore
							else this.rArrow.setAttribute('data-visiable', false);
							// @ts-ignore
			...
			game.qhly_getSkinList(namex, function(ret, list) {
				...
				for (let i = 0; i < skinList.length; i++) {
					...
					skinView.avatar.classList.add('qh-not-replace');
					// @ts-ignore
					skinView.defaultskin = ui.create.div('.qh-skinchange-skin-shousha-default',
						skinView);
					// @ts-ignore
					...
					// @ts-ignore
					skinView.dynamicToggle = ui.create.div('.qh-skinchange-shousha-dynamicChange',
						skinView);
						...
						function() {
							if (this.classList.contains('jing')) {
								...
								if (lib.config.qhly_smallwindowstyle == 'shousha') {
									// @ts-ignore
									this.parentNode.campBack.setAttribute("data-pinzhi", game
										.qhly_getSkinLevel(namex, skin));
					...
					if (game.qhly_skinIs(namex, skin)) {
						// @ts-ignore
						skinView.defaultskin.setAttribute('data-sel', true);
						if (skinView.offsetLeft > viewState1.visibleWidth()) viewState1.offset =
							viewState1.visibleWidth() - (skinView.offsetLeft + viewState1.skinPerWidth);
					} else {
						// @ts-ignore
						skinView.defaultskin.setAttribute('data-sel', false);
					}
			...
			if (cPlayer && cPlayer.name2) {
				...
				game.qhly_getSkinList(namey, function(ret, list) {
					...
					for (let i = 0; i < skinList.length; i++) {
						...
						skinView.campBack = ui.create.div('.qhcamp-shousha-back', skinView);
						// @ts-ignore
						skinView.campBack.setAttribute('data-pinzhi', game.qhly_getSkinLevel(namey,
							skin));
						var campName = ui.create.div('.qhcamp-name', campWrap);
						// @ts-ignore
						campName.style.backgroundImage = 'url("' + lib.qhly_path +
						...
						// @ts-ignore
						viewState2.skinViews.push(skinView);
						skinView.style.left = Math.round((viewState1.skinPerWidth + viewState1
							.skinGap) * i + 16) + "px";
						skinView.style.width = Math.round(viewState1.skinPerWidth) + "px";
						// @ts-ignore
						skinView.avatar.classList.add('qh-not-replace');
						...
						if (game.qhly_skinIs(namey, skin)) {
							// @ts-ignore
							skinView.defaultskin.setAttribute('data-sel', true);
							if (skinView.offsetLeft > viewState2.visibleWidth()) viewState2.offset =
								viewState2.visibleWidth() - (skinView.offsetLeft + viewState2
									.skinPerWidth);
						} else {
							// @ts-ignore
							skinView.defaultskin.setAttribute('data-sel', false);
	...
	// @ts-ignore
	// The current async useSkill pipeline is retained; audio is adapted through game.playAudio.
	...
	game.playCardAudio = function(card, player) {
		if (!player || typeof player !== 'object') return oldPlayCardAudio(card, player);
		...
		if (lib.config.background_audio) {
			...
			var skin = game.qhly_getSkin(player.name1);
			// @ts-ignore
			if (lib.config.qhly_changeSex && lib.config.qhly_changeSex[player.name1] && lib.config
				.qhly_changeSex[player.name1][skin]) sex = (sex == 'female' ? 'male' : 'female');
			// @ts-ignore
			var pkg = game.qhly_foundPackage(player.name1);
			// @ts-ignore
			var realName = game.qhly_getRealName(player.name1);
			...
			var cardURL = pkgPath + (skin ? realName : player.name1) + '/' + game.qhly_earse_ext(skin) + '/';
			// @ts-ignore
			if (lib.qhly_skinChange[realName] && lib.qhly_skinChange[realName][game.qhly_earse_ext(skin)] && lib
				.qhly_skinChange[realName][game.qhly_earse_ext(skin)].cardaudio) {
				// @ts-ignore
				if (game.qhly_getPlayerStatus(player) == 2 && lib.qhly_skinChange[realName][game.qhly_earse_ext(
						skin)].audio1) cardURL = pkgPath + (skin ? realName : player.name1) + '/' + lib
					.qhly_skinChange[realName][game.qhly_earse_ext(skin)].audio1;
				exchangeCA = true;
			}
			if (exchangeCA) {
				// @ts-ignore
	...
	*/
	...
	game.qhly_open_new = function(name, page, ingame) {
		...
		gback.close = function() {
			...
			if (window.inSplash && game.qhly_hasExtension("如真似幻")) {
				// The current lobby owns its window lifecycle.
			}
		};
	...
	ui.qhly_initCheckBox = function(view, checked, forbid) {
		...
		if (view.qhly_checked) {
			// @ts-ignore
			view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin]
				.checkBoxCheckedImage, 'extension/千幻聆音/image/newui_checkbox_checked.png');
		} else {
			// @ts-ignore
			view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin]
		...
		view.qhly_setChecked = function(checked, trigger) {
			...
			if (checked != this.qhly_checked) {
				...
				if (this.qhly_checked) {
					// @ts-ignore
					this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config
							.qhly_currentViewSkin].checkBoxCheckedImage,
						'extension/千幻聆音/image/newui_checkbox_checked.png');
				} else {
					// @ts-ignore
					this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config
							.qhly_currentViewSkin].checkBoxImage,
						'extension/千幻聆音/image/newui_checkbox_unchecked.png');
				}
				// @ts-ignore
	...
	game.qhly_getNonameTaici = function(name) {
		...
		skills.forEach((skill, i) => {
			...
			let content = textMap.filter(current => current.text).reduce((previous, current,
				currentIndex, array) => {
				return previous + current.text + ((currentIndex == array.length - 1) ? "" :
					"<br>");
			}, "");
			ret[skill] = {
				order: i,
		...
		if (record && record[name]) {
			...
			for (var mode of modekeys) {
				...
				for (var identity of identKeys) {
					...
					str += "&nbsp;&nbsp;失败：" + lose;
					str += "&nbsp;&nbsp;总场：" + (win + lose);
					str += "&nbsp;&nbsp;胜率：" + (((win + lose) <= 0) ? 0 : ((win * 100 / (win + lose)).toFixed(
						2))) + "%";
					str += "</p>";
					str += "<br>";
				}
	...
	game.qhly_createBelowMenu = function(items, parent) {
		...
		for (var item of items) {
			content += "<tr>";
			content += "<td class='qh-below-menu-item' id='qhly_below_menu_" + id + "_" + bid_i + "'>" + item
				.name + "</td>";
			content += "</tr>";
			bid_i++;
		}
	...
	game.qhly_initDecadeView = function(name, view, page, cplayer) {
		...
		// @ts-ignore
		view.style['background-image'] = 'url(' + lib.qhly_path + 'theme/decade/background/' + lib.config
			.qhly_decadeBigBak + '.jpg)';
		...
		changeBgBtn.listen(function() {
			...
			game.qhly_playQhlyAudio('qhly_voc_click3', null, true);
			// @ts-ignore
			view.style['background-image'] = 'url(' + lib.qhly_path + 'theme/decade/background/' + lib
				.config.qhly_decadeBigBak + '.jpg)';
		})
		var refreshRank = function() {};
		subView.avatar = ui.create.div('.qh-shousha-big-avatar', view);
		if (lib.config.qhly_lutouType && lib.config.qhly_lutouType == 'shousha') subView.avatar.classList.add(
			'shousha');
		...
		// @ts-ignore
		if (game.qhly_getPlayerStatus(cplayer, cplayer && cplayer.name2 && cplayer.name2 == name) == 2) game
			.qhly_setPlayerStatus(subView.avatarImage, undefined, 2);
		// @ts-ignore
		...
		subView.avatarImage.addEventListener(lib.config.touchscreen ? "touchstart" : "mousedown", function(e) {
			// @ts-ignore
			if (!subView.avatarImage.dynamic || subView.avatarImage.dynamic && subView.avatarImage
				.dynamic.primary == null) {
		...
		subView.avatarImage.addEventListener(lib.config.touchscreen ? "touchend" : "mouseup", function(e) {
			// @ts-ignore
			game.playShoushaAvatar(this, lib.config.qhly_shoushaBigFlip[name][game.qhly_getSkin(name)],
				name); //十周年还需要补充是否翻转X
			clearTimeout(timer);
		});
		...
		subView.dynamicToggle.style.cssText += 'position:relative;top:-39%;width:' + document.body.offsetWidth *
			0.08 + 'px;height:' + document.body.offsetWidth * 0.05 + 'px';
		// @ts-ignore
		var skinStr = game.qhly_getSkin(name);
		if (skinStr == null) skinStr = '经典形象';
		else skinStr = skinStr.split('.')[0];
		if (lib.config.qhly_skinset.djtoggle[name] && lib.config.qhly_skinset.djtoggle[name][skinStr]) subView
			.dynamicToggle.classList.add('jing');
		subView.dynamicToggle.addEventListener(lib.config.touchscreen ? 'touchend' : 'click', function() {
			...
			if (this.classList.contains('jing')) {
				...
				if (lib.config.qhly_skinset.djtoggle[name] && lib.config.qhly_skinset.djtoggle[name][
						skinStr
					]) delete lib.config.qhly_skinset.djtoggle[name][skinStr];
			} else {
				this.classList.add('jing');
				if (djtoggle) {
					// @ts-ignore
					if (!djtoggle.parentNode.stopDynamic) djtoggle.parentNode.stopDynamic =
						qhly_stopdynamic;
					// @ts-ignore
			...
			game.qhlySyncConfig();
			// @ts-ignore
			if (!_status['qhly_primarySkin_' + name] || _status['qhly_primarySkin_' + name] && _status[
					'qhly_primarySkin_' + name] == game.qhly_getSkin(name)) game.qhly_changeDynamicSkin(
				name);
		});
		...
		// @ts-ignore
		subView.rank.style['background-image'] = 'url(' + lib.qhly_path + 'theme/decade/rarity_' + game
			.getRarity(name) + '.png)';
		...
		// @ts-ignore
		subView.pageButton.introduce.downButton = ui.create.div('.qh-otherinfoarrow', subView.pageButton
			.introduce);
		var swipe_up = lib.config.swipe_up;
		lib.config.swipe_up = '';
		var swipe_down = lib.config.swipe_down;
		...
		subView.backButton.listen(function() {
			...
			game.qhly_checkPlayerImageAudio(name, game.qhly_getSkin(name), cplayer, function() {
				...
				cplayer.node[avatar].qhly_origin_setBackgroundImage(cplayer._qhly_skinChange[
					avatar == 'avatar2' ? 1 : 0]);
				// @ts-ignore
				if (!_status.qhly_replaceSkin[playerName]) _status.qhly_replaceSkin[
					playerName] = {};
				// @ts-ignore
				_status.qhly_replaceSkin[playerName][skin] = cplayer._qhly_skinChange[avatar ==
					'avatar2' ? 1 : 0];
				// @ts-ignore
				if (window.decadeUI && !game.qhly_hasExtension('皮肤切换') && !game
					.qhly_hasExtension('EpicFX')) game.qhly_changeDynamicSkin(cplayer,
					undefined, undefined, avatar == 'avatar2');
			});
			if (subView.page.skin.viewState && subView.page.skin.viewState.skinViews) {
				for (var i = 0; i < subView.page.skin.viewState.skinViews.length; i++) {
					if (subView.page.skin.viewState.skinViews[i].dynamic && subView.page.skin.viewState
						.skinViews[i].dynamic.renderer.postMessage) {
			...
			// @ts-ignore
			if (_status['qhly_primarySkin_' + name] !== undefined) game.qhly_setCurrentSkin(name,
				_status['qhly_primarySkin_' + name]);
		...
		subView.page = {
			introduce: {
				...
				refresh: function(name, state) {
					...
					if (!state.introduceExtraPage || state.introduceExtraPage == '简介') {
						...
						// @ts-ignore
						subView.pageButton.introduce.appendChild(subView.pageButton.introduce
							.downButton);
					} else {
						var ret = '';
						var handleView = null;
				...
				init: function(name, state) {
					...
					if (lib.config.qhly_vMiddle === false && (currentViewSkin.isQiLayout ||
							currentViewSkin.isLolBigLayout)) {
				...
				init: function(name, state) {
					...
					if (pkg && pkg.characterTaici) {
						...
						if (taici) {
							...
							viewSkill.sort(function(a, b) {
								var orderA = (taici[a] && taici[a].order) ? taici[a].order :
									Infinity;
								var orderB = (taici[b] && taici[b].order) ? taici[b].order :
									Infinity;
								return orderA - orderB;
							});
						}
					...
					for (var skill of viewSkill) {
						...
						if (detail) {
							...
							skillInfoHead.innerHTML +=
								"<img style='float:right;width:18%;max-width:48px' id='qhly_skillv_" +
								skill + "'/>";
							...
							if (cplayer && lib.config.qhly_skillingame) {
								...
								// @ts-ignore
								if (dtrans && lib.qhly_filterPlainText(dtrans) != lib
									.qhly_filterPlainText(detail)) {
							...
							if (info && (info.frequent || info.subfrequent)) {
								content +=
									"<br>&nbsp;&nbsp;<img style='vertical-align:middle;' id='qhly_autoskill_" +
									skill + "'/><b id='qhly_autoskill_text_" + skill + "'>自动发动</b>"
							}
					...
					for (let skill of viewSkill) {
						...
						if (detail) {
							(function(skill) {
								...
								if (img) {
									// @ts-ignore
									img.src = lib.assetURL + get.qhly_getCurrentViewSkinValue(
										'skillPagePlayAudioButtonImage',
										'extension/千幻聆音/image/newui_playaudio.png');
									// @ts-ignore
									...
									img.listen(function() {
										...
										// @ts-ignore
										window.qhly_TrySkillAudio(skill, {
											name: name
										}, null, count);
										// @ts-ignore
										var skillSkin = game.qhly_getSkillSkin(name, game
											.qhly_getSkin(name), skill);
										if (skillSkin) {
											if (skillSkin === 1) {
												subView.avatarImage.setBackground(name,
													'character');
											} else if (Array.isArray(skillSkin)) {
												subView.avatarImage.setBackgroundImage(
													skillSkin[count % skillSkin.length]);
											} else {
												subView.avatarImage.setBackgroundImage(
													skillSkin);
											}
			...
			skin: {
				...
				onClickSkin: function(num, name, state, father) {
					...
					game.qhly_setCurrentSkin(name, originSkin, function() {
						...
						if (originSkin) {
							...
							if (!lib.config.qhly_skinset.djtoggle[name] || lib.config
								.qhly_skinset.djtoggle[name] && !lib.config.qhly_skinset
								.djtoggle[name][str]) {
								// @ts-ignore
								game.qhly_changeDynamicSkin(subView.avatarImage, str, name);
								// @ts-ignore
								if (subView.avatarImage.dynamic && subView.avatarImage.dynamic
									.primary) _status.currentTexiao = subView.avatarImage
									.dynamic.primary.name;
							}
							// @ts-ignore
						...
						if (state.mainView.avatarImage.dynamic && state.mainView.avatarImage
							.dynamic.primary != null) state.mainView.dynamicToggle.setAttribute(
							'toggle', true);
						...
						// @ts-ignore
						game.qhly_setOriginSkin(name, originSkin, state.mainView.avatarImage,
							state, game.qhly_getPlayerStatus(state.mainView.avatarImage,
								null, state.name) == 2);
					}.bind(this), true);
				},
				refresh: function(name, state) {
				...
				editOpen: function(name, skin, skill, state) {
					...
					var realSkill = skill;
					// @ts-ignore
					if (lib.qhly_skinShare[name] && lib.qhly_skinShare[name].skills && lib
						.qhly_skinShare[name].skills[skill]) realSkill = lib.qhly_skinShare[name]
						.skills[skill];
					if (skin) {
						// @ts-ignore
						...
						if (skinInfo && skinInfo.skill) {
							if (skinInfo.skill[realSkill] && skinInfo.skill[realSkill].content) {
								if (skinInfo.skill[realSkill].content2) {
									initValue = skinInfo.skill[realSkill].content1 + '+' + skinInfo
										.skill[realSkill].content2;
					...
					game.qhly_editDialog(title, detail, initValue, function(value, dialog) {
						...
						// @ts-ignore
						game.qhly_readFileAsText("extension/千幻聆音/data/skininfomodel.txt",
							function(ret, str) {
								if (ret) {
									str = str.replace("_REPLACE_OBJECT_", strobj);
									// @ts-ignore
									...
									// @ts-ignore
									game.qhly_writeTextFile(str, path + realName,
										"skininfo.js",
										function(err) {
											if (!err) {
												alert("保存成功");
				...
				refreshAfterGot: function(name, state) {
					...
					this.viewState.skinTotalWidth = ((this.viewState.skinPerWidth + this.viewState
							.skinGap) * this.skinList.length - this.viewState.skinGap) * this.viewState
						.scale;
					var playBigDynamic = lib.config['extension_千幻聆音_qhly_decadeDynamic'] == 'always' ?
						false : true;
					if (this.dynamicSkinMore && lib.config['extension_千幻聆音_qhly_decadeDynamic'] ==
						'three') playBigDynamic = false;
					if (this.firstRefresh) {
						...
						for (var i = 0; i < this.skinList.length; i++) {
							...
							// @ts-ignore
							if (game.qhly_getPlayerStatus(cplayer, cplayer && cplayer.name2 && cplayer
									.name2 == name) == 2) game.qhly_setPlayerStatus(skinView, undefined,
								2);
							// @ts-ignore
							skinView.avatar = ui.create.div('.primary-avatar', skinView);
							// @ts-ignore
							...
							// @ts-ignore
							skinView.belowText = ui.create.div('.qh-skinchange-decade-big-skin-text',
								skinView);
							...
							// @ts-ignore
							skinView.defaultskin = ui.create.div('.qh-skinchange-skin-big-default',
								skinView);
							// @ts-ignore
							...
							// @ts-ignore
							skinView.dynamicToggle = ui.create.div('.qh-skinchange-big-dynamicChange',
								skinView);
							// @ts-ignore
							skinView.dynamicToggle.id = 'qhlydecadeBigdjtoggle' + skin;
							// @ts-ignore
							if (this.skinList[i].bothSkin) skinView.dynamicToggle.setAttribute('toggle',
								true);
							// @ts-ignore
							if (this.skinList[i].single && lib.config['extension_千幻聆音_qhly_dom2image'])
								skinView.toImageBtn.setAttribute('single', true); //6
							if (skin && lib.config.qhly_skinset.djtoggle[name]) {
								// @ts-ignore
								if (lib.config.qhly_skinset.djtoggle[name][skin.substring(0, skin
										.lastIndexOf('.'))]) skinView.dynamicToggle.classList.add(
									'jing');
							}
							// @ts-ignore
							skinView.dynamicToggle.addEventListener(lib.config.touchscreen ?
								'touchend' : 'click',
								function() {
									var skinStr = this.parentNode.belowText.innerText.substring(0,
										this.parentNode.belowText.innerText.lastIndexOf('*'));
									if (this.classList.contains('jing')) {
										this.classList.remove('jing');
										subView.dynamicToggle.classList.remove('jing');
										// @ts-ignore
										if (playBigDynamic) game.qhly_changeDynamicSkin(this
											.parentNode, skinStr, name);
										// @ts-ignore
										if (this.parentNode.avatar.id == 'qh-skillskin-0' && that
											.skinList[0].bothSkin || _status['qhly_primarySkin_' +
												name] && _status['qhly_primarySkin_' + name]
											.substring(0, _status['qhly_primarySkin_' + name]
												.length - 4) == skinStr) game
											.qhly_changeDynamicSkin(state.mainView.avatarImage,
												skinStr, name);
										if (lib.config.qhly_skinset.djtoggle[name] && lib.config
											.qhly_skinset.djtoggle[name][skinStr]) delete lib.config
											.qhly_skinset.djtoggle[name][skinStr];
									} else {
										this.classList.add('jing');
										subView.dynamicToggle.classList.add('jing');
										if (this.parentNode.stopDynamic) this.parentNode
											.stopDynamic();
										// @ts-ignore
										if (this.parentNode.avatar.id == 'qh-skillskin-0' && that
											.skinList[0].bothSkin || _status['qhly_primarySkin_' +
												name] && _status['qhly_primarySkin_' + name]
											.substring(0, _status['qhly_primarySkin_' + name]
												.length - 4) == skinStr) state.mainView.avatarImage
											.stopDynamic();
										if (!lib.config.qhly_skinset.djtoggle[name]) lib.config
											.qhly_skinset.djtoggle[name] = {};
										lib.config.qhly_skinset.djtoggle[name][skinStr] = true;
									}
									// @ts-ignore
									game.qhlySyncConfig();
							...
							if (skin) {
								...
								if (info) {
									// @ts-ignore
									skinView.belowText.innerText = info.translation + '*' + get
										.rawName2(name);
								}
								// @ts-ignore
								...
								// @ts-ignore
								if (this.skinList[0].skinId == null && this.skinList[0].bothSkin &&
									playBigDynamic) game.qhly_changeDynamicSkin(skinView, '经典形象', name);
							}
							...
							if (game.qhly_skinIs(name, skin)) {
								...
								skinView.defaultskin.setAttribute('data-sel', true);
								// @ts-ignore
								if (this.skinList[i].bothSkin) state.mainView.dynamicToggle
									.setAttribute('toggle', true);
								else state.mainView.dynamicToggle.setAttribute('toggle', false);
							} else {
								// @ts-ignore
							...
							this.viewState.offset = Math.round((98 - (this.viewState.skinPerWidth + this
									.viewState.skinGap) * this.viewState.viewCurrentIndex) * this
								.viewState.scale);
							skinView.style.left = Math.round((this.viewState.skinPerWidth + this
								.viewState.skinGap) * i * this.viewState.scale) + "px";
							skinView.style.width = Math.round(this.viewState.skinPerWidth * this
								.viewState.scale) + "px";
							skinView.style.height = Math.round(396 * this.viewState.scale) + "px";
							skinView.style.transform = 'scale(' + Math.max(0.3, 1 - 0.4 * Math.abs(i + (
								this.viewState.offset - 98 * this.viewState.scale) / ((this
									.viewState.skinPerWidth + this.viewState.skinGap) * this
								.viewState.scale))) + ')';
							// @ts-ignore
							skinView.style.zIndex = Math.round(Math.max(10, 15 - Math.abs(this.viewState
								.viewCurrentIndex - i)));
							// @ts-ignore
							skinView.style.opacity = Math.round(Math.min(1, 1.7 - Math.abs(i + (this
								.viewState.offset - 98 * this.viewState.scale) / ((this
									.viewState.skinPerWidth + this.viewState.skinGap) *
							...
							if (skin) {
								...
								game.qhly_checkFileExist(file, function(s) {
									if (s) {
										...
										if (lib.config.qhly_noSkin == 'origin') skinView2
											.qhly_origin_setBackgroundImage(prefix + name +
												'.jpg'); //原画
										else skinView2.qhly_origin_setBackgroundImage(
											'extension/千幻聆音/image/noSkin.png'); //noskin
									}
						...
						if (lib.config.touchscreen) {
							content.addEventListener('touchstart', function(event) {
								if (event.touches && event.touches.length) {
									that.viewState.handleMouseDown(event.touches[0].clientX,
							...
							content.addEventListener('touchmove', function(event) {
								if (event.touches && event.touches.length)
									that.viewState.handleMouseMove(event.touches[0].clientX,
										event.touches[0].clientY);
							});
						} else {
							content.addEventListener('mousewheel', function(event) {
								...
								if (event.wheelDelta > 0) {
									that.viewState.handleMouseMove(event.clientX - 30, event
										.clientY);
									that.viewState.handleMouseUp(event.clientX - 30, event
										.clientY);
								} else {
									that.viewState.handleMouseMove(event.clientX + 30, event
										.clientY);
					...
					if (!currentSkin.isLocked) {
						...
						// @ts-ignore
						if (game.qhly_getSkin(name)) Vicpath +=
							`${game.qhly_earse_ext(game.qhly_getSkin(name))}/`;
						for (var audio of currentSkin.audios) {
							...
							// @ts-ignore
							if (audio.id == 'victory' && !game.thunderFileExist(lib.assetURL + Vicpath +
									'victory.mp3')) continue;
							var skillAudioInfo = ui.create.div(
								'.qh-skinchange-decade-big-skillaudioinfo', this.viewState
								.skinAudioList);
							skillAudioInfo.innerHTML = cskill;
							var huawen = ui.create.div('.qh-skinchange-decade-big-huawen',
								skillAudioInfo);
							// @ts-ignore
							...
							var skillTaici = ui.create.div('.qh-skinchange-decade-big-skilltaici', this
								.viewState.skinAudioList);
							var objx = this.packObject(name, state);
							var initValuex = "";
							var realSkill = audio.id;
							// @ts-ignore
							if (lib.qhly_skinShare[name] && lib.qhly_skinShare[name].skills && lib
								.qhly_skinShare[name].skills[audio.id]) realSkill = lib.qhly_skinShare[
								name].skills[audio.id];
							if (currentSkin.skinId) {
								// @ts-ignore
								var skinInfox = objx.skin[game.qhly_earseExt(currentSkin.skinId)];
								if (skinInfox && skinInfox.skill) {
									if (skinInfox.skill[realSkill] && skinInfox.skill[realSkill]
										.content) {
										if (skinInfox.skill[realSkill].content2) {
											// @ts-ignore
											initValuex = skinInfox.skill[realSkill]['content' + game
												.qhly_getPlayerStatus(cplayer, (cplayer && cplayer
														.name2 && cplayer.name2 == state.name),
													state.name)
											];
						...
						var skinConfig = ui.create.div('.qh-skinchange-decade-big-skinconfig', this
							.viewState.skinAudioList);
						var skinConfigContent = '';
						if (lib.config.qhly_skinconfig) {
							skinConfigContent += "<h2>皮肤配置</h2>";
							skinConfigContent +=
								"<p><span style='display:inline-block;height:30px;'><span id='qhconfig_checkbox_banInRandom_text' style='display:inline-block;position:relative;bottom:25%;'>随机切换禁用&nbsp;&nbsp;</span><img id='qhconfig_checkbox_banInRandom'/></span></p>";
							if (currentSkin.skinId) {
								skinConfigContent +=
									"<p><span style='display:inline-block;height:30px;'><span id='qhconfig_checkbox_changeSex_text' style='display:inline-block;position:relative;bottom:25%;'>性转皮肤&nbsp;&nbsp;</span><img id='qhconfig_checkbox_changeSex'/></span></p>";
								skinConfigContent +=
									"<p><span style='display:inline-block;height:30px;'><span id='qhconfig_checkbox_dwflip_text' style='display:inline-block;position:relative;bottom:25%;'>出框翻转&nbsp;&nbsp;</span><img id='qhconfig_checkbox_dwflip'/></span></p>";
								skinConfigContent +=
									"<p><span>皮肤品质&nbsp;&nbsp;</span><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_level_select'></select></p>";
								skinConfigContent +=
									"<p><span>皮肤顺序&nbsp;&nbsp;</span><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_order_select'></select></p>";
							}
						...
						if (lib.config.qhly_skinconfig) {
							if (currentSkin.skinId) {
								...
								levelSelect.onchange = function(e) {
									...
									if (event.target) {
										...
										if (opt) {
											...
											if (l == 'default') {
												// @ts-ignore
												delete lib.qhly_level[name + '_' + currentSkin
													.skinId];
												// @ts-ignore
										...
										if (level && map[level] && map[level].startsWith("^^")) {
											var skinQua = document.getElementById('qhly_skinQua' +
												that.viewState.viewCurrentIndex);
											// @ts-ignore
											if (skinQua) skinQua.style['background-image'] =
												'url(' + lib.qhly_path + 'image/diylevels/' + map[
													level].replace("^^", "") + ")";
										} else {
											var skinQua = document.getElementById('qhly_skinQua' +
												that.viewState.viewCurrentIndex);
											// @ts-ignore
											if (skinQua) skinQua.style['background-image'] =
												'url(' + lib.qhly_path + 'theme/decade/dc_' + game
												.qhly_getSkinLevel(name, currentSkin.skinId, true) +
												'.png)';
										}
									}
								};
								...
								if (lib.config.qhly_order[name + '-' + currentSkin.skinId] ===
									undefined) {
									// @ts-ignore
									opt.selected = 'selected';
								}
							...
							var banInRandomCheckbox = document.getElementById(
								'qhconfig_checkbox_banInRandom');
							var bindFunc = function(checkbox, text) {
								if (!text) return;
								// @ts-ignore
								ui.qhly_addListenFunc(text);
							...
							// @ts-ignore
							ui.qhly_initCheckBox(banInRandomCheckbox, game.qhly_skinIsBanned(name,
								currentSkin.skinId));
							bindFunc(banInRandomCheckbox, document.getElementById(
								'qhconfig_checkbox_banInRandom_text'));
							// @ts-ignore
							...
							if (currentSkin.skinId) {
								...
								if (dwflip) {
									// @ts-ignore
									ui.qhly_initCheckBox(dwflip, lib.config.qhly_shoushaBigFlip[name][
										game.qhly_getSkin(name)
									]);
									bindFunc(dwflip, document.getElementById(
										'qhconfig_checkbox_dwflip_text'));
									// @ts-ignore
									dwflip.qhly_onchecked = function(check) {
										if (!check) {
											// @ts-ignore
											if (lib.config.qhly_shoushaBigFlip[name][game
													.qhly_getSkin(name)
												]) {
												// @ts-ignore
												lib.config.qhly_shoushaBigFlip[name][game
													.qhly_getSkin(name)
												] = false;
											}
										} else {
											// @ts-ignore
											lib.config.qhly_shoushaBigFlip[name][game.qhly_getSkin(
												name)] = true;
										}
										game.saveConfig('qhly_shoushaBigFlip', lib.config
											.qhly_shoushaBigFlip);
									};
								}
								...
								if (changeSex) {
									...
									bindFunc(changeSex, document.getElementById(
										'qhconfig_checkbox_changeSex_text'));
									// @ts-ignore
									changeSex.qhly_onchecked = function(check) {
										if (!check) {
											// @ts-ignore
											if (lib.config.qhly_changeSex && lib.config
												.qhly_changeSex[name] && lib.config.qhly_changeSex[
													name][game.qhly_getSkin(name)]) {
												// @ts-ignore
												lib.config.qhly_changeSex[name][game.qhly_getSkin(
													name)] = false;
											}
										} else {
							...
							if (img) {
								...
								(function(id) {
									...
									img.listen(function() {
										...
										// @ts-ignore
										else if (id == 'victory') window
											.qhly_playVictoryAudio(name);
										else {
											// @ts-ignore
											...
											// @ts-ignore
											window.qhly_TrySkillAudio(id, {
												name: name
											}, null, count);
											// @ts-ignore
											var skillSkin = game.qhly_getSkillSkin(name,
												game.qhly_getSkin(name), id);
											if (skillSkin) {
												var currentAvatar = document.getElementById(
													'qh-skillskin-' + that.viewState
													.viewCurrentIndex);
												if (currentAvatar) {
													if (skillSkin === 1) {
														// @ts-ignore
														currentAvatar.setBackground(name,
															'character');
													} else if (Array.isArray(skillSkin)) {
														// @ts-ignore
														currentAvatar.setBackgroundImage(
															skillSkin[count % skillSkin
																.length]);
													} else {
														// @ts-ignore
														currentAvatar.setBackgroundImage(
								...
								if (imgEdit) {
									...
									(function(id) {
										...
										imgEdit.listen(function() {
											that.editOpen(name, currentSkin.skinId, id,
												state);
										});
									})(vid);
								}
					...
					if (window.decadeUI && window.decadeUI.dynamicSkin) {
						...
						if (dynamicSkinList.length) {
							...
							for (let i of this.skinList) {
								// @ts-ignore
								if (i.skinId && i.skinId != null) duibiList.push(i.skinId.substring(0, i
									.skinId.lastIndexOf('.')));
							}
							for (let i of dynamicSkinList) {
								...
								if (i == '经典形象') this.skinList['0'].bothSkin = true;
									...
									this.skinList.push({
										...
										skinInfo: dyinfo,
										// @ts-ignore
										audios: get.qhly_getAudioInfoInSkin(name, state.pkg,
											dyskin),
										single: true, //11
									})
								}
				...
				init: function(name, state) {
					...
					if (lib.config.qhly_lutouType && lib.config.qhly_lutouType == 'shousha') dialog
						.classList.add('shousha');
					...
					var skinAudioList = ui.create.div('.qh-skinchange-decade-big-skinaudiolist', this
						.text);
					if (lib.config.qhly_lutou) dialog.setAttribute('data-outcrop-skin', 'on');
					// @ts-ignore
					if (!_status['qhly_primarySkin_' + name]) _status['qhly_primarySkin_' + name] = game
						.qhly_getSkin(name);
					...
					autoskin.listen(function() {
						var open = false,
							item = 'close';
						if (lib.config.qhly_autoChangeSkin == 'close' || !lib.config
							.qhly_autoChangeSkin) {
							open = true;
							item = lib.config['extension_千幻聆音_qhly_decadeAuto'];
						}
					...
					this.viewState = {
						...
						refresh: function() {
							...
							if (cskin) {
								...
								if (tname.indexOf('.') != -1) tname = tname.substring(0, tname
									.lastIndexOf('.'));
								// @ts-ignore
							} else var tname = '经典形象';
						...
						handleMouseMove: function(x, y) {
							...
							var slideX = x - this.mouseDownX;
							this.tempoffset = this.offset + slideX;
							if (this.tempoffset > ((this.skinPerWidth + this.skinGap) * 0.5 +
									46) * this.scale) {
								this.tempoffset = ((this.skinPerWidth + this.skinGap) * 0.5 +
									46) * this.scale;
							} else if ((this.skinTotalWidth - 372 * this.scale) < -this
								.tempoffset) {
								this.tempoffset = -(this.skinTotalWidth - 372 * this.scale);
							}
							this.content.style.left = Math.round(this.tempoffset) + "px";
							var current = (this.tempoffset - 98 * this.scale) / ((this
								.skinPerWidth + this.skinGap) * this.scale);
							this.viewCurrentIndex = -Math.round(current);
							for (var i = 0; i < this.skinViews.length; i++) {
								var zin = Math.max(10, 15 - Math.abs(i + Math.round(current)));
								// @ts-ignore
								this.skinViews[i].style.transform = 'scale(' + Math.max(0.3, 1 -
									0.4 * Math.abs(i + current)) + ')';
								// @ts-ignore
								this.skinViews[i].style.zIndex = zin;
								// @ts-ignore
								this.skinViews[i].style.opacity = Math.round(Math.min(1, 1.7 -
									Math.abs(i + current)));
							}
							return true;
						},
						...
						handleMouseUp: function(x, y) {
							if (this.isTouching) {
								this.isTouching = false;
								this.tempoffset = (98 - this.viewCurrentIndex * (this
									.skinPerWidth + this.skinGap)) * this.scale;
								var current = (this.tempoffset - 98 * this.scale) / ((this
									.skinPerWidth + this.skinGap) * this.scale);
								for (var i = 0; i < this.skinViews.length; i++) {
									var zin = Math.max(10, 15 - Math.abs(i + Math.round(
										current)));
									// @ts-ignore
									this.skinViews[i].style.transform = 'scale(' + Math.max(0.3,
										1 - 0.4 * Math.abs(i + current)) + ')';
									// @ts-ignore
									this.skinViews[i].style.zIndex = zin;
									// @ts-ignore
									this.skinViews[i].style.opacity = Math.round(Math.min(1,
										1.7 - Math.abs(i + current)));
								}
								this.cancelClick = Math.abs(this.offset - this.tempoffset) > 50;
								this.content.style.left = Math.round(this.tempoffset) + "px";
							} else {
		...
		refreshRank = function() {
			if (subView.rank) {
				if (lib.config.qhly_showrarity) {
					...
					if (rarity) {
						subView.rank.setBackgroundImage('extension/千幻聆音/theme/decade/rarity_' + game
							.getRarity(name) + '.png');
					}
				} else {
					subView.rank.hide();
			...
			(function(m) {
				subView.pageButton[m].listen(function() {
					if (subView.currentPage != m) {
						document.getElementsByClassName('qh-otherinfoarrow')[0].classList.remove(
							'sel');
						showPage(m);
						if (state.extraMenu) {
							state.extraMenu.delete();
						...
						if (state.extraMenu) {
							state.extraMenu.delete();
							document.getElementsByClassName('qh-otherinfoarrow')[0].classList
								.remove('sel');
							delete state.extraMenu;
						} else {
							document.getElementsByClassName('qh-otherinfoarrow')[0].classList.add(
								'sel');
							// @ts-ignore
									...
									(function(obj) {
										arr.push({
											...
											onchange: function() {
												document.getElementsByClassName(
														'qh-otherinfoarrow')[0]
													.classList.remove('sel');
												state.introduceExtraPage = obj.name;
												if (obj.qh_func) {
													state.introduceExtraFunc = game[
														obj.qh_func];
												} else {
													state.introduceExtraFunc = obj
														.func;
												}
												subView.page.introduce.refresh(name,
													state);
												if (state.extraMenu) {
													state.extraMenu.delete();
													delete state.extraMenu;
													view.removeChild(subView
														.menuCover);
												}
											}
										});
		...
		var refreshView = function(state, subView) {
			...
			// @ts-ignore
			game.qhly_setOriginSkin(name, game.qhly_getSkin(name), state.mainView.avatarImage, state, game
				.qhly_getPlayerStatus(state.mainView.avatarImage, null, state.name) == 2);
			// @ts-ignore
			if (game.qhly_getSkin(name)) {
				// @ts-ignore
				var str = game.qhly_getSkin(name).substring(0, game.qhly_getSkin(name).length - 4);
				if (!lib.config.qhly_skinset.djtoggle[name] || lib.config.qhly_skinset.djtoggle[name] && !
					lib.config.qhly_skinset.djtoggle[name][str]) {
					// @ts-ignore
					game.qhly_changeDynamicSkin(subView.avatarImage, str, name);
					// @ts-ignore
					if (subView.avatarImage.dynamic && subView.avatarImage.dynamic.primary) _status
			...
			state.onChangeSkin = function() {
				...
				// @ts-ignore
				subView.characterTitle.innerHTML = get.qhly_verticalStr(lib.qhly_filterPlainText(
					ctitle));
			};
			state.onChangeSkin();
			refreshRank();
			...
			if (!get.infoHujia(hp)) {
				...
				// @ts-ignore
				str += "&nbsp;&nbsp;&nbsp;<img style='height:20px;width:20px;' src='" + lib.qhly_path +
					"theme/decade/shield.png'/>";
	...
	game.qhly_initNewView = function(name, view, page, cplayer) {
		...
		subView.page = {
					...
					if (currentViewSkin.isQiLayout) {
						...
						for (var skill of viewSkill) {
							...
							if (detail) {
								...
								// @ts-ignore
								content +=
									"vertical-align:top;width:100px;height:100px;background-repeat:no-repeat;background-position:top left;background-size:100px 100px;background-image:url(" +
									lib.qhly_path + get.qhly_getIf(currentViewSkin.skillNameImage,
										"theme/shuimo/newui_shuimo_skillname.png") + ");";
								content += "color:";
								if (derivation.includes(skill)) {
									// @ts-ignore
									content += get.qhly_getIf(currentViewSkin
										.skillPageDerivationSkillColor, "#0000ff") + ";";
								} else if (tempSkill.includes(skill)) {
									// @ts-ignore
									content += get.qhly_getIf(currentViewSkin.skillPageTempSkillColor,
										"#00FF00") + ";";
								} else {
									// @ts-ignore
									content += get.qhly_getIf(currentViewSkin.skillPageSkillNameColor,
										"#5B0F00") + ";";
								}
								if (cskill.length <= 2) {
									content += 'font-size:30px;';
									content += 'line-height:' + (lib.config.qhly_hanggaoxiufu ? lib
										.config.qhly_hanggaoxiufu : '250') + '%;';
								...
								content += "'>";
								content += "<img style='width:135px;height:51px;' id='qhly_skillv_" +
									skill + "'/><br><span ";
								var dynamicTranslate = null;
								if (cplayer && lib.config.qhly_skillingame) {
									var dtrans = lib.dynamicTranslate[skill];
						...
						for (var skill of viewSkill) {
							...
							if (detail) {
								...
								if (derivation.includes(skill)) {
									// @ts-ignore
									content += " style='color:" + get.qhly_getIf(currentViewSkin
										.skillPageDerivationSkillColor, "#0000ff") + ";";
								} else if (tempSkill.includes(skill)) {
									// @ts-ignore
									content += " style='color:" + get.qhly_getIf(currentViewSkin
										.skillPageDerivationSkillColor, "#00ff00") + ";";
								} else {
									// @ts-ignore
									content += " style='color:" + get.qhly_getIf(currentViewSkin
										.skillPageSkillNameColor, "#5B0F00") + ";";
								}
								if (cplayer && lib.config.qhly_skillingame) {
									if (!cplayer.hasSkill(skill)) {
										content += "opacity:0.5;"
								...
								if (!currentViewSkin.isLolBigLayout) {
									content +=
										"<img style='vertical-align:middle;width:35px;' id='qhly_skillv_" +
										skill + "'/>";
								}
								content += "</h3>";
								content += "<p";
			...
			skin: {
				...
				onClickSkin: function(num, name, state) {
					...
					game.qhly_setCurrentSkin(name, skin.skinId, function() {
						...
						if ((currentViewSkin.lihuiSupport) && state.pkg.characterLihui) {
							var lihuiPath = state.pkg.characterLihui(name, lib.config
								.qhly_skinset.skin[name]);
							if (lihuiPath) {
						...
						game.qhly_readFileAsText("extension/千幻聆音/data/skininfomodel.txt",
							function(ret, str) {
								if (ret) {
									...
									var path = game.qhly_getSkinImagePath(name, state.pkg);
									// @ts-ignore
									game.qhly_writeTextFile(str, path + name, "skininfo.js",
										function(err) {
											if (!err) {
												alert("保存成功");
												// @ts-ignore
				...
				refreshAfterGot: function(name, state) {
					...
					if (this.firstRefresh) {
						var ret = false;
						for (var i = (currentViewSkin.isLolBigLayout ? -1 : 0); i < this.skinList
							.length; i++) {
							// @ts-ignore
							// @ts-ignore
							// @ts-ignore
					...
					if (!this.hideSkinMode) {
						...
						for (var i = 0; i < 3; i++) {
							...
							if (this.getSkinAt(i)) {
								...
								if (skinId) {
									// @ts-ignore
									currentSkinView.setBackgroundImage(game.qhly_getSkinFile(name, this
										.getSkinAt(i).skinId));
								} else {
					...
					if (!currentViewSkin.isQiLayout && !currentViewSkin.isLolBigLayout) {
						// @ts-ignore
						content += "<h2 style='color:" + get.qhly_getIf(currentViewSkin
								.skinPageHeadTitleColor, "#783f04") + "'>皮肤名称：<span style='color:" + get
							.qhly_getIf(currentViewSkin.skinPageHeadSkinNameColor, "black") + "'>" +
							tname + "</span></h2>";
					}
					var extInfo = "";
					if (currentSkin.skinInfo.info) {
					...
					if (currentViewSkin.isQiLayout) {
						...
						for (var audio of currentSkin.audios) {
							...
							// @ts-ignore
							content +=
								"text-align:center;vertical-align:top;width:100px;height:100px;background-repeat:no-repeat;background-position:top left;background-size:100px 100px;background-image:url(" +
								lib.qhly_path + get.qhly_getIf(currentViewSkin.skillNameImage,
									"theme/shuimo/newui_shuimo_skillname.png") + ");";
							if (audio.id == 'die') {
								...
								// @ts-ignore
								content += "color:" + get.qhly_getIf(currentViewSkin
									.skinPageSkillNameColor, "#4169E1") + ";";
							}
							var cskill;
							if (audio.name) {
								cskill = audio.name;
							...
							if (cskill.length <= 2) {
								content += 'font-size:30px;';
								content += 'line-height:' + (lib.config.qhly_hanggaoxiufu ? lib.config
									.qhly_hanggaoxiufu : '250') + '%;';
							} else if (cskill.length <= 3) {
								content += 'font-size:26px;';
								content += 'line-height:320%;'
							} else if (cskill.length <= 4) {
							...
							content += "<td>";
							// @ts-ignore
							content += "<img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue(
									'skinPagePlayAudioButtonImage',
									'extension/千幻聆音/image/qhly_pic_playaudiobutton.png') +
								"' style='height:40px;' id='qhly_skin_skill_" + audio.id + "'/>"
							// @ts-ignore
							if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
								// @ts-ignore
								content += "<img src='" + lib.qhly_path +
									"image/qhly_editButton.png' style='height:25px;width:25px;'";
						...
						for (var audio of currentSkin.audios) {
							...
							if (audio.id == 'die') {
								...
								// @ts-ignore
								content += "<h3 style='color:" + get.qhly_getIf(currentViewSkin
									.skinPageSkillNameColor, "#4169E1") + "'>【";
							}
							if (audio.name) {
								content += audio.name;
							...
							content += "】"
							// @ts-ignore
							content += "&nbsp;&nbsp;&nbsp;<img src='" + lib.assetURL + get
								.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage',
									'extension/千幻聆音/image/qhly_pic_playaudiobutton.png') +
								"' style='height:23px;vertical-align:middle;' id='qhly_skin_skill_" +
								audio.id + "'/>"
							// @ts-ignore
							if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
								// @ts-ignore
					...
					if (lib.config.qhly_skinconfig) {
						content += "<h2>皮肤配置</h2>";
						content +=
							"<p><span style='display:inline-block;height:30px;'><span id='qhconfig_checkbox_banInRandom_text' style='display:inline-block;position:relative;bottom:25%;'>随机切换禁用&nbsp;&nbsp;</span><img id='qhconfig_checkbox_banInRandom'/></span></p><br>";
						if (currentSkin.skinId) {
							content +=
								"<p><span>皮肤品质&nbsp;&nbsp;</span><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_level_select'></select></p>";
							content +=
						...
						if (img) {
							...
							(function(id) {
								...
								img.listen(function() {
									...
									if (id == 'die') window.qhly_playDieAudio(name);
									// @ts-ignore
									else if (id == 'victory') window.qhly_playVictoryAudio(
										name);
									else {
										// @ts-ignore
										var count = _status.qhly_skillAudioWhich[id];
				...
				init: function(name, state) {
					...
					if (currentViewSkin.isQiLayout) {
						...
						this.skinTitle1.qhText = ui.create.div('.qh-page-skinavatartitle-text', this
							.skinTitle1);
						this.skinTitle2 = ui.create.div('.qh-page-skinavatartitle', this.skinCover2);
						this.skinTitle2.qhText = ui.create.div('.qh-page-skinavatartitle-text', this
							.skinTitle2);
						this.skinTitle3 = ui.create.div('.qh-page-skinavatartitle', this.skinCover3);
						this.skinTitle3.qhText = ui.create.div('.qh-page-skinavatartitle-text', this
							.skinTitle3);
					...
					this.skinViews = [this.skinBoard1, this.skinBoard2, this.skinBoard3, this.skin1,
						this.skin2, this.skin3, this.leftArrow, this.rightArrow
					];
		...
		// @ts-ignore
		subView.pageButton[page].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage,
			'extension/千幻聆音/image/newui_button_selected.png'));
		view.appendChild(subView.mp);
		var state = {
			name: name,
			currentPage: page,
		...
		var showPage = function(pagename) {
			...
			for (var k in subView.pageButton) {
				if (k == pagename) {
					// @ts-ignore
					subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin
						.buttonPressedImage, 'extension/千幻聆音/image/newui_button_selected.png'));
				} else {
					// @ts-ignore
					subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonImage,
						'extension/千幻聆音/image/newui_button.png'));
				}
			}
		};
		state.useLihui = function() {
			...
			if (use) {
				if (currentViewSkin.isLolBigLayout) {
					if (state.currentPage == 'skin') {
						...
						subView.board.setBackgroundImage(
							"extension/千幻聆音/theme/lolbig/newui_lol_bg_center_big.png");
						...
						subView.board.setBackgroundImage(
							"extension/千幻聆音/theme/lolbig/newui_lol_bg1_big.png");
				...
				if (currentViewSkin.isLolBigLayout) {
					if (state.currentPage == 'skin') {
						...
						subView.board.setBackgroundImage(
							"extension/千幻聆音/theme/lolbig/newui_lol_bg_center_small.png");
		...
		var refreshView = function(state, subView) {
			...
			state.onChangeSkin = function() {
				...
				if (currentViewSkin.isQiLayout) {
					if (!lib.config.qhly_titlereplace || lib.config.qhly_titlereplace == 'title') {
						...
						if (pname) {
							subView.characterTitle.innerHTML = get.translation(pname +
								"_character_config");
						}
					}
				} else {
				...
				if (currentViewSkin.isQiLayout || currentViewSkin.isLolBigLayout) {
					if (currentViewSkin.isLolBigLayout) {
						...
						if (lib.qhly_groupimage[group]) {
							var groupHtml =
								"<img style='display:block;position:absolute;width:65%;height:65%;left:50%;top:50%;transform:translate(-50%,-50%);' ";
							// @ts-ignore
			...
			let getTranslation = (name) => {
				if (!get.slimNameHorizontal && pattern != 'raw') {
					...
					switch (pattern) {
						case "full":
							return currentViewSkin.isQiLayout ? get.slimNameHorizontal(name) : get
								.slimName(name);
						case "full_pure":
							// @ts-ignore
			...
			if (currentViewSkin.isQiLayout || currentViewSkin.isLolBigLayout) {
				...
				if (!get.infoHujia(hp)) {
					...
					// @ts-ignore
					str += "&nbsp;&nbsp;&nbsp;<img style='height:40px;weight:40px;' src='" + lib.qhly_path +
						"image/qhly_hudun.png'/>";
					if (get.infoHujia(hp) > 1) {
						str += ("x" + get.infoHujia(hp));
					}
					subView.hpText.innerHTML = str;
	...
	game.qhly_open = function(name, page, ingame) {
		...
		var refreshView = function(name, viewAbstract) {
			...
			if (skills) {
				...
				for (var skill of skills) {
					...
					if (window.qhly_TrySkillAudio) {
						// @ts-ignore
						infoString +=
							"<a style='color:#ffffff' href=\"javascript:window.qhly_TrySkillAudio('" +
							skill + "',{name:'" + name + "'},null,window.qhly_audio_which[\'" + skill +
							"\'],\'" + viewAbstract.skin + "\');window.qhly_audio_which[\'" + skill +
							"\']++;\"><img style=height:22px src=" + lib.assetURL + get
							.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage',
								'extension/千幻聆音/image/qhly_pic_playaudiobutton.png') + "></a><br>";
					}
					infoString += "<br><br>";
					str += infoString;
				}
			...
			str += "【阵亡】";
			// @ts-ignore
			str += "<a style='color:#ffffff' href=\"javascript:window.qhly_playDieAudio(\'" + name +
				"\');\"><img style=height:22px src=" + lib.qhly_path +
				"image/qhly_pic_playaudiobutton.png></a><br>";
			if (info.info) {
				str += info.info;
			}
			if (!viewAbstract.skin) {
	...
	var replaceCharacterCardFunction = function() {
		var clickListener = function() {
			...
			if (arguments[4]) {
				if (lib.config.qhly_replaceCharacterCard2 == 'window') {
					...
					// @ts-ignore
					game.qhly_open_new(arguments[0], lib.config.qhly_doubledefaultpage ? lib.config
						.qhly_doubledefaultpage : 'skill', arguments[4]);
				} else {
					// @ts-ignore
				...
				if (lib.config.qhly_currentViewSkin != 'jingdian') {
					// @ts-ignore
					game.qhly_open_new(arguments[0], lib.config.qhly_listdefaultpage ? lib.config
						.qhly_listdefaultpage : 'introduce');
				} else {
					// @ts-ignore
					game.qhly_open(arguments[0]);
				}
		...
		if (lib.config.qhly_replaceCharacterCard2 != 'nonereplace' && lib.config.qhly_replaceCharacterCard2 !=
			'nonereplace2') {
					...
					var func = function(arr, f) {
						...
						game.qhly_getSkinModels(n, function(list) {
							if (list && list.length) {
								...
								// @ts-ignore
								var player = game.filterPlayer(function(c) {
									return c.name1 == n || c.name2 == n
								})[0];
								if (list.includes(sk)) {
									list.remove(sk);
								}
								...
								// @ts-ignore
								if (!_status.qhly_open && !_status.bigEditing && !_status
									.qhly_playerWindowing) game.qhly_setCurrentSkin(n, list
									.randomGet(),
									function() {
										// @ts-ignore
										game.qhly_changeDynamicSkin(n);
										if (player && lib.config[
												'extension_千幻聆音_qhly_decadeChangeEffect'] &&
	...
	lib.skill._qhly_autoc = {
		...
		filter: function(event, player) {
			// @ts-ignore
			return !_status.qhly_autoChangeSkinSetted && lib.config.qhly_autoChangeSkin && lib.config
				.qhly_autoChangeSkin != 'close';
		},
		content: function() {
			// @ts-ignore
			...
			if (player.name || player.name1) {
				...
				game.qhly_getSkinModels(player.name ? player.name : player.name1, function(list) {
					if (list && list.length) {
						...
						list = list.filter(function(current) {
							// @ts-ignore
							return !game.qhly_skinIsBanned(player.name ? player.name :
								player.name1, current);
						});
						if (list && list.length)
							// @ts-ignore
			...
			if (player.name2) {
				...
				game.qhly_getSkinModels(player.name2, function(list) {
					if (list && list.length) {
						...
						if (list && list.length)
							// @ts-ignore
							game.qhly_setCurrentSkin(player.name2, list.randomGet(), undefined,
								true);
					}
				}, false);
			}
