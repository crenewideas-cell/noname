import { configure, connectCore } from './compat.js';
...
let mainPackageFunc = async function(){
  configure();
  CONFIG.qhly_currentViewSkin.init=lib.config.qhly_currentViewSkin;
  CONFIG.qhly_replaceCharacterCard2.init=lib.config.qhly_replaceCharacterCard2;
  let started=false;
  const start=(config,pack)=>{
    if(started)return;
    CONTENT(config,pack);
    connectCore();
    started=true;
  ...
  let mainPackage = {
    name: "千幻聆音", content:start, precontent:PRECONTENT, config:CONFIG, help: {}, package: {
      character: {
        character: {
        },
        translate: {
