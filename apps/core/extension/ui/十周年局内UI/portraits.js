...
export function mountPortraits({base,metadata,enabled,staticSelected=()=>false,subscribe=()=>()=>{}}) {
 const allowed=avatar=>enabled()&&!staticSelected(avatar.dataset.skinCharacter);
 ...
 function prepare(){
  ...
  renderer.render=function(time){
   if(disposed)return;
   try {
    for(const [avatar,entry] of entries){const r=avatar.getBoundingClientRect();entry.rect=r;const shown=allowed(avatar)&&visible(avatar)&&entry.character===avatar.dataset.skinCharacter;for(const sprite of entry.sprites){sprite.opacity=shown?1:0;sprite.clip={x:r.left,y:innerHeight-r.bottom,width:r.width,height:r.height};}if(!shown)entry.context.clearRect(0,0,entry.canvas.width,entry.canvas.height);}
    draw.call(this,time);
    for(const [avatar,entry] of entries){if(!allowed(avatar)||!visible(avatar)||entry.character!==avatar.dataset.skinCharacter)continue;const r=entry.rect;if(!r.width||!r.height)continue;entry.canvas.width=Math.ceil(r.width);entry.canvas.height=Math.ceil(r.height);entry.context.clearRect(0,0,r.width,r.height);entry.context.drawImage(this.canvas,r.left,r.top,r.width,r.height,0,0,r.width,r.height);}
 ...
 async function add(avatar,character,skin){
  ...
  try{
   ...
   for(const layer of [skin.beijing,skin].filter(Boolean)){
    ...
    if(disposed||entry.disposed||renderer!==current||!allowed(avatar)||!visible(avatar)||avatar.dataset.skinCharacter!==character){remove(avatar);return;}
 ...
 function scan(){
  ...
  for(const [avatar,entry] of entries)if(!allowed(avatar)||!visible(avatar)||entry.character!==avatar.dataset.skinCharacter)remove(avatar);
  if(!enabled())return;
  for(const avatar of document.querySelectorAll('#arena>.player>.avatar,#arena>.player>.avatar2')){
   if(!allowed(avatar)||!visible(avatar))continue;const character=avatar.dataset.skinCharacter,skin=Object.values(metadata.skins[character]||{})[0];
 ...
 return()=>{disposed=true;unsubscribe();observer.disconnect();cancelAnimationFrame(frame);window.removeEventListener('resize',resize);for(const avatar of entries.keys())remove(avatar);for(const cancel of pending)cancel();pending.clear();destroy();loads.clear();};
}
