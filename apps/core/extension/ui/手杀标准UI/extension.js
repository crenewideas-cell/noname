...
function openNativeSettings(runtime){
 ...
 for(const [key,name] of [['rzsh_cards','手杀卡牌插画'],['rzsh_mvp','结算统计面板']]){
...
function createScene(runtime,node,resolve){
 ...
 const scene={screen,
  ...
  async finish(mode){
   ...
   finishing=true;
   try{await runtime.prepareGame();await game.promises.saveConfig('sessionType','offline');resolve(mode);}
  ...
  dispose(){if(closed)return;closed=true;observer?.disconnect();window.removeEventListener('message',onMessage);timers.forEach(clearTimeout);intervals.forEach(clearInterval);frames.forEach(cancelAnimationFrame);tweens.forEach(tween=>tween.kill());tweens.clear();for(const resource of resources)resource.destroy?.();for(const application of apps)if(application.renderer)application.destroy(true,{children:true});runtime.bridge.releaseScene(scene);iframe?.remove();toolbar.remove();loading.remove();},
...
export async function activate(manifest){
 ...
 let runtime,preparing,disposed=false;
 ...
 async function prepare(){
  if(!preparing)preparing=import('./native-runtime.js').then(module=>module.createNativeRuntime(manifest)).then(value=>{
   ...
   runtime=value;activeRuntime=value;return value;
