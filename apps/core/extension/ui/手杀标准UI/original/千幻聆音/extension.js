game.import("extension", function (lib, game, ui, get, ai, _status) {
  ...
  if(lib.config.qhly_lutou==undefined) {
      game.saveConfig('extension_千幻聆音_qhly_lutou', true);
      game.saveConfig('qhly_lutou', true);
  }
  ...
  //Helasisy：我糙我简直就是甜菜
  game.createCss(`.qh_lutou_refreshview:not(.d-skin) {
      height: 104% !important;
      background-position: center top;
  }`);
  game.qhIsNotPause=function(){
      return typeof _status=='object'&&!_status.isQhPaused;
  };
  //出框调整
  //皮切用自己函数播放出框
  ...
  window.qhly_extension_package = {
    ...
    name: "千幻聆音", content: function (config, pack) {
      //查看是否存在某扩展，用来处理兼容事宜
      if(lib.config.qhly_funcLoadInPrecontent === undefined) {
          lib.config.qhly_funcLoadInPrecontent=true;
          game.saveConfig('extension_千幻聆音_qhly_funcLoadInPrecontent', true);
          game.saveConfig('qhly_funcLoadInPrecontent', true);
      }
            ...
            skinSwitch.dynamic.transformDst = function (player, isPrimary, dstInfo, extraParams = { isOrigin: false, huanfuEffect: null }) {
              ...
              if (dstInfo.name == null || dstInfo.name === avatar.name) {
                ...
                if (dstInfo.background) {
                  player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + dstInfo.background + '")';
                }
            ...
            EpicFX.playDynamic = (player, character, character2, play, transform, cutdybg) => {
              ...
              if (showDynamic && _status.mode != null) {
                ...
                for (var i = 0; i < avatars.length; i++) {
                  ...
                  // player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background + '")';
                  if (!skinCopy.dynamicBackground) player.$dynamicWrap.style.backgroundImage = `url(${lib.assetURL}extension/手杀标准UI/original/十周年UI/assets/dynamic/${skinCopy.background}`;
                  if (!increased) {
                    increased = true;
      ...
      var qhly_oldinit = function (character, character2, skill) {
        ...
        lib.setIntro(this);
        this.node.name.innerHTML = get.prefixBoldName(character,'character');
        if (this.classList.contains('minskin') && this.node.name.querySelectorAll('br').length >= 4) {
          this.node.name.classList.add('long');
        }
        var infoArray = [false, false];
        if (info[4].contains('hiddenSkill') && !this.noclick) {
          infoArray[0] = true;
          if (!this.hiddenSkills) this.hiddenSkills = [];
          this.hiddenSkills_1 = [];
          this.hiddenSkills.addArray(skills);
          this.hiddenSkills_1.addArray(skills);
          skills = [];
          ...
          if(this.hiddenSkills_1?.length) {
            this.hiddenSkills.removeArray(this.hiddenSkills_1);
          }
          this.hiddenSkills_1 = [];
          this.classList.remove('unseen');
          this.classList.remove('unseen_v');
        }
        if (character2 && lib.character[character2]) {
          ...
          if (info2[4].contains('hiddenSkill') && !this.noclick) {
            ...
            this.hiddenSkills_2.addArray(info2[3]);
            this.classList.add(_status.video ? 'unseen2_v' : 'unseen2');
            this.storage.nohp = true;
            skills.add('g_hidden_ai');
          }else {
            skills = skills.concat(info2[3]);
            if(this.hiddenSkills_2?.length) {
              this.hiddenSkills.removeArray(this.hiddenSkills_2);
            }
            this.hiddenSkills_2 = [];
            this.classList.remove('unseen2');
            this.classList.remove('unseen2_v');
          }

          this.node.name2.innerHTML = get.prefixBoldName(character2,'character');
        }
        //Helasisy修：隐匿技可逆化
        if(!infoArray[0]&&!infoArray[1]){
          this.storage.nohp = false;
          skills.remove('g_hidden_ai');
        }
      ...
      function qhly_init(character, character2, skill) {
          ...
          if (showDynamic && _status.mode != null) {
            ...
            for (var i = 0; i < avatars.length; i++) {
              ...
              if (lib && lib.config && lib.config.qhly_skinset && lib.config.qhly_skinset.djtoggle && lib.config.extensions && lib.config.extensions.contains('千幻聆音') && lib.config['extension_千幻聆音_enable']) {
                ...
                if (value) value = value.substring(0, value.lastIndexOf('.'));
                //if (value) value = game.qhly_rename(value);
                else {
                  value = '经典形象';
                  realName = avatars[i];
                }
              ...
              if (i == 0 || !this.dynamic.primary) {
                if (skinCopy.background) {
                  this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background + '")';
                } else {
                  this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/皮肤切换/images/card/card.png")'
                }
              }
              // 修改3 end
              if (!increased) {
        ...
        this.node.avatar.addEventListener(lib.config.touchscreen ? "touchend" : "mouseup", function (e) {
          ...
          timer = setTimeout(function () {
            if(this.parentNode&&this.parentNode.cailanzi) {
              this.parentNode.cailanzi();
            }else {
      ...
      function qhly_reinit(from, to, maxHp, online) {
        ...
        if (this.name2 == from) {
          ...
          this.node.name2.innerHTML = get.prefixBoldName(to,'character');
          if (window.qhly_newDynamicExt) {
            data.zhu = false;
            data.to = to;
          ...
          this.node.name.innerHTML = get.prefixBoldName(to,'character');
        ...
        if (!window.qhly_newDynamicExt) {
          ...
          if (this.doubleAvatar) {
            ...
            if (skin) {
              ...
              decadeUI.CUR_DYNAMIC++;
              this.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skin.background + '")';
            }
            var forces = 'qun';
      ...
      function qhly_playdynamic(animation, deputy, cutdybg, ignoreClip) {
        ...
        const dybgxishu = (lib.config.extension_千幻聆音_qhly_lutouType && lib.config.extension_千幻聆音_qhly_lutouType == 'shousha') ? 0.873 : 0.9;
        if (this.doubleAvatar) {
      ...
      if (lib.config.qhly_jianghun === undefined) lib.config.qhly_jianghun = 100000;
      ...
      var qhly_mvp = function (result) {
        ...
        for (var i = 0; i < mvpplayers.length; i++) {
          ...
          for (j = 0; j < mvpplayers[i].stat.length; j++) {
            ...
            /*if (JSON.stringify(mvpplayers[i].stat[j].skill) != '{}')*/
            if (isEmpty(mvpplayers[i].stat[j].skill)) score += Object.keys(mvpplayers[i].stat[j].skill).length;
      ...
      game.qhly_changeDynamicSkin = function (str, name, character, character2, play) {          //str主体，如果为字符串是名称中带有str的所有玩家，name指定更换为某种动皮，character玩家姓名id，character2是否为副将
        ...
        for (var dy = 0; dy < nodes.length; dy++) {
          ...
          if (_status.mode != null) {
            ...
            //太虚幻境时没有game.me的bug
            var gamemeoff=game.me?game.me.offsetHeight:216;
            skinCopy.qhly_resizeRatio = node.offsetHeight * (1 / gamemeoff);
            // 动态背景切割 4 -> 判断提前 end
            ...
            if (window.qhly_newDynamicExt) {
              ...
              node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background + '")';
              game.qhly_checkYH(node);
              // player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background + '")';
              if (!skinCopy.dynamicBackground) { } node.$dynamicWrap.style.backgroundImage = `url(${lib.assetURL}extension/手杀标准UI/original/十周年UI/assets/dynamic/${skinCopy.background}`;
              if (!increased) {
                increased = true;
                decadeUI.CUR_DYNAMIC++;
              }
              ...
              if (skinCopy.background) {
                if (node.doubleAvatar && ui.arena.dataset.newDecadeStyle == 'on') {
                  ...
                  if (character2) {
                    background1 = node.dynamic.primary && node.dynamic.primary != null && node.dynamic.primary.background ? (lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + node.dynamic.primary.background) : '';
                    background2 = lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background;
                  } else {
                    background1 = lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background;
                    background2 = node.dynamic.deputy && node.dynamic.deputy != null && node.dynamic.deputy.background ? (lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + node.dynamic.deputy.background) : '';
                  }
                ...
                else node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/十周年UI/assets/dynamic/' + skinCopy.background + '")';
              } else {
                node.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL + 'extension/手杀标准UI/original/皮肤切换/images/card/card.png")'
              }
              game.qhly_checkYH(node);
              // 修改9 start
              if (game.qhly_hasExtension('皮肤切换') && lib.config[skinSwitch.configKey.useDynamic]) {
      ...
      lib.element.player.playChangeSkinEffect = function (avatar2, first) {
        //alert(window.notPlayChangeSkinEffect);
        //展开其他窗口时不展示特效
        if(window.notPlayChangeSkinEffect) return;
        //大窗口被占用时也不展示
        if(!game.qhIsNotPause()) return;
        ...
        //所有同名武将一起换（加特效）
        var me=this;
        if(!first) allPlayers.forEach(function(current){
           if(current==me) return;
           if(current.name1==name) {
               current.playChangeSkinEffect(false,true);
           }else if(current.name2==name) {
               current.playChangeSkinEffect(true,true);
           }
        });
      ...
      window.qhly_openPluginWindow = function(){
        ...
        var qhv = win.qhv;
        qhv.button.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/image/qhly_ok2.png');
      ...
      game.qhly_setOriginSkin = function (name, skin, node, state, skin2, defaultBlur) {
        if (skin != null) {//国战兼容
        ...
        image.onload = function () {
          ...
          if(!defaultBlur&&lib.config.extension_千幻聆音_lutouFixx&&lib.config.extension_千幻聆音_lutouFixx=='all'&&node.classList.contains('qh-image-lutou')) {
            node.classList.add('qh_lutou_refreshview');
          }else {
            node.classList.remove('qh_lutou_refreshview');
          }
          if (theme) {
      ...
      game.qhly_playerWindow = function (node) {
        ...
        if (window.all_cailanzi) return;
        const player = node.parentNode;
        //const playerName = node.className == 'primary-avatar' ? player.name1 : player.name2;
        //终于修好了
        const playerName = node.className.indexOf('primary-avatar')!=-1 ? player.name1 : player.name2;
        ...
        if (ui.arena.dataset.newDecadeStyle != 'on' && player.dynamic && (player.dynamic.primary != null || player.dynamic.deputy != null)) blackbg.style.cssText += 'top:-11%;width:104.7%;height:114.5%;border-radius: 300px 182px 30px 30px/80px 65px 30px 30px;'
      ...
      game.qhly_setDoubleGroup = function (state) {
        ...
        switch (group) {
          //“西”势力的修改
          //case 'western': path = 'detail_cong'; group = 'detail_cong'; break;
          case 'western': case 'shu': case 'wu': case 'wei': case 'qun': case 'jin': case 'key': case 'shen': case 'devil': case 'ye': path = group; break;
      ...
      if (game.getFileList && lib.config.qhly_lihuiSupport) {
        game.qhly_checkFileExist('extension/手杀标准UI/original/千幻聆音/lihui', function (s) {
          ...
          if (s) {
            game.getFileList('extension/手杀标准UI/original/千幻聆音/lihui', function (folders, files) {
              if (files) {
      ...
      if (get.mode() == 'guozhan' && game.getFileList && lib.config.qhly_guozhan !== false) {
        var cpath = lib.config.qhly_originSkinPath == 'extension/手杀标准UI/original/千幻聆音/sanguolutouskin/' ? 'extension/手杀标准UI/original/千幻聆音/sanguolutouskin' : 'extension/手杀标准UI/original/千幻聆音/sanguoskin';
        ...
        game.qhly_checkFileExist('extension/手杀标准UI/original/千幻聆音/sanguoaudio', function (s) {
          if (s) {
            game.getFileList('extension/手杀标准UI/original/千幻聆音/sanguoaudio', function (folders, files) {
      ...
      game.qhly_parseConfig = function (obj) {
        ...
        var str = "";
        var image = obj.image ? obj.image : "extension/手杀标准UI/original/千幻聆音/image/qhly_pic_config.png";
        ...
        characterLihui: function (name, skin) {
          ...
          if (skin) {
            if (lib.qhly_lihui[name + '-' + skin]) {
              return 'extension/手杀标准UI/original/千幻聆音/lihui/' + lib.qhly_lihui[name + '-' + skin];
            }
            if (!lib.config.qhly_nolihuiOrigin) return null;
          }
          if (lib.qhly_lihui[name]) {
            return 'extension/手杀标准UI/original/千幻聆音/lihui/' + lib.qhly_lihui[name];
          }
          return null;
        },
        prefix: 'image/character/',//武将原图在image/character内
        skin: {
          standard: (lib.config.qhly_originSkinPath ? lib.config.qhly_originSkinPath : 'extension/手杀标准UI/original/千幻聆音/sanguoskin/'),//皮肤图片在千幻聆音扩展内的位置。
          origin: 'extension/手杀标准UI/original/千幻聆音/sanguoyuanhua/',//皮肤原画在千幻聆音扩展内的位置。
        },
        //ssborder: 'shousha/',//手杀边框
        audioOrigin: '',
        audio: 'extension/手杀标准UI/original/千幻聆音/sanguoaudio/',//皮肤配音文件在千幻聆音扩展内的位置。
      };
      ...
      lib.qhly_groupimage['wei'] = 'extension/手杀标准UI/original/千幻聆音/image/name_wei.webp';
      lib.qhly_groupimage['shu'] = 'extension/手杀标准UI/original/千幻聆音/image/name_shu.webp';
      lib.qhly_groupimage['wu'] = 'extension/手杀标准UI/original/千幻聆音/image/name_wu.webp';
      lib.qhly_groupimage['qun'] = 'extension/手杀标准UI/original/千幻聆音/image/name_qun.webp';
      lib.qhly_groupimage['jin'] = 'extension/手杀标准UI/original/千幻聆音/image/name_jin.webp';
      lib.qhly_groupimage['shen'] = 'extension/手杀标准UI/original/千幻聆音/image/name_shen.webp';
      lib.qhly_groupimage['daqin'] = 'extension/手杀标准UI/original/千幻聆音/image/name_daqin.webp';
      lib.qhly_groupimage['key'] = 'extension/手杀标准UI/original/千幻聆音/image/name_key.webp';
      ...
      game.qhly_bigEdit = function (state, bg) {
        ...
        //Helasisy修：如真列表编辑动皮报错
        let themeType = 'shousha';
        if(ui.arena) themeType = ui.arena.dataset.newDecadeStyle == 'on' ? 'decade' : 'shousha';
        ...
        if (!bg) {
          if(editObject == 'player' && game.createCss) {
            game.createCss(`.qhly_dynamicSkinFixed {
              z-index: 35 !important;
              top: calc(50% - ${focus.offsetHeight * 0.5}px) !important;
              left: calc(30% - ${focus.offsetWidth * 0.5}px) !important;
              pointer-events: none;
            ...
            focus.classList.add('qhly_dynamicSkinFixed');
            focus.parentNode.appendChild(blackbg);
            blackbg.classList.add('qhly_blackbgFixed');
          }else {
          ...
          buttons[i].addEventListener(lib.config.touchscreen ? 'touchstart' : 'mousedown', function () {
            ...
            switch (this.id) {
              ...
              case 'qhly_bigedit6': {
                ...
                game.qhly_readFileAsText("extension/手杀标准UI/original/千幻聆音/data/skineditmodel.txt", function (ret, str) {
                  if (ret) {
                    str = str.replace("_REPLACE_OBJECT_", strobj);
                    game.qhly_writeTextFile(str, "extension/手杀标准UI/original/千幻聆音", "skinEdit.js", function (err) {
                      if (!err) {
                        console.log("保存成功");
                      } else {
        ...
        function exit() {
          ...
          //清除这个毛病
          if (!bg) {
            if(focus.classList.contains('qhly_dynamicSkinFixed')) {
              focus.classList.remove('qhly_dynamicSkinFixed');
            }else {
      ...
      HTMLDivElement.prototype.setBackgroundImage = function (name) {
        ...
        //     return;
        // }
        if(lib.config.qhly_chooseButtonOrigin||(lib.config['extension_千幻聆音_qhly_chooseButtonOrigin_guozhan']&&get.mode()=='guozhan')){
        ...
        if (this.classList.contains('qh-must-replace') || (!this.classList.contains('qh-not-replace') && (lib.config.qhly_forceall || (this.classList.contains('avatar') || this.classList.contains('avatar2'))))) {
          ...
          var setByName = function (cname, opath) {
            if (lib.config.qhly_skinset.skin[cname]) {
              ...
              //Helasisy修：woc终于找到你了把我gz_caocao国战皮肤弄没的罪灰祸首
              var destpath = dest + game.qhly_getRealName(cname) + "/" + skin;
      ...
      //标记一个异步（看我加一个promise，有更好的想法，先拿着）
      //Helasisy修：我简直就是个甜菜
      //思路，A和B先后放进来，但是跑着去打饭，所以最后谁先打到饭顺序未知
      HTMLDivElement.prototype.setBackground = function (name, type, ext, subfolder) {
        //让B写名字到桌上（A先进先写，B后进把A的盖住了）
        this.currentSkinName = name;
        //放变量里方便传递
        let avatarMe = this;
        //留住我的卡牌武将！只要是卡牌类的图片一律送到原版处理器那里，那里有人接收
		if(type&&type=='character'&&get.type(this)!=undefined&&!this.style['backgroundImage']) {
        ...
        if (type == 'character' && (this.classList.contains('qh-must-replace') || (!this.classList.contains('qh-not-replace') && (lib.config.qhly_forceall || (this.classList.contains('avatar') || this.classList.contains('avatar2') || this.classList.contains('primary-avatar') || this.classList.contains('deputy-avatar') || this.classList.contains('button')))))) {
          ...
          var setByName = function (cname) {
            if (lib.config.qhly_skinset.skin[cname]) {
              ...
              //这个老登整了个异步，就是可以乱插队了，这不好
              game.qhly_checkFileExist(destpath, function (s) {
                //呐，规矩：如果你打到饭时桌子上不是写你的名字（最后一个进的人），你不准上桌
                if(avatarMe.currentSkinName != name) {
                    return false;
                }
                if (s) {
                  ...
                  else that.qhly_origin_setBackgroundImage('extension/手杀标准UI/original/千幻聆音/image/noSkin.png');//noskin
                }
              })
      ...
      //皮肤名字格式修正
      game.qhly_rename = function(name,all) {
          var name2=name;
          if(all==true) {
              name2=name2.replace('2','Ⅱ');
              return name2;
          }
          //if(name2.indexOf('2')!=0) name2=name2.replace('2','');
          if(name2.length>4) name2=name2.slice(0,4);
          return name2;
      }
      //获取皮肤名称。
      ...
      game.qhly_foundPackageExt = function (name) {
        ...
        if (cp) {
          ...
          return {
            ...
            audio: ((lib.config.qhly_extSkinPath == undefined || lib.config.qhly_extSkinPath == 'default') ? 'extension/' + extNameInPic + '/skin/audio/' : 'extension/手杀标准UI/original/千幻聆音/sanguoaudio/'),//皮肤配音文件在千幻聆音扩展内的位置。
          };
        }
        return null;
      };
      ...
      game.qhly_dom2image = function (from, name, node, path, state) {
        ...
        else if (lib.config.extension_千幻聆音_qhly_lutouType && lib.config.extension_千幻聆音_qhly_lutouType == 'shousha') {
        ...
        var state = {
          ...
          refreshLocation: function (view) {
            ...
            if (skin == currentSkinId || (!skin && !currentSkinId)) {
              view.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_skin_background_shuimo.png');
            } else {
        ...
        try {
          ...
          game.qhly_getSkinList(name, function (ret, list) {
            ...
            for (var i = 0; i < skinList.length; i++) {
              ...
              //skinView.belowText.innerHTML=game.qhly_rename(skinView.belowText.innerHTML);
              if (game.qhly_skinIs(name, skin)) {
                skinView.style.filter = "saturate(100%)";
                skinView.belowText.style.textShadow = '.2rem 0rem .5rem red,-.2rem 0rem .5rem red,0rem .2rem .5rem red,0rem -.2rem .5rem red';
              } else {
      ...
      if(!lib.config.qhly_autoChangeSkin) {
         lib.config.qhly_autoChangeSkin="random";
         game.saveConfig('extension_千幻聆音_qhly_autoChangeSkin', 'random');
         game.saveConfig('qhly_autoChangeSkin', 'random');
         lib.config.qhly_smallwinclosewhenchange=true;
         game.saveConfig('extension_千幻聆音_qhly_smallwinclosewhenchange', true);
         game.saveConfig('qhly_smallwinclosewhenchange', true);
      }
      ...
      game.qhly_open_small_decade = function (name, from, ingame) {
        try {
          ...
          if (lib.config.extension_千幻聆音_qhly_lutouType && lib.config.extension_千幻聆音_qhly_lutouType == 'shousha') dialog.classList.add('shousha');
          ...
          game.qhly_getSkinList(namex, function (ret, list) {
            ...
            for (let i = 0; i < skinList.length; i++) {
              ...
              avatarName.innerHTML = get.prefixBoldName(name,'justView').replace(/<br>/g, '\n');
              ...
              if (level) {
                ...
                var img = null;
                if (map[level]) {
                  img = "extension/手杀标准UI/original/千幻聆音/theme/decade/dc_" + map[level] + ".png";
                } else if (level.indexOf("#") == 0) {
                  var l2 = level.replace("#", "");
                  img = "extension/手杀标准UI/original/千幻聆音/image/" + l2 + ".png";
                } else if (level.indexOf("$") == 0) {
                  var l2 = level.replace("$", "");
                  img = l2;
                }
              ...
              if (skin) {
                ...
                game.qhly_checkFileExist(file, function (s) {
                  if (s) {
                    ...
                    else skinView2.qhly_origin_setBackgroundImage('extension/手杀标准UI/original/千幻聆音/image/noSkin.png');//noskin
                  }
                })
              } else {
          ...
          if (cPlayer && cPlayer.name2) {
            ...
            game.qhly_getSkinList(namey, function (ret, list) {
              ...
              for (let i = 0; i < skinList.length; i++) {
                ...
                avatarName.innerHTML = get.prefixBoldName(namey,'justView').replace(/<br>/g, '\n');
                var hpWrap = ui.create.div('.qhhp-wrap', skinView);
                hpWrap.setAttribute("data-border-level", playerDengjie);
                skinView.belowText = ui.create.div('.qh-skinchange-decade-skin-text', hpWrap);
      ...
      game.qhly_open_small_shousha = function (name, from, ingame) {
        try {
          ...
          if (lib.config.extension_千幻聆音_qhly_lutouType && lib.config.extension_千幻聆音_qhly_lutouType == 'shousha') dialog.classList.add('shousha');
          if (lib.config.qhly_lutou) {
              //dialog.setAttribute('data-outcrop-skin', 'on');
              if(lib.config.extension_千幻聆音_lutouFixx&&lib.config.extension_千幻聆音_lutouFixx!='off') {
                  dialog.setAttribute('data-outcrop-skin', 'fix');
              }else {
                  dialog.setAttribute('data-outcrop-skin', 'on');
              }
          }
          ...
          game.qhly_getSkinList(namex, function (ret, list) {
            ...
            for (let i = 0; i < skinList.length; i++) {
              ...
              avatarName.innerHTML = get.prefixBoldName(namex,'justView').replace(/<br>/g, '\n');
              ...
              if (level) {
                ...
                var img = null;
                if (map[level]) {
                  img = "extension/手杀标准UI/original/千幻聆音/image/" + map[level] + ".png";
                } else if (level.indexOf("#") == 0) {
                  var l2 = level.replace("#", "");
                  img = "extension/手杀标准UI/original/千幻聆音/image/" + l2 + ".png";
                } else if (level.indexOf("$") == 0) {
      ...
      game.qhly_open_new = function (name, page, ingame) {
        ...
        try {
          ...
          if(typeof txhj=='object'&&txhj.servant) txhj.servant.hide();
          //if(name.indexOf('gz_') == 0){
          //    name = name.slice(3);
          //}
          var cplayer = null;
          ...
          backButton.listen(function () {
            //如果有开着菜单就呼出菜单
            setTimeout(function(){
                game.noReturnMenu=false;
            },500);
            game.qhly_playQhlyAudio(lib.config.qhly_currentViewSkin == 'decade' ? 'qhly_voc_dec_press' : 'qhly_voc_press', null, true);
            if(typeof txhj=='object'&&txhj.servant) txhj.servant.show();
            gback.delete(500, function () {
              lib.onresize.remove(resize);
              if(!window.hasMenu) game.resume2();
              _status.qhly_open = false;
              if(typeof game.qhly_refresh_temp=='function') {
                  game.qhly_refresh_temp();
                  game.qhly_refresh_temp = undefined;
              }
            });
      ...
      ui.qhly_initCheckBox = function (view, checked, forbid) {
        ...
        if (view.qhly_checked) {
          view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxCheckedImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_checked.png');
        } else {
          view.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png');
        }
        view.qhly_setChecked = function (checked, trigger) {
          ...
          if (checked != this.qhly_checked) {
            ...
            if (this.qhly_checked) {
              this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxCheckedImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_checked.png');
            } else {
              this.src = lib.assetURL + get.qhly_getIf(lib.qhly_viewskin[lib.config.qhly_currentViewSkin].checkBoxImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png');
            }
      ...
      game.qhly_initDecadeView = function (name, view, page, cplayer) {
        ...
        if (lib.config.extension_千幻聆音_qhly_lutouType && lib.config.extension_千幻聆音_qhly_lutouType == 'shousha') subView.avatar.classList.add('shousha');
        ...
        //skinStr=game.qhly_rename(skinStr);
        ...
        subView.page = {
              ...
              for (let skill of viewSkill) {
                ...
                if (detail) {
                  (function (skill) {
                    ...
                    if (img) {
                      img.src = lib.assetURL + get.qhly_getCurrentViewSkinValue('skillPagePlayAudioButtonImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_playaudio.png');
          ...
          skin: {
            ...
            editOpen: function (name, skin, skill, state) {
              ...
              game.qhly_editDialog(title, detail, initValue, function (value, dialog) {
                ...
                game.qhly_readFileAsText("extension/手杀标准UI/original/千幻聆音/data/skininfomodel.txt", function (ret, str) {
                  if (ret) {
                    str = str.replace("_REPLACE_OBJECT_", strobj);
            ...
            refreshAfterGot: function (name, state) {
              ...
              if (this.firstRefresh) {
                ...
                for (var i = 0; i < this.skinList.length; i++) {
                  ...
                  //skinView.belowText.innerText=game.qhly_rename(skinView.belowText.innerText);
          ...
          config: {
            ...
            init: function (name, state) {
              ...
              var that = this;
              var content = "";
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('favouriteImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_fav.png') + "' style='width:50px;margin-bottom:-4px;'/>收藏设置</h2>";
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('forbidImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_forbid.png') + "' style='width:50px;margin-bottom:-4px;'/>禁用设置</h2>";
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('rankImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_rank_icon.png') + "' style='width:50px;margin-bottom:-4px;'/>等阶设置</h2>";
              ...
              if (lib.config.qhly_enableCharacterMusic) {
                content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('musicImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_music_icon.png') + "' style='width:50px;margin-bottom:-4px;'/>音乐设置</h2>";
        ...
        //subView.pageButton[page].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/手杀标准UI/original/千幻聆音/newui_button_selected.png'));
        ...
        refreshRank = function () {
          if (subView.rank) {
            if (lib.config.qhly_showrarity) {
              ...
              if (rarity) {
                subView.rank.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/decade/rarity_' + game.getRarity(name) + '.png');
              }
            } else {
              subView.rank.hide();
            }
      ...
      game.qhly_initNewView = function (name, view, page, cplayer) {
        ...
        if (currentViewSkin.isQiLayout) {
          ...
          if (lib.config.qhly_hideShuimoCover) {
            subView.cover.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_shuimo_bg3.png');
          } else {
            subView.cover.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_shuimo_bg2.png');
          }
        ...
        subView.page = {
            ...
            refreshAfterGot: function (name, state) {
                  ...
                  if (this.getSkinAt(i)) {
                    ...
                    if (currentViewSkin.isQiLayout) {
                      if (currentSkin == this.getSkinAt(i).skinId) {
                        currentSkinView.qhBoard.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_skin_background_shuimo.png');
                    ...
                    if (level) {
                      ...
                      var img = null;
                      if (map[level]) {
                        img = "extension/手杀标准UI/original/千幻聆音/image/level_" + map[level] + ".png";
                      } else if (level.indexOf("#") == 0) {
                        var l2 = level.replace("#", "");
                        img = "extension/手杀标准UI/original/千幻聆音/image/" + l2 + ".png";
                      } else if (level.indexOf("$") == 0) {
              ...
              if (currentViewSkin.isQiLayout) {
                ...
                for (var audio of currentSkin.audios) {
                  ...
                  content += "<td>";
                  content += "<img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/手杀标准UI/original/千幻聆音/image/qhly_pic_playaudiobutton.png') + "' style='height:40px;' id='qhly_skin_skill_" + audio.id + "'/>"
                  if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
                ...
                for (var audio of currentSkin.audios) {
                  ...
                  content += "】"
                  content += "&nbsp;&nbsp;&nbsp;<img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/手杀标准UI/original/千幻聆音/image/qhly_pic_playaudiobutton.png') + "' style='height:23px;vertical-align:middle;' id='qhly_skin_skill_" + audio.id + "'/>"
                  if (lib.config.qhly_editmode && !game.qhly_isForbidEditTaici(name)) {
                    content += "<img src='" + lib.qhly_path + "image/qhly_editButton.png' style='height:25px;width:25px;'";
                    content += " id='qhly_skin_skill_edit_" + audio.id + "'";
                    content += "/>";
          ...
          config: {
            ...
            init: function (name, state) {
              ...
              var that = this;
              var content = "";
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('favouriteImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_fav.png') + "' style='width:50px'/>收藏设置</h2>";
              content += "<p>可以选择收藏此武将。进行自由选将操作时，可以更快找到此武将。</p>";
              content += "<p><span style='display:inline-block;height:30px;'><img id='qhconfig_checkbox_fav'/><span id='qhconfig_checkbox_text_fav' style='display:inline-block;position:relative;bottom:25%;'>收藏" + get.translation(name) + "</span></span></p>";

              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('forbidImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_forbid.png') + "' style='width:50px'/>禁用设置</h2>";
              content += "<p>可以选择在某模式下禁用或启用该武将。该设置将在重启游戏后生效。</p>"
              content += "<p><span style='display:inline-block;height:30px;'><img id='qhconfig_checkbox_banned_mode_all'/><span id='qhconfig_checkbox_text_all' style='display:inline-block;position:relative;bottom:25%;'>所有模式禁用</span></span></p>";
              for (var mode in lib.mode) {
                if (mode != 'connect') {
              ...
              content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('rankImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_rank_icon.png') + "' style='width:50px'/>等阶设置</h2>";
              content += "<p>可以设置" + get.translation(name) + "的等阶，重启后生效。</p>";
              content += "<p><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_rank_select'></select></p>";

              if (lib.config.qhly_enableCharacterMusic) {
                content += "<h2><img src='" + lib.assetURL + get.qhly_getCurrentViewSkinValue('musicImage', 'extension/手杀标准UI/original/千幻聆音/image/newui_music_icon.png') + "' style='width:50px'/>音乐设置</h2>";
                content += "<p>可以设置" + get.translation(name) + "的专属背景音乐，在游戏开始时将自动切换。</p>";
                content += "<p><select style='font-size:22px;font-family:'qh_youyuan';' id='qhconfig_music_select'></select></p>";
              }
              var extraConfigs = [];
        ...
        subView.pageButton[page].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_button_selected.png'));
        view.appendChild(subView.mp);
        var state = {
          name: name,
          currentPage: page,
        ...
        refreshRank = function () {
          if (subView.rank) {
            if (lib.config.qhly_showrarity) {
              ...
              var rarity = game.getRarity(state.name);
              if (rarity) {
                subView.rank.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/image/rarity_' + rarity + ".png");
              }
            } else {
              subView.rank.hide();
            }
        ...
        var showPage = function (pagename) {
          ...
          for (var k in subView.pageButton) {
            if (k == pagename) {
              subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonPressedImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_button_selected.png'));
            } else {
              subView.pageButton[k].setBackgroundImage(get.qhly_getIf(currentViewSkin.buttonImage, 'extension/手杀标准UI/original/千幻聆音/image/newui_button.png'));
            }
          }
        };
        state.useLihui = function () {
          ...
          if (use) {
            if (currentViewSkin.isLolBigLayout) {
              if (state.currentPage == 'skin') {
                ...
                subView.board.setBackgroundImage("extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_lol_bg_center_big.png");
                ...
                subView.board.setBackgroundImage("extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_lol_bg1_big.png");
            ...
            if (currentViewSkin.isLolBigLayout) {
              if (state.currentPage == 'skin') {
                ...
                subView.board.setBackgroundImage("extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_lol_bg_center_small.png");
                ...
                subView.board.setBackgroundImage("extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_lol_bg1.png");
                ...
                if (isNaN(hp1) || isNaN(hp2)) {
                  ...
                  if (hp2 >= 6) {
                    ...
                    for (var i = 0; i < hp2; i++) {
                      ...
                      if (i >= hp1) {
                        if (currentViewSkin.isQiLayout) {
                          img.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_shuimo_hpimg_gray.jpg');
                        } else if (currentViewSkin.isLolBigLayout) {
                          img.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_lol_hpimg_gray.png');
                        }
                      }
                    }
                    if (hujia) {
          ...
          if (viewAbstract.skin) {
            ...
            if (title) {
              ...
              if (['精品', '史诗', '传说', '限定'].contains(title)) {
                ...
                levelText.innerHTML = '';
                levelText.setBackgroundImage('extension/手杀标准UI/original/千幻聆音/image/level_' + obj[title] + ".png");
              } else {
          ...
          if (skills) {
            ...
            for (var skill of skills) {
              ...
              if (window.qhly_TrySkillAudio) {
                infoString += "<a style='color:#ffffff' href=\"javascript:window.qhly_TrySkillAudio('" + skill + "',{name:'" + name + "'},null,window.qhly_audio_which[\'" + skill + "\'],\'" + viewAbstract.skin + "\');window.qhly_audio_which[\'" + skill + "\']++;\"><img style=height:22px src=" + lib.assetURL + get.qhly_getCurrentViewSkinValue('skinPagePlayAudioButtonImage', 'extension/手杀标准UI/original/千幻聆音/image/qhly_pic_playaudiobutton.png') + "></a><br>";
              }
              infoString += "<br><br>";
              str += infoString;
            }
          ...
          Object.defineProperty(ui.click, 'charactercard', {
            ...
            set: function (value) {
              if (!lib.config.qhly_mentionConflitCC) {
                alert("你安装的扩展中，有扩展试图修改ui.click.charactercard，此行为与《千幻聆音》冲突，你可以关闭有冲突的功能。");
      ...
      get.nodeintro=function(node,simple,evt){
          ...
          if(node.name){
              if(get.character(node.name)){
                  ...
                  zhu.innerHTML = "<img style=width:30px src="+lib.assetURL+"extension/手杀标准UI/original/千幻聆音/qhly_skin_bt1.png>";
                  zhu.listen(function(){
          ...
          if(node.name2 && get.character(node.name2)){
              ...
              fu.innerHTML = "<img style=width:30px src="+lib.assetURL+"extension/手杀标准UI/original/千幻聆音/qhly_skin_bt2.png>";
              fu.listen(function(){
      ...
      game.qhly_autoChangeSkin = function () {
		...
		if (offline=='offline'&&lib.config.qhly_autoChangeSkin=='random') return;
        if (lib.config.qhly_autoChangeSkin && lib.config.qhly_autoChangeSkin != 'close') {
          _status.qhly_changeSkinFunc = function () {
            if (game && game.players && game.players.length) {
              ...
              if(pls.contains(game.me)) pls.remove(game.me);
              var names = [];
              for (var p of pls) {
              ...
              var func = function (arr, f) {
                ...
                game.qhly_getSkinList(n, function (ret, list) {
                  if (list && list.length) {
                    ...
                    list = list.filter(function (current) {
                      return current.toString().indexOf('娘化')==-1&&current.toString().indexOf('云涯')==-1;
                    });
                    if (list.length == 0) {
                      f(arr, f);
                      return;
          ...
          if(lib.config.qhly_autoChangeSkin=='random') {
            var timenum=[10,30,60,90,120,180,300].randomGet();
          }else {
            var timenum=lib.config.qhly_autoChangeSkin;
          }
          setTimeout(_status.qhly_changeSkinFunc, parseInt(timenum) * 1000);
        }
      };
      ...
      if(lib.config.qhly_randskin === undefined) {
          lib.config.qhly_randskin=true;
          game.saveConfig('extension_千幻聆音_qhly_randskin', true);
          game.saveConfig('qhly_randskin', true);
      }
      lib.skill._qhly_randskin = {
        forced: true,
        popup: false,
        //Helasisy修，调整一下发动时机，防止皮肤错位
        priority: -Infinity,
        lastDo: true,
        trigger: {
          global: 'gameStart',
        ...
        filter: function (event, player) {
          if(lib.config['extension_千幻聆音_qhly_randskin_me']&&player==game.me) return false;
          return lib.config.qhly_randskin;
        },
        content: function () {
          if (player.name || player.name1) {
            game.qhly_getSkinList((player.name&&player.name!='unknown') ? player.name : player.name1, function (ret, list) {
              if (list && list.length) {
                list.push(false);
                list = list.filter(function (current) {
                  return !game.qhly_skinIsBanned((player.name&&player.name!='unknown') ? player.name : player.name1, current);
                });
                list = list.filter(function (current) {
                  return current.toString().indexOf('娘化')==-1&&current.toString().indexOf('云涯')==-1;
                });
                if (list && list.length)
                  game.qhly_setCurrentSkin((player.name&&player.name!='unknown') ? player.name : player.name1, list.randomGet(), undefined, true);
              }
            }, false);
          }
          if (player.name2) {
      ...
      game.qhly_checkFileExist('extension/手杀标准UI/original/千幻聆音/music/', function (s) {
        if (s && game.getFileList) {
          game.getFileList('extension/手杀标准UI/original/千幻聆音/music/', function (folders, files) {
            if (files) {
              for (var f of files) {
                if (f.endsWith('.mp3')) {
                  var path = 'extension/手杀标准UI/original/千幻聆音/music/' + f;
      ...
      game.getFileList('extension/手杀标准UI/original/千幻聆音/plugins', function (folders, files) {
        if (files) {
          for (var file of files) {
            if (file.endsWith('.js')) {
...
//-----Q-----END-----
      ...
      lib.qhly_viewskin['shikongshuniu'] = {
        name: '时空枢纽',
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/sksn/newui_button_sksn.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/sksn/newui_button_selected_sksn.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/qhly_pic_playaudiobutton_wz.png',
        onchange: function () {
      ...
      lib.qhly_viewskin['sanguo2'] = {
        name: '三国2',
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/sanguo2/newui_button_sanguo2.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/sanguo2/newui_button_selected_sanguo2.png',
        onchange: function () {
      ...
      lib.qhly_viewskin['wangzhe'] = {
        name: '耀世星辉',
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_button_wz.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_button_selected_wz.png',
        favouriteImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_fav_wz.png',
        forbidImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_forbid_wz.png',
        checkBoxImage: 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png',
        checkBoxCheckedImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_checkbox_checked_wz.png',
        skillPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_playaudio_wz.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/qhly_pic_playaudiobutton_wz.png',
      ...
      lib.qhly_viewskin['ranqi'] = {
        name: '染柒的世界',
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/rq/newui_button_rq.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/rq/newui_button_selected_rq.png',
        favouriteImage: 'extension/手杀标准UI/original/千幻聆音/theme/rq/newui_fav_rq.png',
        forbidImage: 'extension/手杀标准UI/original/千幻聆音/theme/rq/newui_forbid_rq.png',
        checkBoxImage: 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png',
        checkBoxCheckedImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_checkbox_checked_wz.png',
      ...
      lib.qhly_viewskin['shuimo'] = {
        ...
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_button_shuimo.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_button_selected_shuimo.png',
        skillPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/shuimo/newui_playaudio_shuimo.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/shuimo/qhly_pic_playaudiobutton_shuimo.png',
        onchange: function () {
      ...
      lib.qhly_viewskin['decade'] = {
        name: '十周年',
        whr: 2.22,
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/button.jpg',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/buttonsel.jpg',
        favouriteImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/newui_fav_dc.png',
        forbidImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/newui_forbid_dc.png',
        rankImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/newui_rank_dc.png',
        musicImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/newui_music_dc.png',
        checkBoxImage: 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png',
        checkBoxCheckedImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_checkbox_checked_wz.png',
        skillPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/skillvoice.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/qhly_pic_playaudiobutton_wz.png',
      ...
      lib.qhly_viewskin['shousha'] = {
        name: '手杀',
        whr: 2.22,
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/chr_detail_skill_button_normal.jpg',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/chr_detail_skill_button_selected.jpg',
        favouriteImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/newui_fav_ss.png',
        forbidImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/newui_forbid_ss.png',
        rankImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/newui_rank_ss.png',
        musicImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/newui_music_ss.png',
        skinImage: 'extension/手杀标准UI/original/千幻聆音/theme/shousha/newui_skin_ss.png',
        checkBoxImage: 'extension/手杀标准UI/original/千幻聆音/image/newui_checkbox_unchecked.png',
        checkBoxCheckedImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/newui_checkbox_checked_wz.png',
        skillPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/decade/skillvoice.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/wz/qhly_pic_playaudiobutton_wz.png',
      ...
      lib.qhly_viewskin['lolbig'] = {
        ...
        isLolBigLayout: true,
        buttonTextSpace: false,
        favouriteImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_fav_lol.png',
        lihuiSupport: true,
        layoutType: 'lolbig',
        skillPageSkillNameColor: '#C0B588',
        skillPageDerivationSkillColor: '#00F5FF',
        skinPageSkillNameColor: '#FFFFFF',
        buttonImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_button_lol.png',
        buttonPressedImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_button_selected_lol.png',
        skillPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_playaudio_lol.png',
        skinPagePlayAudioButtonImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/qhly_pic_playaudiobutton_lol.png',
        checkBoxCheckedImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_checkbox_checked_lol.png',
        checkBoxImage: 'extension/手杀标准UI/original/千幻聆音/theme/lolbig/newui_checkbox_unchecked_lol.png',
        onchange: function () {
      ...
      lib.qhly_path = lib.assetURL + 'extension/手杀标准UI/original/千幻聆音/';
      window.qhly_version = 5;
      var cssUrl = lib.assetURL + 'extension/手杀标准UI/original/千幻聆音';
      ...
      "qhly_smallwinclosewhenchange": {
        ...
        "init": lib.config.qhly_smallwinclosewhenchange === undefined ? true : lib.config.qhly_smallwinclosewhenchange,
        onclick: function (item) {
      ...
      "qhly_randskin": {
        ...
        "init": lib.config.qhly_randskin === undefined ? true : lib.config.qhly_randskin,
        onclick: function (item) {
      ...
      "qhly_randskin_me": {
        "name": "不随自己",
        "intro": "上一个选项开启时，不会随机切换玩家自己的武将皮肤。",
        'init':true,
        //"init": lib.config.qhly_randskin === undefined ? true : lib.config.qhly_randskin,
        /*onclick: function (item) {
          game.saveConfig('extension_千幻聆音_qhly_randskin', item);
      ...
      "qhly_lutouType": {
        "name": "露头模式",
        "intro": "仅在适配露头生效时生效。",
        "init": lib.config.extension_千幻聆音_qhly_lutouType === undefined ? 'shousha' : lib.config.extension_千幻聆音_qhly_lutouType,
        item: {
          'decade': '十周年露头',
          'shousha': '手杀露头',
        },
      ...
      "lutouFixx": {
        "name": "露头修复",
        "init": 'all',
        "intro": "修复露头的显示错位问题，仅手杀风格窗口开启露头有效。（选择保留原画可以使原画不露头出框修复而其他修复，一般用于动皮的效果，且会使磨皮滤镜失效）",
        item: {
          'off': '关闭效果',
          'all': '启用效果',
          /*'yuanhua': '保留原画',*/
      ...
      "lvjingFixx": {
        "name": "磨皮滤镜",
        "init": false,
        "intro": "资料展示页的武将大图开启磨皮滤镜，可以缓解低清图片的锐化问题，同时增加切换皮肤的过渡效果，仅手杀风格的窗口有效。"
      },
      "qhly_skinButton": {
        "name": "头像显示换肤按钮",
      ...
      "qhly_originSkinPath": {
        "name": "本体武将皮肤目录",
        "intro": "可设置本体武将的皮肤目录。",
        "init": lib.config.qhly_originSkinPath === undefined ? "extension/手杀标准UI/original/千幻聆音/sanguoskin/" : lib.config.qhly_originSkinPath,
        "item": {
          "extension/手杀标准UI/original/千幻聆音/sanguoskin/": "千幻聆音目录",
          "extension/手杀标准UI/original/千幻聆音/sanguolutouskin/": "千幻露头目录",
          "image/skin/": "本体目录",
        },
        onclick: function (item) {
      ...
      "qhly_autoChangeSkin": {
        ...
        "init": lib.config.qhly_autoChangeSkin === undefined ? "random" : lib.config.qhly_autoChangeSkin,
        "item": {
          "close": "关闭",
          "10": "每10秒",
          "30": "每半分钟",
      ...
      "qhly_chooseButtonOrigin_guozhan":{
        "name": "国战选将显示原皮",
        "intro": "设置此选项，国战模式下选将界面只显示原皮（为了避免出现空皮肤的情况）。",
        "init": true,
      },
      "qhly_mvp": {
        "name": "播放MVP武将的胜利语音",
      ...
      "qhly_funcLoadInPrecontent": {
        ...
        "init": lib.config.qhly_funcLoadInPrecontent === undefined ? true : lib.config.qhly_funcLoadInPrecontent,
        onclick: function (item) {
      ...
      intro: "版本号："+"4.13.8.1"+"<br>对局内实时换肤换音扩展！<br>感谢七.提供的【水墨龙吟】界面素材。<br>感谢灵徒℡丶提供的【海克斯科技】界面素材。<br>感谢雷开发的十周年、手杀界面。<br>感谢以下群友参与了BUG反馈，并给出了可行的建议：<br>柚子 Empty city° ꧁彥꧂ 折月醉倾城 世中人 ᴀᴅɪᴏs 废城<b><br><br>",
      author: "玄武江湖工作室 & 雷",
      diskURL: "",
      forumURL: "",
      version: "4.13.8.1",
