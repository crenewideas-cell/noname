  ...
  var plugin = {
        ...
        skillControl: function (clear) {
          if (!ui.skillControl) {
            ...
            node.node = {
              ...
              specialEnable: ui.create.div('.special-enable', node),
              specialTrigger: ui.create.div('.special-trigger', node)
            };
          ...
          if (clear) {
            ...
            ui.skillControl.node.specialEnable.innerHTML = '';
            ui.skillControl.node.specialTrigger.innerHTML = '';
          }
      ...
      Object.assign(ui, {
        updateSkillControl: function (player, clear) {
          ...
          if (ui.skills2 && ui.skills2.skills.length) {
             ...
             try{
              gSkills.remove('_recasting')
             }catch(e){
               }                   
                  }
          skills.addArray(player.invisibleSkills.slice(0).filter(function(skill){
    ...
    controlElement: {
      add: function (skill, eSkills) {
        ...
        showSkills.forEach(function (item) {
          ...
          if (node) return;
          if (item.type === 'enable') {
          if(lib.noskillControl&&lib.noskillControl.contains(item.id))return;
          //兼容作弊立即生效
          if(item.id=='_ui_auto_cheat') return;
          //谋仁德只显示一个按钮！
          if(_status.event&&lib.skill&&lib.skill['sbrende_give']&&item.id.indexOf('sbrende')==0) {
            var skillsrd = [];
            if (ui.skills) skillsrd.addArray(ui.skills.skills);
            if (ui.skills2) skillsrd.addArray(ui.skills2.skills);
            if (ui.skills3) skillsrd.addArray(ui.skills3.skills);
            var rdforbid=(skillsrd.contains('sbrende_give'))?'sbrende':'sbrende_give';
            if(item.id==rdforbid) return;
          }
         // if(item.id=='lianhuan5' || item.id=='ollianhuan5' || item.id=='lianhuan2' || item.id=='twjichou_give' )return;
            let name = get.translation(item.name).slice(0, 4);
            //丈八蛇矛改成丈八矛
            if(item.id=='zhangba_skill') name='丈八矛';
            if(item.id=='ruyijingubang_skill') name='金箍棒';
            if(item.id=='txhj_dinglanyemingzhu') name='夜明珠';
            var cmjian = document.getElementsByClassName('cmjian')[0];
            //修改司马徽技能单独分离
            if (item.id == 'smh_huoji') {
            ...
            } else if (cmjian&&item.id == 'mbjuejin') {
              var tpm = get.translation(item.id).slice(0, 4);
              if(cmjian)  node = ui.create.div('.skillitem_mbjuejin', self.node.specialEnable);
              else  node = ui.create.div('.skillitem_mbjuejin', self.node.enable);
              var img = document.createElement('img');
              if(cmjian)  img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs1/' + tpm + '.png';  
              else img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs/' + tpm + '.png';    

              img.style.transform = 'scale(1)';
              img.style.marginTop = '-1px';
              img.style.filter = 'contrast(1.07) brightness(0.93)';
              node.appendChild(img);
            } else if (cmjian&&item.id == 'mbcmjiushi_1') {
              var tpm = get.translation(item.id).slice(0, 4);
              if(cmjian)  node = ui.create.div('.skillitem_mbcmjiushi', self.node.specialEnable);
              else node = ui.create.div('.skillitem_mbcmjiushi', self.node.enable);
              var img = document.createElement('img');
              if(cmjian)  img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs1/' + tpm + '.png';  
              else img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs/' + tpm + '.png';                       
              img.style.transform = 'scale(1)';
              ...
              img.style.filter = 'contrast(1.07) brightness(0.93)';
              node.appendChild(img);
            } else if (cmjian&&item.id == 'mbcmfangzhu') {
              var tpm = get.translation(item.id).slice(0, 4);
              if(cmjian)  node = ui.create.div('.skillitem_mbcmfangzhu', self.node.specialEnable);
              else node = ui.create.div('.skillitem_mbcmfangzhu', self.node.enable);
              var img = document.createElement('img');
              if(cmjian)  img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs1/' + tpm + '.png';  
              else img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs/' + tpm + '.png';                         
              img.style.transform = 'scale(1)';
              ...
              img.style.filter = 'contrast(1.07) brightness(0.93)';
              node.appendChild(img);
            } else if (lib.config['extension_手杀ui_xiandingskill_ss']&&lib.skill[item.id].limited) {
              node = ui.create.div('.xiandingji', self.node.enable, name);
            } else {
            ...
            if (showAddSpan) {
              ...
              var forbidother=false;
              //不允许重新检测技能的列表
              var fobidskill=['shencai','sbkeji','oltiaoxin'];
              if(fobidskill.contains(item.id)) forbidother=true;
              //count为该技能使用了的次数
              var count=game.me.getStat().skill[item.id];
              if(!count) count=0;
              //神裁单独判断 神张飞
                if (item.id=='shencai') {
					 //if(!count||count<=game.me.countMark('shencai')) {
					 var sknums=game.me.countMark('shencai') + 1 - count;
					 if(game.me.countMark('shencai')&&sknums>0) addSpan(node, sknums);
					 //}
					 boolRename = true;
                }
              //谋克己单独判断 谋吕蒙
                if (item.id=='sbkeji') {
				     if(!game.me.storage.sbkeji) {
					      var sknums=item.info.usable - count;
					      if(sknums>0) addSpan(node, sknums);
				     }
					 boolRename = true;
                }
              //谋挑衅单独判断 谋姜维
                if (item.id=='oltiaoxin') {
				     if(count>0 && !game.me.hasSkill('oltiaoxin2')) {
					      var sknums=item.info.usable - count;
					      if(sknums>0) addSpan(node, sknums);
				     }
					 boolRename = true;
                }
              if (!forbidother && item.info && typeof item.info.usable == 'number' && (item.info.usable > 1)) {
                if (game.me.getStat()) {
              ...
              } else if (!forbidother && item.info && item.info.subSkill && item.info.subSkill.add && (item.info.subSkill.add.usable > 1)) {
                if (game.me.getStat()) {
                  if (game.me.getStat().skill) {
              ...
              } else if (!forbidother && item.id == 'dbquedi') {//却敌特殊机制，每回合限一次usable=1，能重置次数,单独判断
            ...
            if (item.id) {
              if (game.me.hasSkill('baiban') && baibanSkillBlocker && baibanSkillBlocker.contains(item.id)) {
                ...
                node.classList.add('lockedSkill');//嘿嘿哈哈
          ...
          //node = ui.create.div('.skillitem', self.node.trigger, item.name.slice(0, 4));
           ...
           var cmjian = document.getElementsByClassName('cmjian')[0];
            //潜龙曹髦
          if(cmjian&&item.id == 'mbcmqingzheng')  {
            var tpm = get.translation(item.id).slice(0, 4);
            node = ui.create.div('.skillitem-mbcmqingzheng', self.node.specialTrigger);
            var img = document.createElement('img');      
            img.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/skillimgs1/' + tpm + '.png';
            img.style.transform = 'scale(0.92) translateX(3px)';
            img.style.filter = 'contrast(1.07) brightness(0.93)';
            ...
            img2.src = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/images/qianlong_btn_unget.png';
            img2.style.transform = 'scale(0.85)';
            /*img2.style.width = '108px';*/
            img2.style.width = '113.5px';
            img2.style.height = '54px';
            img2.style.zIndex = '-1';
            img2.style.marginTop = '5px';
            img2.style.marginLeft = '-108px';
          ...
          if (item.id) {//这里修改5{//这是被动技能的上锁和按钮切换
            ...
            if (game.me.hasSkill('baiban') && baibanSkillBlocker && baibanSkillBlocker.contains(item.id)) {
              //白板锁
              var img = ui.create.div('.suo.baibansuo', node, "");
              img.style.position = "absolute";
              node.style['-webkit-text-fill-color'] = 'silver';//失效变灰
              node.style['-webkit-text-stroke'] = '0.8px rgba(0,0,0,0.55)';
            } else if (game.me.hasSkill('fengyin') && fengyinSkillBlocker && fengyinSkillBlocker.contains(item.id)) {
              //封印锁
              var img = ui.create.div('.suo.fengyinsuo', node, "");
              img.style.position = "absolute";
              node.style['-webkit-text-fill-color'] = 'silver';//失效变灰
              node.style['-webkit-text-stroke'] = '0.8px rgba(0,0,0,0.55)';
            } else if (game.me.shixiaoedSkills.contains(item.id)) {
              //技能锁
              var img = ui.create.div('.suo.jinengsuo', node, "");
              img.style.position = "absolute";
              node.style['-webkit-text-fill-color'] = 'silver';//失效变灰
              node.style['-webkit-text-stroke'] = '0.8px rgba(0,0,0,0.55)';
            };
      ...
      update: function () {
       ...
       try {skills.remove('lianhuan5');
       skills.remove('lianhuan2');
       skills.remove('ollianhuan5');}
       catch(e){}
        ...
        Array.from(this.node.specialEnable.childNodes).forEach(function (item) {
          var cmjian = document.getElementsByClassName('cmjian')[0];  

          if (skills.contains(item.dataset.id) && _status.event.player==game.me/*这里*/) {
            item.classList.add('usable'); 
          } else {
            item.classList.remove('usable');
          }

          if (_status.event.skill === item.dataset.id && _status.event.player == game.me && (_status.event.name == 'chooseToUse' || _status.event.name == 'chooseToRespond')) {
            item.classList.add('select');
          } else {
            item.classList.remove('select');
          }
          var img2 = item.querySelector('img');
          if (img2 != null) {
            if (item.classList.contains('on') || item.classList.contains('select')) {
              if(cmjian)  img2.style.transform = 'scale(1.09,1.11)';
              else  img2.style.transform = 'scale(1.03,1.05)';
            } else {
              if(cmjian)  img2.style.transform = 'scale(0.94,0.96)';
              else  img2.style.transform = 'scale(0.88,0.9)';
            };
          };
        });
        ...
        ui.skillControl.node.enable.style.width = ui.skillControl.node.enable.childNodes.length > 2
          ? '200px' : ui.skillControl.node.enable.childNodes.length > 0
            ? '120px' : '0px';
         ui.skillControl.node.trigger.style.width = ui.skillControl.node.enable.childNodes.length > 2
          ? '200px' : '120px';
         // ui.skillControl.style.width = ui.skillControl.node.enable.childNodes.length > 2
    //      ? '200px' :'120px';

           //潜龙曹髦
          var cmjian = document.getElementsByClassName('cmjian')[0];  
          var zhuangshiBar = game.me?.ZhuangshiBar;//game.me?.hasSkill('potzhuangshi')&&get.info("potzhuangshi").fleshBar;
          var hasWei = get.info("lianzhaoEffect")?.hasWei?.(game.me);
          if((cmjian||zhuangshiBar||hasWei)&&game.me.isAlive()){
            var rightBar = '15px';
            if(hasWei) rightBar = '57px';
            if(cmjian) rightBar = '100px';
            ui.skillControl.node.enable.style.right = rightBar;//cmjian?'100px':'57px';
            
            //小小的修复，判定区上移
            var nowHeight=ui.skillControl.node.enable.offsetHeight+ui.skillControl.node.trigger.offsetHeight;
            var toHeight=0;
            if(nowHeight<135) toHeight=135-nowHeight
            ui.skillControl.node.enable.style.marginTop = toHeight+'px';
            
            ui.skillControl.node.trigger.style.right = rightBar;//cmjian?'100px':'57px';
            ui.skillControl.node.trigger.style.bottom = '6px';
            ui.skillControl.node.enable.style.bottom = '6px';   
            //ui.skillControl.node.specialEnable.style.right = '33px';
            //ui.skillControl.node.specialTrigger.style.right = '33px';
            ui.skillControl.node.specialEnable.style.right = '39.5px';
            ui.skillControl.node.specialTrigger.style.right = '44.5px';
            //ui.skillControl.node.specialEnable.style.zoom = '0.71';
            //ui.skillControl.node.specialTrigger.style.zoom = '0.71';
            ui.skillControl.node.specialEnable.style.zoom = '0.672';
            ui.skillControl.node.specialTrigger.style.zoom = '0.672';
            //ui.skillControl.node.specialEnable.style.bottom = '49px';   
            //ui.skillControl.node.specialTrigger.style.bottom = '16px'; 
            ui.skillControl.node.specialEnable.style.bottom = '39.5px';   
            ui.skillControl.node.specialTrigger.style.bottom = '6.5px';   
            ui.skillControl.node.specialEnable.style.width =  '119px';             
          } else if(!cmjian&&game.me.isAlive()){
            ui.skillControl.node.enable.style.right = '0px';
            ui.skillControl.node.trigger.style.right = '0px';
            ui.skillControl.node.trigger.style.bottom = '0px';
            ui.skillControl.node.enable.style.bottom = '0px';   
          }
    ...
    updateSkillMarks: function (player, skills1, skills2) {
      ...
      for (var k in skills1) {
        ...
        if (!item) {
          if (!info.zhuanhuanji) item = ui.create.div('.skillMarkItem.xiandingji', node, get.skillTranslation(k, player),abb);
          //如果不是转换技就调用限定技的标记      
          else {
            //判断图片存在，不存在就用底图
            var url = lib.assetURL + 'extension/手杀标准UI/original/手杀ui/skill/images/' + k + '_yang.png';
            ...
            try {//容错函数，优先执行try的内容，try报错时自动执行catch内容
              var a = bridge.resourceExists(url);
              if (a) {
                item = ui.create.div('.skillMarkItem.zhuanhuanji', node, '');
                item.setBackgroundImage('extension/手杀标准UI/original/手杀ui/skill/images/' + k + '_yang.png');
              }
            }
            catch (err) {
        ...
        if (!item) { item = ui.create.div('.skillMarkItem.zhuanhuanji', node, get.skillTranslation(k, player)); item.setBackgroundImage('extension/手杀标准UI/original/手杀ui/skill/images/ditu_yang.png'); item.style.setProperty('--w', '42px'); } if (skills1[k]) item.classList.add('used');
