			...
			app.waitAllFunction([
				function (next) {
					if (lib.device == 'ios' || lib.device == 'android') {
						...
						lib.init.css(lib.assetURL + 'extension/手杀标准UI/original/' + app.name + '/' + plugin.name, 'main2_window', next);
					}
				},
			], next);
		},
				...
				showSkills.forEach(function (item) {
					...
					if (item.type === 'enable') {
						...
						if (lib.skill[item.id].limited)
							node = ui.create.div('.xiandingji', self.node.enable, get.translation(item.name).slice(0, 2));
						else node = ui.create.div('.skillitem', self.node.enable, get.translation(item.name).slice(0, 2)); //咸鱼大佬提供代码
