...
(function(duilib){
	...
	duilib.AnimationPlayer = (function(){
		...
		// Share on-demand and background loads of the same skeleton.
...
decadeModule.import(function(lib, game, ui, get, ai, _status){
	decadeUI.animation = (function(){
		...
		var fileNameList = (fileList || []).slice();
 ...
 fileNameList.sort((a,b)=>(priorityEffects.includes(b.name)?1:0)-(priorityEffects.includes(a.name)?1:0));
		...
		var read = function() {
			if (fileNameList.length) {
				if(!priorityEffects.includes(fileNameList[0].name)&&!game.roundNumber){setTimeout(read,500);return;}
 var file = fileNameList.shift();
				if (file.follow) {
					...
					animation.cap.loadSpine(file.name, file.fileType, function(){
						setTimeout(read,150);
					}, function(){setTimeout(read,150);});
				} else {
					//	这个是专门播放全屏位置的动画
					animation.loadSpine(file.name, file.fileType, function(){
						setTimeout(read,150);
						// Parse skeletons when played, rather than decoding the whole library during selection.
					}, function(){setTimeout(read,150);});
				}
			}
		};setTimeout(read,150);
		...
		var skillAnimation = (function(){
			var defines = {
				skill:{
					...
					lebu: { skill: 'lebu', name: 'SZN_lebusishu', scale: 0.7 },
					bingliang: { skill: 'bingliang', name: 'SZN_bingliangcunduan', scale: 0.8, y: [0,0.3] },
				...
				lib.skill._jiuwo_ = {
					...
					content: function () {
					    ...
					    if (!player.storage._jiuwo_ ) {
					        player.storage._jiuwo_ = decadeUI.animation.playSpine({ name: 'SZN_jiuwo', loop: true, }, { x: [0, 0.55], scale: 0.85, parent: player });
					    }
			        }
				};
				//死亡或脱离濒死，取消救我
    ...
    lib.skill._dyjinengtx = {
        ...
        content: function() {
            ...
            dcdAnim.loadSpine('ZWshen', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWshen',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZWwei', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWwei',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZWshu', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWshu',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZWwu', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWwu',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZWqun', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWqun',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZWjin', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZWjin',
                }, {
                    scale: 0.7,
            ...
            dcdAnim.loadSpine('ZW', 'skel', function() {
                dcdAnim.playSpine({
                    name: 'ZW',
                }, {
                    scale: 0.7,
            ...
            if (!info.equipSkill) {
                ...
                if (info.zhuanhuanji) {
                  ...
                  /*  if (player.group == 'shen'){
                    dcdAnim.loadSpine('SLshen', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLshen',
                        }, {
                            scale: 0.8,
                    ...
                    dcdAnim.loadSpine('SLwei', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLwei',
                        }, {
                            scale: 0.8,
                    ...
                    dcdAnim.loadSpine('SLshu', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLshu',
                        }, {
                            scale: 0.8,
                    ...
                    dcdAnim.loadSpine('SLwu', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLwu',
                        }, {
                            scale: 0.8,
                    ...
                    dcdAnim.loadSpine('SLjin', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLjin',
                        }, {
                            scale: 0.8,
                    ...
                    dcdAnim.loadSpine('SLqun', 'skel', function() {
                        dcdAnim.playSpine({
                            name: 'SLqun',
                        }, {
                            scale: 0.8,
                    ...
                    if (player.storage.dyjinengtx && player.storage.dyjinengtx != 0) {
                        dcdAnim.loadSpine('effect_jineng_SZN', 'skel', function() {
                            dcdAnim.playSpine({
                                name: 'effect_jineng_SZN',
                                speed: 2,
                            }, {
                                scale: 1,
                                x: [0, 0.52],
				...
				lib.skill._losetl_ = {
					...
					forced: true,
					content: function () {
						decadeUI.animation.playSpine({name: 'SZN_loseHp',speed: 0.8 }, { scale: 0.6, parent: player });
						game.playAudio('../extension/手杀标准UI/original/十周年UI/audio/SZN_loseHp.mp3');
				    }
				}
				
                //锦囊牌特效
				lib.skill._jinnangpai_ = {
                ...
                if (trigger.card.name == 'taoyuan' && get.type(trigger.card) == 'trick') {//桃园结义
                    decadeUI.animation.playSpine({ name: 'SZN_taoyuanjieyi', scale: 0.9, speed: 0.8, },);}
                if (trigger.card.name == 'nanman' && get.type(trigger.card) == 'trick') {//南蛮入侵
                    decadeUI.animation.playSpine({ name: 'SZN_nanmanruqin', scale: 0.8 },);
                    game.playAudio('../extension/手杀标准UI/original/十周年UI/audio/nanmanruqin.mp3');}
                if (trigger.card.name == 'wanjian' && get.type(trigger.card) == 'trick') {//万箭齐发
                    decadeUI.animation.playSpine({ name: 'effect_wanjianqifa_full', scale: 0.9 },);
                    game.playAudio('../extension/手杀标准UI/original/十周年UI/audio/wanjianqifa.mp3');}
                    }
                }
				...
				lib.skill._jiubuff_ = {
					...
					audio: false,
					content: function () {
						player.storage._jiubuff_ = decadeUI.animation.playSpine({ name: 'jiubuff', speed: 1, loop: true, }, { scale: 0.5, parent: player });
					},
				}
				//酒特效消失//
				lib.skill._jiubuff_xiaoshi_ = {
					trigger: {
						player: ["useCard", "phaseAfter", "dieAfter"]
					},
					priority: 20,
					firstDo: true,
					forceDie: true,
                ...
                //-----------数值特效---------//
			lib.skill._player_hp = {
				trigger: { player: ["damageBegin5", "recoverBegin"], },
				forced: true,
				charlotte: true,
				priority: -1,
				filter: function (event, player) {
				...
				content: function () {
					...
					event.name = event.triggername;
					event.actionname = event.num;
					if (event.name == "damageBegin5") {
						decadeUI.animation.playSpine({
							name: 'shuzi',
							action: event.actionname,
							speed: 0.6,
						}, { scale: 0.5, parent: player });
					} else if (event.name == "recoverBegin") {
						decadeUI.animation.playSpine({
							name: 'shuzi2',
							action: event.actionname,
							speed: 0.6,
