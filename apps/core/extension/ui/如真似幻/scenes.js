        ...
        function yX() {
            ...
            for (let kw in lib["imported"][KX(0x2fd, "6)6d")]) {
                ...
                for (let kr in lib["imported"]["character"][kw]["character"]) {
                    ...
                    ku["addChild"](kt), ku["setChildIndex"](kt, 0x0), ku["interactive"] = !![], ku["on"]("pointerup", event => { if (event.data.global.x === ku.startX) lifecycle.character(ku.name); }), ku["on"]("pointerdown", kD => { ku["startX"] = kD["data"]["global"]["x"]; }), ku["addChild"](kM); lifecycle.portraits.fit(kM, ku);
                    const kW = {};
