...
将待整理 ZIP 放入 `扩展包/武将扩展` 后，在项目根目录执行 `pnpm extensions:organize`。
结果位于 `扩展包/整理结果`，可从中文整理报告选择生成的 ZIP 再导入游戏，或执行 `pnpm extensions:install` 批量安装到工程。
新增、修改或删除原包后，重复执行该命令即可增量更新。整理不等于安装，也不影响已安装扩展。
